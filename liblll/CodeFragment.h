...
CodeFragment(sp::utree const &_t, CompilerState &_s,
             ReadCallback const &_readFile, bool _allowASM = false);
...
Assembly &assembly(CompilerState const &_cs) {
	finalise(_cs);
	return m_asm;
}
...
template <class T> static void error() {
	BOOST_THROW_EXCEPTION(T());
}
