...
CodeFragment::CodeFragment(sp::utree const &_t, CompilerState &_s,
                           ReadCallback const &_readFile, bool _allowASM)
  ...
  */
  ...
  case sp::utree_type::string_type: {
    auto sr = _t.get<sp::basic_string<boost::iterator_range<char const *>,
  ...
  case sp::utree_type::symbol_type: {
    ...
    if (_allowASM && c_instructions.count(us) && validAssemblyInstruction(us))
      ...
      m_asm.append(_s.outers.at(s).m_asm);
             ...
             us.find_first_not_of("QWERTYUIOPASDFGHJKLZXCVBNM1234567890_-") ==
                 string::npos) {
...
void CodeFragment::constructOperation(sp::utree const &_t, CompilerState &_s) {
  if (_t.tag() == 0 && _t.empty())
    ...
    case 0: {
      auto sr = _t.front()
                    .get<sp::basic_string<boost::iterator_range<char const *>,
                                          sp::utree_type::symbol_type>>();
    ...
    auto firstAsString = [&]() {
      ...
      if (i.which() == sp::utree_type::string_type) {
        auto sr = i.get<sp::basic_string<boost::iterator_range<char const *>,
                                         sp::utree_type::string_type>>();
    ...
    auto varAddress = [&](string const &n, bool createMissing = false) {
      ...
      if (it == _s.vars.end()) {
        if (createMissing) {
          ...
          bool ok;
          tie(it, ok) =
              _s.vars.insert(make_pair(n, make_pair(_s.stackSize, 32)));
          _s.stackSize += 32;
        } else
    ...
    if (us == "ASM") {
      ...
      if (contents.empty())
        error<InvalidName>(std::string("File not found (or empty): ") +
                           fileName);
      ...
      for (auto const &i : _t) {
        if (ii == 1) {
          ...
          if (i.which() == sp::utree_type::string_type) {
            auto sr =
                i.get<sp::basic_string<boost::iterator_range<char const *>,
                                       sp::utree_type::string_type>>();
          ...
          if (_t.size() == 3) {
            /// NOTE: some compilers could do the assignment first if this is
            ...
            for (auto const &j : i) {
              ...
              auto sr =
                  j.get<sp::basic_string<boost::iterator_range<char const *>,
                                         sp::utree_type::symbol_type>>();
      ...
      for (auto const &i : _t) {
        if (ii == 0) {
          ...
          auto sr = i.get<sp::basic_string<boost::iterator_range<char const *>,
                                           sp::utree_type::string_type>>();
          data.insert(data.end(), (byte const *)sr.begin(),
                      (byte const *)sr.end());
    ...
    std::map<std::string, Instruction> const c_arith = {
        ...
        {"%", Instruction::MOD}, {"&", Instruction::AND},
        {"|", Instruction::OR},  {"^", Instruction::XOR}};
    std::map<std::string, pair<Instruction, bool>> const c_binary = {
        {"<", {Instruction::LT, false}},   {"<=", {Instruction::GT, true}},
        {">", {Instruction::GT, false}},   {">=", {Instruction::LT, true}},
        {"S<", {Instruction::SLT, false}}, {"S<=", {Instruction::SGT, true}},
        {"S>", {Instruction::SGT, false}}, {"S>=", {Instruction::SLT, true}},
        {"=", {Instruction::EQ, false}},   {"!=", {Instruction::EQ, true}}};
    std::map<std::string, Instruction> const c_unary = {
        {"!", Instruction::ISZERO}, {"~", Instruction::NOT}};
    ...
    if (_s.macros.count(make_pair(s, code.size()))) {
      ...
      int startDeposit = m_asm.deposit();
      int targetDeposit =
          hasDefault ? code[code.size() - 1].m_asm.deposit() : 0;
      ...
      // The targets - appending in reverse makes the top case the most
      // efficient.
      if (code.size() > 1) {
        auto end = m_asm.appendJump();
      ...
      // (alloc N):
      //  - Evaluates to (msize) before the allocation - the start of the
      //  allocated memory
      ...
      auto subPush =
          m_asm.appendSubroutine(make_shared<Assembly>(code[0].assembly(ns)));
      m_asm.append(Instruction::DUP1);
      if (code.size() == 3) {
        requireDeposit(2, 1);
