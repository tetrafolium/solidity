...
void CompilerState::populateStandard() {
  static const string s =
      ...
      // Alternative macro version of alloc, which is currently implemented in
      // the parser
      // "(def 'alloc (n) (raw (msize) (when n (pop (mload (+ (msize) (& (- n 1)
      // (~ 0x1f))))))))"
      "(def 'allgas (- (gas) 21))"
      "(def 'send (to value) (call allgas to value 0 0 0 0))"
      "(def 'send (gaslimit to value) (call gaslimit to value 0 0 0 0))"
      // NOTE: in this macro, memory location 0 is set in order to force msize
      // to be at least 32 bytes.
      "(def 'msg (gaslimit to value data datasize outsize) { [0]:0 [0]:(msize) "
      "(call gaslimit to value data datasize @0 outsize) @0 })"
      "(def 'msg (gaslimit to value data datasize) { (call gaslimit to value "
      "data datasize 0 32) @0 })"
      "(def 'msg (gaslimit to value data) { [0]:data (msg gaslimit to value 0 "
      "32) })"
      "(def 'msg (to value data) { [0]:data (msg allgas to value 0 32) })"
      "(def 'msg (to data) { [0]:data (msg allgas to 0 0 32) })"
      // NOTE: in the create macros, memory location 0 is set in order to force
      // msize to be at least 32 bytes.
      "(def 'create (value code) { [0]:0 [0]:(msize) (create value @0 (lll "
      "code @0)) })"
      ...
      "(def 'makeperm (name pos) { (def name (sload pos)) (def name (v) "
      "(sstore pos v)) } )"
      "(def 'permcount 0)"
      "(def 'perm (name) { (makeperm name permcount) (def 'permcount (+ "
      "permcount 1)) } )"
      "(def 'ecrecover (hash v r s) { [0] hash [32] v [64] r [96] s (msg "
      "allgas 1 0 0 128) })"
