...
BOOST_AUTO_TEST_CASE(value_types) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode); callContractFunction("f()");
	              REQUIRE_LOG_DATA(encodeArgs(10, u256(65534), u256(0x121212),
	                                          u256(-1), string("\x1b\xab\xab"),
	                                          true, u160(u256(-5)))); )
}
...
BOOST_AUTO_TEST_CASE(string_literal) {
	...
	BOTH_ENCODERS(
		compileAndRun(sourceCode); callContractFunction("f()"); REQUIRE_LOG_DATA(
			encodeArgs(0x60, string("abcde"), 0xa0, 6, string("abcdef"), 0x8b,
			           string("abcdefabcdefgehabcabcasdfjklabcdefabcedefghabcabca"
			                  "sdfjklabcdefabcdefghabcabcasdfjklabcdeefabcdefghab"
			                  "cabcasdefjklabcdefabcdefghabcabcasdfjkl"))); )
}
...
BOOST_AUTO_TEST_CASE(conversion) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode); callContractFunction("f()");
	              REQUIRE_LOG_DATA(encodeArgs(string(3, 0) + string("\x0a"),
	                                          string("\xf1\xf2"), 0xff, 0xff,
	                                          u256(-1), u256(1))); )
}
...
BOOST_AUTO_TEST_CASE(memory_array_one_dim) {
	...
	REQUIRE_LOG_DATA(encodeArgs(10, 0x60, 11, 3, u256("0xfffffffe"),
	                            u256("0xffffffff"), u256("0x100000000")));
...
BOOST_AUTO_TEST_CASE(memory_array_two_dim) {
	...
	NEW_ENCODER(compileAndRun(sourceCode); callContractFunction("f()");
	            REQUIRE_LOG_DATA(encodeArgs(10, 0x60, 11, 0x40, 0xc0, 3, 7,
	                                        0x0506, u256(-1), 2, 4, 5)); )
}
...
BOOST_AUTO_TEST_CASE(memory_byte_array) {
	...
	NEW_ENCODER(
		compileAndRun(sourceCode); callContractFunction("f()");
		REQUIRE_LOG_DATA(encodeArgs(10, 0x60, 11, 2, 0x40, 0xc0, 66,
		                            string("abcabcdefghjklmnopqrsuvwabcdefgijklmn"
		                                   "opqrstuwabcdefgijklmnoprstuvw"),
		                            63,
		                            string("abcdefghijklmnopqrtuvwabcfghijklmnopq"
		                                   "stuvwabcdeghijklmopqrstuvw"))); )
}
...
BOOST_AUTO_TEST_CASE(storage_byte_array) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode); callContractFunction("f()");
	              REQUIRE_LOG_DATA(
			      encodeArgs(0x40, 0x80, 31,
			                 string("123456789012345678901234567890a"), 75,
			                 string("ffff123456789012345678901234567890afffff"
			                        "ffff123456789012345678901234567890a"))); )
}
...
BOOST_AUTO_TEST_CASE(storage_array_compact) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode); callContractFunction("f()");
	              REQUIRE_LOG_DATA(encodeArgs(0x20, 8, u256(-1), 2, u256(-3), 4,
	                                          u256(-5), 6, u256(-7), 8)); )
}
...
BOOST_AUTO_TEST_CASE(external_function) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode);
	              ...
	              string functionIdF =
			      asString(m_contractAddress.ref()) +
			      asString(FixedHash<4>(dev::keccak256("f(uint256)")).ref());
...
BOOST_AUTO_TEST_CASE(external_function_cleanup) {
	...
	BOTH_ENCODERS(
		compileAndRun(sourceCode); callContractFunction("f(uint256)", u256(0));
...
BOOST_AUTO_TEST_CASE(calldata) {
	...
	string s("abcdef");
	string t("abcdefgggggggggggggggggggggggggggggggggggggggghhheeeeeeeeeeeeeeeeee"
	         "eeeeeeeeeeeeeeeeeeeeeeeeggg");
	bool newEncoder = false;
	BOTH_ENCODERS(compileAndRun(sourceCode);
	              callContractFunction("f(bytes)", 0x20, s.size(), s);
	              // The old encoder did not pad to multiples of 32 bytes
	              REQUIRE_LOG_DATA(encodeArgs(0x20, s.size()) +
	                               (newEncoder ? encodeArgs(s) : asBytes(s)));
	              callContractFunction("f(bytes)", 0x20, t.size(), t);
	              REQUIRE_LOG_DATA(encodeArgs(0x20, t.size()) +
	                               (newEncoder ? encodeArgs(t) : asBytes(t)));
	              newEncoder = true; )
}
...
BOOST_AUTO_TEST_CASE(function_name_collision) {
	...
	BOTH_ENCODERS(compileAndRun(sourceCode);
	              BOOST_CHECK(callContractFunction("f(uint256)", encodeArgs(0)) ==
	                          encodeArgs(7));
	              BOOST_CHECK(callContractFunction("f(uint256)", encodeArgs(1)) ==
	                          encodeArgs(1)); )
}
...
BOOST_AUTO_TEST_CASE(structs) {
	...
	NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
	            bytes encoded = encodeArgs(u256(7), 0x40, 8, 9, 0x80, 10, 3, 11,
	                                       0, 12, 0, 0, 13);
