...
BOOST_AUTO_TEST_CASE(value_types) {
  ...
  BOTH_ENCODERS(compileAndRun(sourceCode); ABI_CHECK(
                    callContractFunction(
                        "f(uint256,uint16,uint24,int24,bytes3,bool,address)", 1,
                        2, 3, 4, string("abc"), true, u160(m_contractAddress)),
                    encodeArgs(u256(20)));)
}
...
BOOST_AUTO_TEST_CASE(enums) {
  ...
  BOTH_ENCODERS(
      ...
      ABI_CHECK(callContractFunction("f(uint8)", 2),
                (newDecoder ? encodeArgs() : encodeArgs(2)));
      ABI_CHECK(callContractFunction("f(uint8)", u256(-1)),
                (newDecoder ? encodeArgs() : encodeArgs(u256(0xff))));
      newDecoder = true;)
}
...
BOOST_AUTO_TEST_CASE(cleanup) {
  ...
  BOTH_ENCODERS(
      compileAndRun(sourceCode);
      ABI_CHECK(callContractFunction("f(uint16,int16,address,bytes3,bool)", 1,
                                     2, 3, "a", true),
                encodeArgs(u256(1), u256(2), u256(3), string("a"), true));
      ABI_CHECK(callContractFunction("f(uint16,int16,address,bytes3,bool)",
                                     u256(0xffffff), u256(0x1ffff), u256(-1),
                                     string("abcd"), u256(4)),
                encodeArgs(u256(0xffff), u256(-1), (u256(1) << 160) - 1,
                           string("abc"), true));)
}
...
BOOST_AUTO_TEST_CASE(fixed_arrays) {
  ...
  BOTH_ENCODERS(
      compileAndRun(sourceCode);
      bytes args = encodeArgs(1, 2, 3, 11, 12, 21, 22, 31, 32, 1, 2, 1);
      ABI_CHECK(callContractFunction(
                    "f(uint16[3],uint16[2][3],uint256,uint256,uint256)", args),
...
BOOST_AUTO_TEST_CASE(dynamic_arrays) {
  ...
  BOTH_ENCODERS(
      compileAndRun(sourceCode);
      bytes args = encodeArgs(6, 0x60, 9, 7, 11, 12, 13, 14, 15, 16, 17);
      ABI_CHECK(callContractFunction("f(uint256,uint16[],uint256)", args),
...
BOOST_AUTO_TEST_CASE(dynamic_nested_arrays) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode);
              bytes args = encodeArgs(0x12, 4 * 0x20, 17 * 0x20, 0x13,
                                      // b
                                      3, 3 * 0x20, 6 * 0x20, 11 * 0x20, 2, 85,
                                      86, 4, 101, 102, 103, 104, 0,
                                      // c
                                      3 * 0x20, 6 * 0x20, 17 * 0x20, 1, 0, 117,
                                      5, 0, 0, 0, 133, 0, 0, 0, 0, 0, 0, 0);

              bytes expectation = encodeArgs(0x12, 3, 4, 0x66, 5, 0x85, 0x13);
              ABI_CHECK(callContractFunction("test()"), expectation); ABI_CHECK(
                  callContractFunction(
                      "f(uint256,uint16[][],uint256[2][][3],uint256)", args),
                  expectation);)
}
...
BOOST_AUTO_TEST_CASE(byte_arrays) {
  ...
  BOTH_ENCODERS(
      compileAndRun(sourceCode);
      bytes args = encodeArgs(6, 0x60, 9, 7, "abcdefg");
      ABI_CHECK(callContractFunction("f(uint256,bytes,uint256)", args),
                encodeArgs(u256(6), u256(7), "d", 9));
      ABI_CHECK(callContractFunction("f_external(uint256,bytes,uint256)", args),
                encodeArgs(u256(6), u256(7), "d", 9));)
}
...
BOOST_AUTO_TEST_CASE(calldata_arrays_too_large) {
  ...
  BOTH_ENCODERS(
      compileAndRun(sourceCode);
      bytes args = encodeArgs(6, 0x60, 9, (u256(1) << 255) + 2, 1, 2);
      ABI_CHECK(callContractFunction("f(uint256,uint256[],uint256)", args),
...
BOOST_AUTO_TEST_CASE(decode_function_type) {
  ...
  BOTH_ENCODERS(compileAndRun(sourceCode, 0, "C");
  ...
  NEW_ENCODER(
      compileAndRun(
          ...
          encodeArgs(
              ...
              3, 0x21, 0x22, 0x23,
              // c
              0x40, 0x80, 8, string("abcdefgh"), 52,
      ...
      ABI_CHECK(callContractFunction("_b(uint256)", 2), encodeArgs(0x23));
      ABI_CHECK(callContractFunction("_b(uint256)", 3), encodeArgs());
      ABI_CHECK(callContractFunction("_c(uint256)", 0),
                encodeArgs(0x20, 8, string("abcdefgh")));
      ABI_CHECK(
          callContractFunction("_c(uint256)", 1),
          encodeArgs(
              0x20, 52,
              string("ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ")));
...
BOOST_AUTO_TEST_CASE(short_input_value_type) {
  ...
  bool newDecoder = false;
  BOTH_ENCODERS(compileAndRun(sourceCode);
                ABI_CHECK(callContractFunction("f(uint256,uint256)", 1, 2),
                          encodeArgs(1));
                ABI_CHECK(callContractFunctionNoEncoding("f(uint256,uint256)",
                                                         bytes(64, 0)),
                          encodeArgs(0));
                ABI_CHECK(callContractFunctionNoEncoding("f(uint256,uint256)",
                                                         bytes(63, 0)),
                          newDecoder ? encodeArgs() : encodeArgs(0));
                newDecoder = true;)
}
...
BOOST_AUTO_TEST_CASE(short_input_array) {
  ...
  bool newDecoder = false;
  BOTH_ENCODERS(
      compileAndRun(sourceCode); ABI_CHECK(
          callContractFunctionNoEncoding("f(uint256[])", encodeArgs(0x20, 0)),
          encodeArgs(7));
      ABI_CHECK(
          callContractFunctionNoEncoding("f(uint256[])", encodeArgs(0x20, 1)),
          newDecoder ? encodeArgs() : encodeArgs(7));
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(uint256[])", encodeArgs(0x20, 1) + bytes(31, 0)),
                newDecoder ? encodeArgs() : encodeArgs(7));
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(uint256[])", encodeArgs(0x20, 1) + bytes(32, 0)),
                encodeArgs(7));
      ABI_CHECK(callContractFunctionNoEncoding("f(uint256[])",
                                               encodeArgs(0x20, 2, 5, 6)),
                encodeArgs(7));
      newDecoder = true;)
}
...
BOOST_AUTO_TEST_CASE(short_input_bytes) {
  ...
  NEW_ENCODER(
      ...
      ABI_CHECK(callContractFunctionNoEncoding("e(bytes)", encodeArgs(0x20, 7) +
                ...
                encodeArgs(7));
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(bytes[])", encodeArgs(0x20, 1, 0x20, 7) + bytes(5, 0)),
                encodeArgs());
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(bytes[])", encodeArgs(0x20, 1, 0x20, 7) + bytes(6, 0)),
                encodeArgs());
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(bytes[])", encodeArgs(0x20, 1, 0x20, 7) + bytes(7, 0)),
                encodeArgs(7));
      ABI_CHECK(callContractFunctionNoEncoding(
                    "f(bytes[])", encodeArgs(0x20, 1, 0x20, 7) + bytes(8, 0)),
                encodeArgs(7));)
}
...
BOOST_AUTO_TEST_CASE(struct_simple) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ABI_CHECK(callContractFunction("f((uint256,uint8,uint8,bytes2))",
                                             1, 2, 3, "ab"),
                        encodeArgs(1, 2, 3, 'a' * 0x100 + 'b'));)
}
...
BOOST_AUTO_TEST_CASE(struct_cleanup) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ABI_CHECK(callContractFunction("f((int16,uint8,bytes2))", 0xff010,
                                             0xff0002, "abcd"),
                        encodeArgs(u256("0xffffffffffffffffffffffffffffffffffff"
                                        "fffffffffffffffffffffffff010"),
                                   2, "ab"));)
}
...
BOOST_AUTO_TEST_CASE(struct_short) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ABI_CHECK(callContractFunction("f((int256,uint256,bytes16))",
                                             0xff010, 0xff0002, "abcd"),
                        encodeArgs(0xff010, 0xff0002, "abcd"));
              ABI_CHECK(callContractFunctionNoEncoding(
                            "f((int256,uint256,bytes16))",
...
BOOST_AUTO_TEST_CASE(empty_struct) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ABI_CHECK(callContractFunction("f(uint256,(),uint256)", 7, 8),
                        encodeArgs(7, 8));
...
BOOST_AUTO_TEST_CASE(mediocre_struct) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ...
              ABI_CHECK(callContractFunction(
                            sig, 7, u256(u160(m_contractAddress)), 0, 8),
...
BOOST_AUTO_TEST_CASE(mediocre2_struct) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              ...
              ABI_CHECK(callContractFunction(sig, 7, 0x60, 8, 0x40, 7 * 0x20,
                                             u256(u160(m_contractAddress)),
                                             0x40, 2, 0x11, 0x12, 0x99, 0x40, 4,
                                             0x31, 0x32, 0x34, 0x35),
                        encodeArgs(7, u256(u160(m_contractAddress)), 8));)
}
...
BOOST_AUTO_TEST_CASE(complex_struct) {
  ...
  NEW_ENCODER(compileAndRun(sourceCode, 0, "C");
              string sig = "f(uint256,(address,(uint256,uint8,uint8)[])[2],("
                           "address,(uint256,uint8,uint8)[])[],uint256)";
              bytes args = encodeArgs(7, 0x80, 0x1e0, 8,
                                      // S[2] s1
                                      0x40, 0x100,
                                      // S s1[0]
                                      ...
                                      // s2[1].t
                                      3, // length
                                      0, 0, 0, 0x21, 2, 0x22, 0, 0, 0);
              ABI_CHECK(callContractFunction(sig, args),
                        encodeArgs(7, u256(u160(m_contractAddress)), 8, 2,
                                   0x1234, 3, 2, 0x22));
              // invalid enum value
              args.data()[0x20 * 28] = 3;
