...
public:
  ...
  bytes const &compileAndRunWithOptimizer(std::string const &_sourceCode,
                                          u256 const &_value = 0,
  ...
  void compileBothVersions(std::string const &_sourceCode,
                           u256 const &_value = 0,
                           std::string const &_contractName = "",
                           unsigned const _optimizeRuns = 200) {
    bytes nonOptimizedBytecode = compileAndRunWithOptimizer(
        _sourceCode, _value, _contractName, false, _optimizeRuns);
    m_nonOptimizedContract = m_contractAddress;
    bytes optimizedBytecode = compileAndRunWithOptimizer(
    ...
    BOOST_CHECK_MESSAGE(
        ...
        string("Optimizer did not reduce bytecode size. Non-optimized size: ") +
            std::to_string(nonOptimizedSize) +
            " - optimized size: " + std::to_string(optimizedSize));
  ...
  void compareVersions(std::string _sig, Args const &... _arguments) {
    ...
    BOOST_CHECK_MESSAGE(!optimizedOutput.empty(),
                        "No optimized output for " + _sig);
    BOOST_CHECK_MESSAGE(!nonOptimizedOutput.empty(),
                        "No un-optimized output for " + _sig);
    BOOST_CHECK_MESSAGE(nonOptimizedOutput == optimizedOutput,
  ...
  /// @returns the number of intructions in the given bytecode, not taking the
  /// metadata hash into account.
  size_t numInstructions(bytes const &_bytecode) {
    BOOST_REQUIRE(_bytecode.size() > 5);
    size_t metadataSize = (_bytecode[_bytecode.size() - 2] << 8) +
                          _bytecode[_bytecode.size() - 1];
    BOOST_REQUIRE_MESSAGE(metadataSize == 0x29, "Invalid metadata size");
    BOOST_REQUIRE(_bytecode.size() >= metadataSize + 2);
    bytes realCode =
        bytes(_bytecode.begin(), _bytecode.end() - metadataSize - 2);
    size_t instructions = 0;
    solidity::eachInstruction(
        realCode, [&](Instruction, u256 const &) { instructions++; });
    return instructions;
  }
...
BOOST_AUTO_TEST_CASE(constant_folding_both_sides) {
  // if constants involving the same associative and commutative operator are
  // applied from both sides, the operator should be applied only once, because
  // the expression compiler pushes literals as late as possible
  char const *sourceCode = R"(
		contract test {
...
BOOST_AUTO_TEST_CASE(retain_information_in_branches) {
  ...
  // keccak256(y)" inside both branches.
  char const *sourceCode = R"(
		contract c {
			bytes32 d;
			uint a;
  ...
  bytes optimizedBytecode =
      compileAndRunWithOptimizer(sourceCode, 0, "c", true);
  size_t numSHA3s = 0;
...
BOOST_AUTO_TEST_CASE(store_tags_as_unions) {
  // This calls the same function from two sources and both calls have a certain
  // Keccak-256 on the stack at the same position. Without storing tags as
  // unions, the return from the shared function would not know where to jump
  // and thus all jumpdests are forced to clear their state and we do not know
  // about the sha3 anymore. Note that, for now, this only works if the
  // functions have the same number of return parameters since otherwise, the
  // return jump addresses are at different stack positions which triggers the
  // "unknown jump target" situation.
  char const *sourceCode = R"(
		contract test {
			bytes32 data;
  ...
  bytes optimizedBytecode =
      compileAndRunWithOptimizer(sourceCode, 0, "test", true);
  size_t numSHA3s = 0;
  eachInstruction(optimizedBytecode, [&](Instruction _instr, u256 const &) {
    if (_instr == Instruction::KECCAK256)
      numSHA3s++;
...
BOOST_AUTO_TEST_CASE(incorrect_storage_access_bug) {
  // This bug appeared because a Keccak-256 operation with too low sequence
  // number was used, resulting in memory not being rewritten before the
  // Keccak-256. The fix was to take the max of the min sequence numbers when
  // merging the states.
  char const *sourceCode = R"(
		contract C
		{
...
BOOST_AUTO_TEST_CASE(sequence_number_for_calls) {
  // This is a test for a bug that was present because we did not increment the
  // sequence number for CALLs - CALLs can read and write from memory (and
  // DELEGATECALLs can do the same to storage), so the sequence number should be
  // incremented.
  char const *sourceCode = R"(
		contract test {
...
BOOST_AUTO_TEST_CASE(computing_constants) {
  ...
  bytes optimizedBytecode =
      compileAndRunWithOptimizer(sourceCode, 0, "C", true, 1);
  bytes complicatedConstant = toBigEndian(u256(
      "0x817416927846239487123469187231298734162934871263941234127518276"));
  unsigned occurrences = 0;
  for (auto iter = optimizedBytecode.cbegin(); iter < optimizedBytecode.cend();
       ++occurrences) {
    iter = search(iter, optimizedBytecode.cend(), complicatedConstant.cbegin(),
  ...
  bytes constantWithZeros = toBigEndian(u256(
      "0x77abc0000000000000000000000000000000000000000000000000000000001"));
  BOOST_CHECK(search(optimizedBytecode.cbegin(), optimizedBytecode.cend(),
                     constantWithZeros.cbegin(),
                     constantWithZeros.cend()) == optimizedBytecode.cend());
}

BOOST_AUTO_TEST_CASE(constant_optimization_early_exit) {
  // This tests that the constant optimizer does not try to find the best
  // representation indefinitely but instead stops after some number of
  // iterations.
  char const *sourceCode = R"(
  ...
  double duration =
      std::chrono::duration<double>(std::chrono::steady_clock::now() - start)
          .count();
  BOOST_CHECK_MESSAGE(duration < 20,
...
BOOST_AUTO_TEST_CASE(dead_code_elimination_across_assemblies) {
  // This tests that a runtime-function that is stored in storage in the
  // constructor is not removed as part of dead code elimination.
  char const *sourceCode = R"(
		contract DCE {
