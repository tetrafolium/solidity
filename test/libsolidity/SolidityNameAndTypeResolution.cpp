...
BOOST_AUTO_TEST_CASE(type_checking_return_wrong_number) {
	...
	CHECK_ERROR(text, TypeError,
	            "Different number of arguments in return statement than in "
	            "returns declaration.");
}
...
BOOST_AUTO_TEST_CASE(type_checking_return_wrong_type) {
	...
	CHECK_ERROR(text, TypeError,
	            "Return argument type bool is not implicitly convertible to "
	            "expected type (type of first return variable) uint256.");
}
...
BOOST_AUTO_TEST_CASE(type_conversion_for_comparison_invalid) {
	...
	CHECK_ERROR(text, TypeError,
	            "Operator == not compatible with types int32 and uint64");
}
...
BOOST_AUTO_TEST_CASE(function_no_implementation) {
	...
	ContractDefinition *contract =
		dynamic_cast<ContractDefinition *>(nodes[1].get());
...
BOOST_AUTO_TEST_CASE(abstract_contract) {
	...
	ContractDefinition *derived =
		dynamic_cast<ContractDefinition *>(nodes[2].get());
	BOOST_REQUIRE(base);
...
BOOST_AUTO_TEST_CASE(function_canonical_signature) {
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			    dynamic_cast<ContractDefinition *>(node.get())) {
			auto functions = contract->definedFunctions();
			BOOST_CHECK_EQUAL("foo(uint256,uint64,bool)",
...
BOOST_AUTO_TEST_CASE(function_canonical_signature_type_aliases) {
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			...
			if (functions.empty())
				continue;
			BOOST_CHECK_EQUAL("boo(uint256,bytes32,address)",
			                  functions[0]->externalSignature());
		}
}
...
BOOST_AUTO_TEST_CASE(external_structs) {
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			...
			BOOST_CHECK_EQUAL("f(uint8,uint256,())",
			                  functions[0]->externalSignature());
			BOOST_CHECK_EQUAL("g(address,((bytes32,address,()[])[2][],uint256))",
			                  functions[1]->externalSignature());
			BOOST_CHECK_EQUAL("h(function[])", functions[2]->externalSignature());
			BOOST_CHECK_EQUAL("i(((bytes32,address,()[])[2][],uint256)[])",
...
BOOST_AUTO_TEST_CASE(external_structs_in_libraries) {
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			...
			auto functions = contract->definedFunctions();
			BOOST_REQUIRE(!functions.empty());
			BOOST_CHECK_EQUAL("f(Test.ActionChoices,uint256,Test.Empty)",
			                  functions[0]->externalSignature());
			BOOST_CHECK_EQUAL("g(Test,Test.Nested)",
			                  functions[1]->externalSignature());
			BOOST_CHECK_EQUAL("h(function[])", functions[2]->externalSignature());
...
BOOST_AUTO_TEST_CASE(functions_with_stucts_of_non_external_types_in_interface) {
	...
	CHECK_ERROR(text, TypeError,
	            "Internal or recursive type is not allowed for public or "
	            "external functions.");
}

BOOST_AUTO_TEST_CASE(
	functions_with_stucts_of_non_external_types_in_interface_2) {
	char const *text = R"(
		pragma experimental ABIEncoderV2;
...
BOOST_AUTO_TEST_CASE(
	functions_with_stucts_of_non_external_types_in_interface_nested) {
	char const *text = R"(
		pragma experimental ABIEncoderV2;
...
BOOST_AUTO_TEST_CASE(function_external_call_not_allowed_conversion) {
	...
	CHECK_ERROR(text, TypeError,
	            ...
	            "conversion from address to contract C requested.");
}
...
BOOST_AUTO_TEST_CASE(illegal_override_remove_constness) {
	...
	CHECK_ERROR(text, TypeError,
	            "Overriding function changes state mutability from \"view\" to "
	            "\"nonpayable\".");
}
...
BOOST_AUTO_TEST_CASE(implicit_base_to_derived_conversion) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type contract A is not implicitly convertible to expected type "
	            "contract B.");
}
...
BOOST_AUTO_TEST_CASE(super_excludes_current_contract) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"f\" not found or not visible after argument-dependent "
	            "lookup in contract super B");
}
...
BOOST_AUTO_TEST_CASE(invalid_function_modifier_type) {
	...
	CHECK_ERROR(text, TypeError,
	            "Invalid type for argument in modifier invocation. Invalid "
	            "implicit conversion from bool to uint256 requested.");
}
...
BOOST_AUTO_TEST_CASE(state_variable_accessors) {
	...
	BOOST_REQUIRE((contract = retrieveContractByName(*source, "test")) !=
	              nullptr);
	FunctionTypePointer function =
		retrieveFunctionBySignature(*contract, "foo()");
	...
	function =
		retrieveFunctionBySignature(*contract, "multiple_map(uint256,uint256)");
	BOOST_REQUIRE(function && function->hasDeclaration());
...
BOOST_AUTO_TEST_CASE(missing_state_variable) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"stateVar\" not found or not visible after "
	            "argument-dependent lookup in type(contract Scope)");
}
...
BOOST_AUTO_TEST_CASE(state_variable_member_of_wrong_class1) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"m_aMember1\" not found or not visible after "
	            "argument-dependent lookup in type(contract Parent2)");
}
...
BOOST_AUTO_TEST_CASE(state_variable_member_of_wrong_class2) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"m_aMember2\" not found or not visible after "
	            "argument-dependent lookup in type(contract Child)");
}
...
BOOST_AUTO_TEST_CASE(fallback_function_with_constant_modifier) {
	...
	CHECK_ERROR(text, TypeError,
	            "Fallback function must be payable or non-payable");
}
...
BOOST_AUTO_TEST_CASE(double_event_declaration) {
	...
	CHECK_ERROR(text, DeclarationError,
	            "Event with same name and arguments defined twice.");
}
...
BOOST_AUTO_TEST_CASE(access_to_default_state_variable_visibility) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"a\" not found or not visible after argument-dependent "
	            "lookup in contract c");
}
...
BOOST_AUTO_TEST_CASE(error_count_in_named_args) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Wrong argument count for function call: 1 arguments given but "
	            "expected 2.");
}
...
BOOST_AUTO_TEST_CASE(empty_in_named_args) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Wrong argument count for function call: 0 arguments given but "
	            "expected 2.");
}
...
BOOST_AUTO_TEST_CASE(disallow_declaration_of_void_type) {
	...
	CHECK_ERROR(
		sourceCode, TypeError,
		"Not enough components (0) in value to assign all variables (1).");
}
...
BOOST_AUTO_TEST_CASE(overflow_caused_by_ether_units) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type int_const "
	            "1157920892373161954235709850086879078532699846656405640394580000"
	            "00000000000000 is not implicitly convertible to expected type "
	            "uint256.");
}
...
BOOST_AUTO_TEST_CASE(exp_operator_exponent_too_big) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Operator ** not compatible with types int_const 2 and int_const "
	            "10000000000");
}
...
BOOST_AUTO_TEST_CASE(warn_var_from_zero) {
	...
	CHECK_WARNING(sourceCode, "uint256, which can hold values between 0 and "
...
BOOST_AUTO_TEST_CASE(enum_invalid_member_access) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"RunAroundWavingYourHands\" not found or not visible "
	            "after argument-dependent lookup in type(enum "
	            "test.ActionChoices)");
}
...
BOOST_AUTO_TEST_CASE(enum_implicit_conversion_is_not_okay_256) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type enum test.ActionChoices is not implicitly convertible to "
...
BOOST_AUTO_TEST_CASE(enum_to_enum_conversion_is_not_okay) {
	...
	CHECK_ERROR(text, TypeError,
	            "Explicit type conversion not allowed from \"enum test.Paper\" "
	            "to \"enum test.Ground\".");
}
...
BOOST_AUTO_TEST_CASE(array_copy_with_different_types1) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type bytes storage ref is not implicitly convertible to "
	            "expected type uint256[] storage ref.");
}
...
BOOST_AUTO_TEST_CASE(array_copy_with_different_types2) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type uint32[] storage ref is not implicitly convertible to "
	            "expected type uint8[] storage ref.");
}
...
BOOST_AUTO_TEST_CASE(array_copy_with_different_types_dynamic_static) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type uint256[] storage ref is not implicitly convertible to "
	            "expected type uint256[80] storage ref.");
}
...
BOOST_AUTO_TEST_CASE(storage_variable_initialization_with_incorrect_type_int) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type int_const 1000 is not implicitly convertible to expected "
	            "type uint8.");
}

BOOST_AUTO_TEST_CASE(
	storage_variable_initialization_with_incorrect_type_string) {
	char const *text = R"(
		contract c {
			uint a = "abc";
		}
	...
	CHECK_ERROR(text, TypeError,
	            "Type literal_string \"abc\" is not implicitly convertible to "
	            "expected type uint256.");
}

BOOST_AUTO_TEST_CASE(test_fromElementaryTypeName) {

	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::Int, 0, 0)) ==
	            *make_shared<IntegerType>(256, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 8, 0)) ==
	            *make_shared<IntegerType>(8, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 16, 0)) ==
	            *make_shared<IntegerType>(16, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 24, 0)) ==
	            *make_shared<IntegerType>(24, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 32, 0)) ==
	            *make_shared<IntegerType>(32, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 40, 0)) ==
	            *make_shared<IntegerType>(40, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 48, 0)) ==
	            *make_shared<IntegerType>(48, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 56, 0)) ==
	            *make_shared<IntegerType>(56, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 64, 0)) ==
	            *make_shared<IntegerType>(64, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 72, 0)) ==
	            *make_shared<IntegerType>(72, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 80, 0)) ==
	            *make_shared<IntegerType>(80, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 88, 0)) ==
	            *make_shared<IntegerType>(88, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 96, 0)) ==
	            *make_shared<IntegerType>(96, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 104, 0)) ==
	            *make_shared<IntegerType>(104, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 112, 0)) ==
	            *make_shared<IntegerType>(112, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 120, 0)) ==
	            *make_shared<IntegerType>(120, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 128, 0)) ==
	            *make_shared<IntegerType>(128, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 136, 0)) ==
	            *make_shared<IntegerType>(136, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 144, 0)) ==
	            *make_shared<IntegerType>(144, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 152, 0)) ==
	            *make_shared<IntegerType>(152, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 160, 0)) ==
	            *make_shared<IntegerType>(160, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 168, 0)) ==
	            *make_shared<IntegerType>(168, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 176, 0)) ==
	            *make_shared<IntegerType>(176, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 184, 0)) ==
	            *make_shared<IntegerType>(184, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 192, 0)) ==
	            *make_shared<IntegerType>(192, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 200, 0)) ==
	            *make_shared<IntegerType>(200, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 208, 0)) ==
	            *make_shared<IntegerType>(208, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 216, 0)) ==
	            *make_shared<IntegerType>(216, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 224, 0)) ==
	            *make_shared<IntegerType>(224, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 232, 0)) ==
	            *make_shared<IntegerType>(232, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 240, 0)) ==
	            *make_shared<IntegerType>(240, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 248, 0)) ==
	            *make_shared<IntegerType>(248, IntegerType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::IntM, 256, 0)) ==
	            *make_shared<IntegerType>(256, IntegerType::Modifier::Signed));

	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UInt, 0, 0)) ==
	            *make_shared<IntegerType>(256, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 8, 0)) ==
	            *make_shared<IntegerType>(8, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 16, 0)) ==
	            *make_shared<IntegerType>(16, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 24, 0)) ==
	            *make_shared<IntegerType>(24, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 32, 0)) ==
	            *make_shared<IntegerType>(32, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 40, 0)) ==
	            *make_shared<IntegerType>(40, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 48, 0)) ==
	            *make_shared<IntegerType>(48, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 56, 0)) ==
	            *make_shared<IntegerType>(56, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 64, 0)) ==
	            *make_shared<IntegerType>(64, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 72, 0)) ==
	            *make_shared<IntegerType>(72, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 80, 0)) ==
	            *make_shared<IntegerType>(80, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 88, 0)) ==
	            *make_shared<IntegerType>(88, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 96, 0)) ==
	            *make_shared<IntegerType>(96, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 104, 0)) ==
	            *make_shared<IntegerType>(104, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 112, 0)) ==
	            *make_shared<IntegerType>(112, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 120, 0)) ==
	            *make_shared<IntegerType>(120, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 128, 0)) ==
	            *make_shared<IntegerType>(128, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 136, 0)) ==
	            *make_shared<IntegerType>(136, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 144, 0)) ==
	            *make_shared<IntegerType>(144, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 152, 0)) ==
	            *make_shared<IntegerType>(152, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 160, 0)) ==
	            *make_shared<IntegerType>(160, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 168, 0)) ==
	            *make_shared<IntegerType>(168, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 176, 0)) ==
	            *make_shared<IntegerType>(176, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 184, 0)) ==
	            *make_shared<IntegerType>(184, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 192, 0)) ==
	            *make_shared<IntegerType>(192, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 200, 0)) ==
	            *make_shared<IntegerType>(200, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 208, 0)) ==
	            *make_shared<IntegerType>(208, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 216, 0)) ==
	            *make_shared<IntegerType>(216, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 224, 0)) ==
	            *make_shared<IntegerType>(224, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 232, 0)) ==
	            *make_shared<IntegerType>(232, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 240, 0)) ==
	            *make_shared<IntegerType>(240, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 248, 0)) ==
	            *make_shared<IntegerType>(248, IntegerType::Modifier::Unsigned));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UIntM, 256, 0)) ==
	            *make_shared<IntegerType>(256, IntegerType::Modifier::Unsigned));

	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::Byte, 0, 0)) == *make_shared<FixedBytesType>(1));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 1, 0)) == *make_shared<FixedBytesType>(1));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 2, 0)) == *make_shared<FixedBytesType>(2));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 3, 0)) == *make_shared<FixedBytesType>(3));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 4, 0)) == *make_shared<FixedBytesType>(4));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 5, 0)) == *make_shared<FixedBytesType>(5));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 6, 0)) == *make_shared<FixedBytesType>(6));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 7, 0)) == *make_shared<FixedBytesType>(7));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 8, 0)) == *make_shared<FixedBytesType>(8));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 9, 0)) == *make_shared<FixedBytesType>(9));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 10, 0)) == *make_shared<FixedBytesType>(10));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 11, 0)) == *make_shared<FixedBytesType>(11));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 12, 0)) == *make_shared<FixedBytesType>(12));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 13, 0)) == *make_shared<FixedBytesType>(13));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 14, 0)) == *make_shared<FixedBytesType>(14));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 15, 0)) == *make_shared<FixedBytesType>(15));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 16, 0)) == *make_shared<FixedBytesType>(16));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 17, 0)) == *make_shared<FixedBytesType>(17));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 18, 0)) == *make_shared<FixedBytesType>(18));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 19, 0)) == *make_shared<FixedBytesType>(19));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 20, 0)) == *make_shared<FixedBytesType>(20));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 21, 0)) == *make_shared<FixedBytesType>(21));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 22, 0)) == *make_shared<FixedBytesType>(22));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 23, 0)) == *make_shared<FixedBytesType>(23));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 24, 0)) == *make_shared<FixedBytesType>(24));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 25, 0)) == *make_shared<FixedBytesType>(25));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 26, 0)) == *make_shared<FixedBytesType>(26));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 27, 0)) == *make_shared<FixedBytesType>(27));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 28, 0)) == *make_shared<FixedBytesType>(28));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 29, 0)) == *make_shared<FixedBytesType>(29));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 30, 0)) == *make_shared<FixedBytesType>(30));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 31, 0)) == *make_shared<FixedBytesType>(31));
	BOOST_CHECK(*Type::fromElementaryTypeName(ElementaryTypeNameToken(
							  Token::BytesM, 32, 0)) == *make_shared<FixedBytesType>(32));

	BOOST_CHECK(
		*Type::fromElementaryTypeName(
			ElementaryTypeNameToken(Token::Fixed, 0, 0)) ==
		*make_shared<FixedPointType>(128, 19, FixedPointType::Modifier::Signed));
	BOOST_CHECK(*Type::fromElementaryTypeName(
			    ElementaryTypeNameToken(Token::UFixed, 0, 0)) ==
	            *make_shared<FixedPointType>(128, 19,
...
BOOST_AUTO_TEST_CASE(ambiguous_overloaded_function) {
	// literal 1 can be both converted to uint and uint8, so the call is
	// ambiguous.
	char const *sourceCode = R"(
		contract test {
...
BOOST_AUTO_TEST_CASE(uninitialized_var) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Assignment necessary for type detection.");
}
...
BOOST_AUTO_TEST_CASE(string_length) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Member \"length\" not found or not visible after "
	            "argument-dependent lookup in string storage ref");
}
...
BOOST_AUTO_TEST_CASE(negative_integers_to_signed_out_of_bound) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type int_const -129 is not implicitly convertible to expected "
	            "type int8.");
}
...
BOOST_AUTO_TEST_CASE(positive_integers_to_signed_out_of_bound) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type int_const 128 is not implicitly convertible to expected "
	            "type int8.");
}
...
BOOST_AUTO_TEST_CASE(negative_integers_to_unsigned) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type int_const -1 is not implicitly convertible to expected "
	            "type uint8.");
}
...
BOOST_AUTO_TEST_CASE(positive_integers_to_unsigned_out_of_bound) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type int_const 700 is not implicitly convertible to expected "
	            "type uint8.");
}
...
BOOST_AUTO_TEST_CASE(exp_signed_variable) {
	...
	CHECK_ERROR(sourceCode2, TypeError,
	            "Operator ** not compatible with types int256 and uint256");
	char const *sourceCode3 = R"(
		contract test { function() public { int x = -3; int y = -4; x ** y; } }
	)";
	CHECK_ERROR(sourceCode3, TypeError,
	            "Operator ** not compatible with types int256 and int256");
}
...
BOOST_AUTO_TEST_CASE(reference_compare_operators) {
	...
	CHECK_ERROR(sourceCode1, TypeError,
	            "Operator == not compatible with types bytes storage ref and "
	            "bytes storage ref");
	char const *sourceCode2 = R"(
		contract test { struct s {uint a;} s x; s y; function() public { x == y; } }
	)";
	CHECK_ERROR(sourceCode2, TypeError,
	            "Operator == not compatible with types struct test.s storage ref "
	            "and struct test.s storage ref");
}
...
BOOST_AUTO_TEST_CASE(overwrite_memory_location_external) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Location has to be calldata for external functions (remove the "
	            "\"memory\" or \"storage\" keyword).");
}
...
BOOST_AUTO_TEST_CASE(no_mappings_in_memory_array) {
	...
	CHECK_ERROR(
		sourceCode, TypeError,
		"Type mapping(uint256 => uint256)[] memory is only valid in storage.");
}
...
BOOST_AUTO_TEST_CASE(assignment_mem_to_local_storage_variable) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type uint256[] memory is not implicitly convertible to expected "
...
BOOST_AUTO_TEST_CASE(storage_assign_to_different_local_variable) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type uint8[] storage pointer is not implicitly convertible to "
	            "expected type uint256[] storage pointer.");
}
...
BOOST_AUTO_TEST_CASE(uninitialized_mapping_variable) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Uninitialized mapping. Mappings cannot be created dynamically, "
	            "you have to assign them from a state variable");
}
...
BOOST_AUTO_TEST_CASE(no_delete_on_storage_pointers) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Unary operator delete cannot be applied to type uint256[] "
	            "storage pointer");
}
...
BOOST_AUTO_TEST_CASE(function_argument_mem_to_storage) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Invalid type for argument in function call. Invalid implicit "
	            "conversion from uint256[] memory to uint256[] storage pointer "
	            "requested.");
}
...
BOOST_AUTO_TEST_CASE(mem_array_assignment_changes_base_type) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Type uint8[] memory is not implicitly convertible to expected "
	            "type uint256[] memory.");
}
...
BOOST_AUTO_TEST_CASE(dynamic_return_types_not_possible) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Explicit type conversion not allowed from \"inaccessible "
...
BOOST_AUTO_TEST_CASE(memory_structs_with_mappings) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"b\" is not available in struct Test.S memory outside "
	            "of storage.");
}
...
BOOST_AUTO_TEST_CASE(creating_contract_within_the_contract) {
	...
	CHECK_ERROR(sourceCode, TypeError,
	            "Circular reference for contract creation (cannot create "
	            "instance of derived or same contract).");
}
...
BOOST_AUTO_TEST_CASE(multi_variable_declaration_wildcards_fail_1) {
	...
	CHECK_ERROR(
		text, TypeError,
		"Not enough components (1) in value to assign all variables (2).");
}
...
BOOST_AUTO_TEST_CASE(multi_variable_declaration_wildcards_fail_5) {
	...
	CHECK_ERROR(text, TypeError,
	            "Wildcard both at beginning and end of variable declaration list "
	            "is only allowed if the number of components is equal.");
}
...
BOOST_AUTO_TEST_CASE(multi_variable_declaration_wildcards_fail_6) {
	...
	CHECK_ERROR(text, TypeError,
	            "Not enough components (2) in value to assign all variables (3)");
}
...
BOOST_AUTO_TEST_CASE(using_for_mismatch) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"double\" not found or not visible after "
...
BOOST_AUTO_TEST_CASE(using_for_not_used) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"double\" not found or not visible after "
	            "argument-dependent lookup in uint16");
}
...
BOOST_AUTO_TEST_CASE(invalid_different_types_for_conditional_expression) {
	...
	CHECK_ERROR(text, TypeError,
	            "True expression's type bool doesn't match false expression's "
	            "type uint8.");
}
...
BOOST_AUTO_TEST_CASE(conditional_expression_with_different_struct) {
	...
	CHECK_ERROR(text, TypeError,
	            "True expression's type struct C.s1 memory doesn't match false "
	            "expression's type struct C.s2 memory.");
}
...
BOOST_AUTO_TEST_CASE(conditional_expression_with_different_function_type) {
	...
	CHECK_ERROR(text, TypeError,
	            "True expression's type function (bool) doesn't match false "
	            "expression's type function ().");
}
...
BOOST_AUTO_TEST_CASE(conditional_expression_with_different_enum) {
	...
	CHECK_ERROR(text, TypeError,
	            "True expression's type enum C.small doesn't match false "
	            "expression's type enum C.big.");
}
...
BOOST_AUTO_TEST_CASE(conditional_expression_with_different_mapping) {
	...
	CHECK_ERROR(text, TypeError,
	            "True expression's type mapping(uint8 => uint8) doesn't match "
	            "false expression's type mapping(uint32 => uint8).");
}
...
BOOST_AUTO_TEST_CASE(constructor_call_invalid_arg_count) {
	...
	CHECK_ERROR(text, TypeError,
	            "Wrong argument count for modifier invocation: 1 arguments given "
	            "but expected 0.");
}
...
BOOST_AUTO_TEST_CASE(library_functions_do_not_have_value) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"value\" not found or not visible after "
	            "argument-dependent lookup in function ()");
}
...
BOOST_AUTO_TEST_CASE(library_instances_cannot_be_used) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"l\" not found or not visible after argument-dependent "
	            "lookup in library L");
}
...
BOOST_AUTO_TEST_CASE(invalid_int_implicit_conversion_from_fixed) {
	...
	CHECK_ERROR(
		text, TypeError,
		"Type fixed128x19 is not implicitly convertible to expected type int256");
}
...
BOOST_AUTO_TEST_CASE(fixed_type_invalid_implicit_conversion_size) {
	...
	CHECK_ERROR(text, TypeError,
	            "Type ufixed128x19 is not implicitly convertible to expected "
	            "type ufixed248x8");
}
...
BOOST_AUTO_TEST_CASE(fixed_type_invalid_implicit_conversion_lost_data) {
	...
	CHECK_ERROR(text, TypeError,
	            "is not implicitly convertible to expected type ufixed256x1");
}
...
BOOST_AUTO_TEST_CASE(rational_to_bytes_implicit_conversion) {
	...
	CHECK_ERROR(text, TypeError,
	            "is not implicitly convertible to expected type bytes32");
}
...
BOOST_AUTO_TEST_CASE(fixed_to_bytes_implicit_conversion) {
	...
	CHECK_ERROR(
		text, TypeError,
		"fixed128x19 is not implicitly convertible to expected type bytes32");
}
...
BOOST_AUTO_TEST_CASE(rational_index_access) {
	...
	CHECK_ERROR(text, TypeError,
	            "rational_const 1/2 is not implicitly convertible to expected "
	            "type uint256");
}
...
BOOST_AUTO_TEST_CASE(one_divided_by_three_integer_conversion) {
	...
	CHECK_ERROR(text, TypeError,
	            "is not implicitly convertible to expected type uint256. Try "
	            "converting to type ufixed256x77");
}
...
BOOST_AUTO_TEST_CASE(unused_return_value_send) {
	...
	CHECK_WARNING(text, "Failure condition of 'send' ignored. Consider using "
	              "'transfer' instead.");
}
...
BOOST_AUTO_TEST_CASE(warn_about_callcode) {
	...
	CHECK_WARNING(
		text, "\"callcode\" has been deprecated in favour of \"delegatecall\"");
	text = R"(
		pragma experimental "v0.5.0";
		contract test {
			function f() pure public {
...
BOOST_AUTO_TEST_CASE(illegal_override_payable) {
	...
	CHECK_ERROR(text, TypeError,
	            "Overriding function changes state mutability from \"payable\" "
	            "to \"nonpayable\".");
}
...
BOOST_AUTO_TEST_CASE(calling_nonpayable) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"value\" not found or not visible after "
	            "argument-dependent lookup in function () external - did you "
...
BOOST_AUTO_TEST_CASE(non_payable_constructor) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"value\" not found or not visible after "
	            "argument-dependent lookup in function () returns (contract C) - "
	            "did you forget the \"payable\" modifier?");
}
...
BOOST_AUTO_TEST_CASE(unsatisfied_version) {
	...
	BOOST_CHECK(
		searchErrorMessage(*sourceAndError.second,
...
BOOST_AUTO_TEST_CASE(using_directive_for_missing_selftype) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"b\" not found or not visible after argument-dependent "
	            "lookup in bytes memory");
}
...
BOOST_AUTO_TEST_CASE(call_value_on_non_payable_function_type) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"value\" not found or not visible after "
	            "argument-dependent lookup in function (uint256) external "
	            "returns (uint256) - did you forget the \"payable\" modifier?");
}
...
BOOST_AUTO_TEST_CASE(internal_function_returned_from_public_function) {
	// It should not be possible to return internal functions from external
	// functions.
	char const *text = R"(
		contract C {
...
BOOST_AUTO_TEST_CASE(
	internal_function_as_external_parameter_in_library_external) {
	char const *text = R"(
		library L {
			function f(function(uint) internal returns (uint) x) public {
			}
...
BOOST_AUTO_TEST_CASE(shift_constant_left_negative_rvalue) {
	...
	CHECK_ERROR(
		text, TypeError,
		"Operator << not compatible with types int_const 66 and int_const -8");
}
...
BOOST_AUTO_TEST_CASE(shift_constant_left_excessive_rvalue) {
	...
	CHECK_ERROR(text, TypeError,
	            "Operator << not compatible with types int_const 66 and "
	            "int_const 4294967296");
}
...
BOOST_AUTO_TEST_CASE(inline_assembly_unbalanced_negative_stack) {
	...
	CHECK_ERROR(text, DeclarationError,
	            "Unbalanced stack at the end of a block: 1 missing item(s).");
}
...
BOOST_AUTO_TEST_CASE(inline_assembly_unbalanced_two_stack_load) {
	...
	CHECK_ERROR(
		text, TypeError,
		"Only local variables are supported. To access storage variables,");
}
...
BOOST_AUTO_TEST_CASE(inline_assembly_local_variable_access_out_of_functions) {
	...
	CHECK_ERROR(text, DeclarationError,
	            "Cannot access local Solidity variables from inside an inline "
	            "assembly function.");
}

BOOST_AUTO_TEST_CASE(
	inline_assembly_local_variable_access_out_of_functions_storage_ptr) {
	char const *text = R"(
		contract test {
			uint[] r;
			function f() public {
...
BOOST_AUTO_TEST_CASE(inline_assembly_calldata_variables) {
	...
	CHECK_ERROR(text, TypeError,
	            "Call data elements cannot be accessed directly.");
}
...
BOOST_AUTO_TEST_CASE(warns_msg_value_in_non_payable_public_function) {
	...
	CHECK_WARNING(text, "\"msg.value\" used in non-payable function. Do you want "
	              "to add the \"payable\" modifier to this function?");
}
...
BOOST_AUTO_TEST_CASE(
	does_not_warn_msg_value_in_modifier_following_non_payable_public_function) {
	char const *text = R"(
		contract c {
			function f() pure public { }
...
BOOST_AUTO_TEST_CASE(inconstructible_internal_constructor) {
	...
	CHECK_ERROR(text, TypeError,
	            "Contract with internal constructor cannot be created directly.");
}

BOOST_AUTO_TEST_CASE(inconstructible_internal_constructor_inverted) {
	// Previously, the type information for A was not yet available at the point
	// of "new A".
	char const *text = R"(
		contract B {
			A a;
...
BOOST_AUTO_TEST_CASE(invalid_address_length_long) {
	...
	CHECK_WARNING_ALLOW_MULTI(
		text, "This looks like an address but has an invalid checksum.");
}
...
BOOST_AUTO_TEST_CASE(bare_others) {
	CHECK_WARNING("contract C { function f() pure public { selfdestruct; } }",
	              "Statement has no effect.");
	CHECK_WARNING("contract C { function f() pure public { assert; } }",
	              "Statement has no effect.");
	CHECK_WARNING("contract C { function f() pure public { require; } }",
	              "Statement has no effect.");
	CHECK_WARNING("contract C { function f() pure public { suicide; } }",
	              "Statement has no effect.");
}
...
BOOST_AUTO_TEST_CASE(warn_unused_function_parameter) {
	...
	CHECK_WARNING(text, "Unused function parameter. Remove or comment out the "
...
BOOST_AUTO_TEST_CASE(returndatacopy_as_variable) {
	...
	CHECK_WARNING_ALLOW_MULTI(text, "Variable is shadowed in inline assembly by "
	                          "an instruction of the same name");
}
...
BOOST_AUTO_TEST_CASE(warn_unspecified_storage) {
	...
	CHECK_WARNING(text, "Variable is declared as a storage pointer. Use an "
	              "explicit \"storage\" keyword to silence this warning");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
			struct S { uint a; }
	...
	CHECK_ERROR(text, TypeError,
	            "Storage location must be specified as either \"memory\" or "
	            "\"storage\".");
}
...
BOOST_AUTO_TEST_CASE(implicit_conversion_disallowed) {
	...
	CHECK_ERROR(text, TypeError,
	            "Return argument type uint32 is not implicitly convertible to "
	            "expected type (type of first return variable) bytes4.");
}
...
BOOST_AUTO_TEST_CASE(explicit_literal_to_storage_string) {
	...
	CHECK_ERROR(text, TypeError,
	            "Explicit type conversion not allowed from \"literal_string "
	            "\"abc\"\" to \"string storage pointer\"");
}
...
BOOST_AUTO_TEST_CASE(builtin_reject_gas) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"gas\" not found or not visible after "
...
BOOST_AUTO_TEST_CASE(large_storage_array_simple) {
	...
	CHECK_WARNING(
		text, "covers a large part of storage and thus makes collisions likely");
}
...
BOOST_AUTO_TEST_CASE(library_function_without_implementation) {
	...
	CHECK_ERROR(text, TypeError,
	            "Internal library function must be implemented if declared.");
	text = R"(
		library L {
			function f() private;
		}
...
BOOST_AUTO_TEST_CASE(experimental_pragma) {
	...
	CHECK_WARNING(text, "Experimental features are turned on. Do not use "
...
BOOST_AUTO_TEST_CASE(reject_interface_constructors) {
	...
	CHECK_ERROR(text, TypeError,
	            "Wrong argument count for constructor call: 1 arguments given "
	            "but expected 0.");
}
...
BOOST_AUTO_TEST_CASE(tight_packing_literals) {
	...
	//	CHECK_WARNING(text, "The type of \"int_const 1\" was inferred as
	//uint8.");
	text = R"(
		contract C {
			function f() pure public returns (bytes32) {
...
BOOST_AUTO_TEST_CASE(warn_about_suicide) {
	...
	CHECK_WARNING(
		text, "\"suicide\" has been deprecated in favour of \"selfdestruct\"");
}
...
BOOST_AUTO_TEST_CASE(array_length_invalid_expression) {
	...
	CHECK_ERROR(
		text, TypeError,
		"Operator / not compatible with types int_const 3 and int_const 0");
}
...
BOOST_AUTO_TEST_CASE(no_address_members_on_contract) {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"balance\" not found or not visible after "
	            "argument-dependent lookup in contract");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
			function f() {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"transfer\" not found or not visible after "
	            "argument-dependent lookup in contract");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"send\" not found or not visible after "
	            "argument-dependent lookup in contract");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"call\" not found or not visible after "
	            "argument-dependent lookup in contract");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"callcode\" not found or not visible after "
	            "argument-dependent lookup in contract");
	text = R"(
		pragma experimental "v0.5.0";
		contract C {
	...
	CHECK_ERROR(text, TypeError,
	            "Member \"delegatecall\" not found or not visible after "
	            "argument-dependent lookup in contract");
}
