...
bool containsError(Json::Value const &_compilerResult, string const &_type,
	...
	for (auto const &error : _compilerResult["errors"]) {
		...
		if ((error["type"].asString() == _type) &&
		    (error["message"].asString() == _message))
			return true;
	}
...
Json::Value getContractResult(Json::Value const &_compilerResult,
                              string const &_file, string const &_name) {
...
BOOST_AUTO_TEST_CASE(assume_object_input) {
	...
	BOOST_CHECK(
		containsError(result, "JSONError", "Input is not a JSON object."));
...
BOOST_AUTO_TEST_CASE(no_sources) {
	...
	Json::Value result = compile(input);
	BOOST_CHECK(
		containsError(result, "JSONError", "No input sources specified."));
}
...
BOOST_AUTO_TEST_CASE(basic_compilation) {
	...
	BOOST_CHECK_EQUAL(dev::jsonCompactPrint(contract["devdoc"]),
	                  "{\"methods\":{}}");
	BOOST_CHECK(contract["userdoc"].isObject());
	BOOST_CHECK_EQUAL(dev::jsonCompactPrint(contract["userdoc"]),
	                  "{\"methods\":{}}");
	BOOST_CHECK(contract["evm"].isObject());
	/// @TODO check evm.methodIdentifiers, legacyAssembly, bytecode,
	/// deployedBytecode
	BOOST_CHECK(contract["evm"]["bytecode"].isObject());
	BOOST_CHECK(contract["evm"]["bytecode"]["object"].isString());
	BOOST_CHECK_EQUAL(dev::test::bytecodeSansMetadata(
				  contract["evm"]["bytecode"]["object"].asString()),
	                  "60606040523415600e57600080fd5b603580601b6000396000f3006060"
	                  "604052600080fd00");
	...
	BOOST_CHECK_EQUAL(dev::jsonCompactPrint(contract["evm"]["gasEstimates"]),
	                  "{\"creation\":{\"codeDepositCost\":\"10600\","
	                  "\"executionCost\":\"61\",\"totalCost\":\"10661\"}}");
	...
	BOOST_CHECK_EQUAL(
		...
		"{\"attributes\":{\"absolutePath\":\"fileA\",\"exportedSymbols\":{\"A\":["
		"1]}},\"children\":"
		"[{\"attributes\":{\"baseContracts\":[null],\"contractDependencies\":["
		"null],\"contractKind\":\"contract\","
		"\"documentation\":null,\"fullyImplemented\":true,"
		"\"linearizedBaseContracts\":[1],\"name\":\"A\",\"nodes\":[null],"
		"\"scope\":2},"
		"\"id\":1,\"name\":\"ContractDefinition\",\"src\":\"0:14:0\"}],\"id\":2,"
		"\"name\":\"SourceUnit\",\"src\":\"0:14:0\"}");
}
