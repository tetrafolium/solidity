...
public:
FirstExpressionExtractor(ASTNode &_node) : m_expression(nullptr) {
	_node.accept(*this);
}
...
private:
virtual bool visit(Assignment &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(UnaryOperation &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(BinaryOperation &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(FunctionCall &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(MemberAccess &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(IndexAccess &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(Identifier &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(ElementaryTypeNameExpression &_expression) override {
	return checkExpression(_expression);
}
virtual bool visit(Literal &_expression) override {
	return checkExpression(_expression);
}
...
Declaration const &resolveDeclaration(SourceUnit const &_sourceUnit,
...
bytes compileFirstExpression(const string &_sourceCode,
                             vector<vector<string> > _functions = {},
                             vector<vector<string> > _localVariables = {},
                             vector<shared_ptr<MagicVariableDeclaration const> >
                             _globalDeclarations = {}) {
	ASTPointer<SourceUnit> sourceUnit;
	try {
		ErrorList errors;
		ErrorReporter errorReporter(errors);
		sourceUnit = Parser(errorReporter)
		             .parse(make_shared<Scanner>(CharStream(_sourceCode)));
		if (!sourceUnit)
			return bytes();
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			...
			BOOST_REQUIRE_MESSAGE(resolver.resolveNamesAndTypes(*contract),
			                      "Resolving names failed");
	...
	for (ASTPointer<ASTNode> const &node : sourceUnit->nodes())
		if (ContractDefinition *contract =
			...
			unsigned parametersSize =
				_localVariables.size(); // assume they are all one slot on the stack
			context.adjustStackOffset(parametersSize);
			for (vector<string> const &variable : _localVariables)
				context.addVariable(
					dynamic_cast<VariableDeclaration const &>(
						resolveDeclaration(*sourceUnit, variable, resolver)),
					parametersSize--);

			ExpressionCompiler(context).compile(*extractor.expression());

			for (vector<string> const &function : _functions)
				context << context.functionEntryLabel(
...
BOOST_AUTO_TEST_CASE(literal_true) {
	...
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(int_with_finney_ether_subdenomination) {
	...
	bytes expectation(
		{byte(Instruction::PUSH7), 0x3, 0x8d, 0x7e, 0xa4, 0xc6, 0x80, 0x00});
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(int_with_ether_ether_subdenomination) {
	...
	bytes expectation({byte(Instruction::PUSH8), 0xd, 0xe0, 0xb6, 0xb3, 0xa7,
	                   0x64, 0x00, 0x00});
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(comparison) {
	...
	bytes expectation({byte(Instruction::PUSH1), 0x1, byte(Instruction::ISZERO),
	                   byte(Instruction::ISZERO), byte(Instruction::PUSH2), 0x11,
	                   0xaa, byte(Instruction::PUSH2), 0x10, 0xaa,
	                   byte(Instruction::LT), byte(Instruction::ISZERO),
	                   byte(Instruction::ISZERO), byte(Instruction::EQ),
...
BOOST_AUTO_TEST_CASE(arithmetics) {
	...
	bytes code = compileFirstExpression(sourceCode, {},
	                                    {{"test", "f", "y"}, {"test", "f", "x"}});
	bytes expectation({byte(Instruction::PUSH1),
	                   0x1,
	                   byte(Instruction::PUSH1),
	                   0x2,
	                   byte(Instruction::PUSH1),
...
BOOST_AUTO_TEST_CASE(unary_operators) {
	...
	bytes expectation({byte(Instruction::PUSH1), 0x2, byte(Instruction::DUP3),
	                   byte(Instruction::PUSH1), 0x0, byte(Instruction::SUB),
	                   byte(Instruction::NOT), byte(Instruction::EQ),
	                   byte(Instruction::ISZERO)});
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(unary_inc_dec) {
	...
	// Stack: a, x
	bytes expectation({byte(Instruction::DUP2), byte(Instruction::DUP1),
	                   byte(Instruction::PUSH1), 0x1, byte(Instruction::ADD),
	                   // Stack here: a x a (a+1)
	                   byte(Instruction::SWAP3),
	                   byte(Instruction::POP), // first ++
	                   // Stack here: (a+1) x a
	                   byte(Instruction::DUP3), byte(Instruction::PUSH1), 0x1,
	                   byte(Instruction::ADD),
	                   ...
	                   byte(Instruction::DUP3), byte(Instruction::DUP1),
	                   byte(Instruction::PUSH1), 0x1, byte(Instruction::SWAP1),
	                   byte(Instruction::SUB),
	                   ...
	                   byte(Instruction::DUP3), byte(Instruction::PUSH1), 0x1,
	                   byte(Instruction::SWAP1), byte(Instruction::SUB),
...
BOOST_AUTO_TEST_CASE(assignment) {
	...
	// Stack: a, b
	bytes expectation({byte(Instruction::PUSH1), 0x2, byte(Instruction::DUP2),
	                   byte(Instruction::DUP4), byte(Instruction::ADD),
...
BOOST_AUTO_TEST_CASE(negative_literals_8bits) {
	...
	bytes expectation(bytes({byte(Instruction::PUSH32)}) + bytes(31, 0xff) +
	                  bytes(1, 0x80));
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(negative_literals_16bits) {
	...
	bytes expectation(bytes({byte(Instruction::PUSH32)}) + bytes(30, 0xff) +
	                  bytes {0xf5, 0x43});
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
...
BOOST_AUTO_TEST_CASE(intermediately_overflowing_literals) {
	// first literal itself is too large for 256 bits but it fits after all
	// constant operations have been applied
	char const *sourceCode = R"(
		contract test {
...
BOOST_AUTO_TEST_CASE(blockhash) {
	...
	bytes expectation(
		{byte(Instruction::PUSH1), 0x03, byte(Instruction::BLOCKHASH)});
	BOOST_CHECK_EQUAL_COLLECTIONS(code.begin(), code.end(), expectation.begin(),
	                              expectation.end());
}
