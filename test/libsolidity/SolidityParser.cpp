...
Error getError(std::string const &_source) {
  ...
  Error const *error =
      Error::containsErrorOfType(errors, Error::Type::ParserError);
  BOOST_REQUIRE(error);
  return *error;
}

void checkFunctionNatspec(FunctionDefinition const *_function,
                          std::string const &_expectedDoc) {
  auto doc = _function->documentation();
  BOOST_CHECK_MESSAGE(doc != nullptr,
                      "Function does not have Natspec Doc as expected");
...
BOOST_AUTO_TEST_CASE(function_natspec_documentation) {
  ...
  BOOST_REQUIRE_MESSAGE(function = functions.at(0),
...
BOOST_AUTO_TEST_CASE(multiple_functions_natspec_documentation) {
  ...
  BOOST_REQUIRE_MESSAGE(function = functions.at(1),
                        "Failed to retrieve function");
  checkFunctionNatspec(function, "This is test function 2");

  BOOST_REQUIRE_MESSAGE(function = functions.at(2),
                        "Failed to retrieve function");
  BOOST_CHECK_MESSAGE(
      function->documentation() == nullptr,
      "Should not have gotten natspec comment for functionName3()");

  BOOST_REQUIRE_MESSAGE(function = functions.at(3),
                        "Failed to retrieve function");
...
BOOST_AUTO_TEST_CASE(from_is_not_keyword) {
  // "from" is not a keyword although it is used as a keyword in import
  // directives.
  char const *text = R"(
		contract from {
		}
