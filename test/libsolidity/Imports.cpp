...
BOOST_AUTO_TEST_CASE(relative_import) {
	CompilerStack c;
	c.addSource("a",
	            "import \"./dir/b\"; contract A is B {} pragma solidity >=0.0;");
	c.addSource("dir/b", "contract B {} pragma solidity >=0.0;");
	c.addSource("dir/c",
	            "import \"../a\"; contract C is A {} pragma solidity >=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(relative_import_multiplex) {
	...
	c.addSource(
		"dir/a/b/c",
		"import \"../../.././a\"; contract B is A {} pragma solidity >=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(simple_alias) {
	CompilerStack c;
	c.addSource("a", "contract A {} pragma solidity >=0.0;");
	c.addSource("dir/a/b/c", "import \"../../.././a\" as x; contract B is x.A { "
	            "function() { x.A r = x.A(20); } } pragma solidity "
	            ">=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(complex_import) {
	CompilerStack c;
	c.addSource("a", "contract A {} contract B {} contract C { struct S { uint "
	            "a; } } pragma solidity >=0.0;");
	c.addSource("b", "import \"a\" as x; import {B as b, C as c, C} from \"a\"; "
	            "contract D is b { function f(c.S var1, x.C.S var2, C.S "
	            "var3) internal {} } pragma solidity >=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(name_clash_in_import) {
	...
	BOOST_CHECK(!c.compile());
	c.addSource("b",
	            "import {A} from \"a\"; contract A {} pragma solidity >=0.0;");
	BOOST_CHECK(!c.compile());
	c.addSource("b",
	            "import {A} from \"a\"; contract B {} pragma solidity >=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(remappings) {
	...
	c.addSource("a",
	            "import \"s/s.sol\"; contract A is S {} pragma solidity >=0.0;");
	c.addSource(
...
BOOST_AUTO_TEST_CASE(context_dependent_remappings) {
	...
	c.addSource(
		"a/a.sol",
		"import \"s/s.sol\"; contract A is SSix {} pragma solidity >=0.0;");
	c.addSource(
		"b/b.sol",
		"import \"s/s.sol\"; contract B is SSeven {} pragma solidity >=0.0;");
...
BOOST_AUTO_TEST_CASE(filename_with_period) {
	CompilerStack c;
	c.addSource("a/a.sol",
	            "import \".b.sol\"; contract A is B {} pragma solidity >=0.0;");
	c.addSource("a/.b.sol", "contract B {} pragma solidity >=0.0;");
	BOOST_CHECK(!c.compile());
}

BOOST_AUTO_TEST_CASE(
	context_dependent_remappings_ensure_default_and_module_preserved) {
	CompilerStack c;
	c.setRemappings(vector<string>{"foo=vendor/foo_2.0.0",
	                               "vendor/bar:foo=vendor/foo_1.0.0",
	                               "bar=vendor/bar"});
	c.addSource("main.sol", "import \"foo/foo.sol\"; import {Bar} from "
	            "\"bar/bar.sol\"; contract Main is Foo2, Bar {} "
	            "pragma solidity >=0.0;");
	c.addSource("vendor/bar/bar.sol", "import \"foo/foo.sol\"; contract Bar "
	            "{Foo1 foo;} pragma solidity >=0.0;");
	c.addSource("vendor/foo_1.0.0/foo.sol",
	            "contract Foo1 {} pragma solidity >=0.0;");
	c.addSource("vendor/foo_2.0.0/foo.sol",
	            "contract Foo2 {} pragma solidity >=0.0;");
	BOOST_CHECK(c.compile());
}
...
BOOST_AUTO_TEST_CASE(context_dependent_remappings_order_independent) {
	...
	c.addSource(
		"a/main.sol",
		"import \"x/y/z/z.sol\"; contract Main is D {} pragma solidity >=0.0;");
	c.addSource(
		"a/b/main.sol",
		"import \"x/y/z/z.sol\"; contract Main is E {} pragma solidity >=0.0;");
...
BOOST_AUTO_TEST_CASE(shadowing_builtins_with_imports) {
	...
	for (auto const &e : c.errors()) {
		...
		if (msg->find("pre-release") != string::npos)
			continue;
		BOOST_CHECK(msg->find("shadows a builtin symbol") != string::npos);
		errorCount++;
	}
...
BOOST_AUTO_TEST_CASE(shadowing_builtins_with_multiple_imports) {
	CompilerStack c;
	c.addSource("B.sol",
	            "contract msg {} contract block{} pragma solidity >=0.0;");
	c.addSource("b", R"(
		pragma solidity >=0.0;
	...
	for (auto const &e : c.errors()) {
		string const *msg = e->comment();
		BOOST_REQUIRE(msg);
		BOOST_CHECK(msg->find("pre-release") != string::npos ||
