...
bool parse(string const &_source, ErrorReporter &errorReporter) {
  try {
    ...
    if (parserResult) {
      ...
      return (assembly::AsmAnalyzer(analysisInfo, errorReporter, true))
...
boost::optional<Error> parseAndReturnFirstError(string const &_source,
...
BOOST_AUTO_TEST_CASE(vardecl_complex) {
  BOOST_CHECK(
      successParse("{ function add(a:u256, b:u256) -> c:u256 {} let y:u256 := "
                   "2:u256 let x:u256 := add(7:u256, add(6:u256, y)) }"));
}

BOOST_AUTO_TEST_CASE(blocks) {
  BOOST_CHECK(successParse("{ let x:u256 := 7:u256 { let y:u256 := 3:u256 } { "
                           "let z:u256 := 2:u256 } }"));
}

BOOST_AUTO_TEST_CASE(function_definitions) {
  BOOST_CHECK(
      successParse("{ function f() { } function g(a:u256) -> x:u256 { } }"));
}

BOOST_AUTO_TEST_CASE(function_definitions_multiple_args) {
  BOOST_CHECK(successParse("{ function f(a:u256, d:u256) { } function "
                           "g(a:u256, d:u256) -> x:u256, y:u256 { } }"));
}

BOOST_AUTO_TEST_CASE(function_calls) {
  BOOST_CHECK(successParse("{ function f(a:u256) -> b:u256 {} function "
                           "g(a:u256, b:u256, c:u256) {} function x() { "
                           "g(1:u256, 2:u256, f(3:u256)) x() } }"));
}

BOOST_AUTO_TEST_CASE(tuple_assignment) {
  BOOST_CHECK(successParse("{ function f() -> a:u256, b:u256, c:u256 {} let "
                           "x:u256, y:u256, z:u256 := f() }"));
}
...
BOOST_AUTO_TEST_CASE(lacking_types) {
  CHECK_ERROR("{ let x := 1:u256 }", ParserError,
              "Expected token Identifier got 'Assign'");
  CHECK_ERROR("{ let x:u256 := 1 }", ParserError,
              "Expected token Colon got 'RBrace'");
  CHECK_ERROR("{ function f(a) {} }", ParserError,
              "Expected token Colon got 'RParen'");
  CHECK_ERROR("{ function f(a:u256) -> b {} }", ParserError,
              "Expected token Colon got 'LBrace'");
}

BOOST_AUTO_TEST_CASE(invalid_types) {
  /// testing invalid literal
  /// NOTE: these will need to change when types are compared
  CHECK_ERROR("{ let x:bool := 1:invalid }", TypeError,
              "\"invalid\" is not a valid type (user defined types are not yet "
              "supported).");
  /// testing invalid variable declaration
  CHECK_ERROR("{ let x:invalid := 1:bool }", TypeError,
              "\"invalid\" is not a valid type (user defined types are not yet "
              "supported).");
  CHECK_ERROR("{ function f(a:invalid) {} }", TypeError,
              "\"invalid\" is not a valid type (user defined types are not yet "
              "supported).");
}
...
BOOST_AUTO_TEST_CASE(number_literals) {
  ...
  CHECK_ERROR("{ let x:u256 := .1:u256 }", ParserError,
              "Invalid number literal.");
  CHECK_ERROR("{ let x:u256 := 1e5:u256 }", ParserError,
              "Invalid number literal.");
  CHECK_ERROR("{ let x:u256 := 67.235:u256 }", ParserError,
              "Invalid number literal.");
}
...
BOOST_AUTO_TEST_CASE(multiple_assignment) {
  CHECK_ERROR(
      "{ let x:u256 function f() -> a:u256, b:u256 {} 123:u256, x := f() }",
      ParserError,
...
BOOST_AUTO_TEST_CASE(if_statement) {
  ...
  BOOST_CHECK(successParse(
      "{ function f() -> x:bool {} if f() { let b:bool := f() } }"));
}

BOOST_AUTO_TEST_CASE(if_statement_invalid) {
  CHECK_ERROR("{ if let x:u256 {} }", ParserError,
              "Literal or identifier expected.");
  CHECK_ERROR("{ if true:bool let x:u256 := 3:u256 }", ParserError,
