...
string getIPCSocketPath() {
	...
	if (ipcPath.empty())
		BOOST_FAIL("ERROR: ipcPath not set! (use --ipcpath <path> or the "
		           "environment variable ETH_TEST_IPC)");
...
ExecutionFramework::compareAndCreateMessage(bytes const &_result,
	...
	for (size_t i = 0; i < std::max(resultHex.size(), expectedHex.size());
	     i += 0x40) {
		std::string result {i >= resultHex.size() ? string {}
				    : resultHex.substr(i, 0x40)};
		std::string expected {i > expectedHex.size() ? string {}
				      : expectedHex.substr(i, 0x40)};
...
void ExecutionFramework::sendMessage(bytes const &_data, bool _isCreation,
                                     u256 const &_value) {
	if (m_showMessages) {
		if (_isCreation)
			cout << "CREATE " << m_sender.hex() << ":" << endl;
		else
			cout << "CALL   " << m_sender.hex() << " -> " << m_contractAddress.hex()
			     << ":" << endl;
		if (_value > 0)
	...
	RPCSession::TransactionReceipt receipt(
		m_rpc.eth_getTransactionReceipt(txHash));
