...
*/
/**
 * Common code generator for translating Julia / inline assembly to EVM and
 * EVM1.5.
 */
...
void CodeTransform::operator()(VariableDeclaration const &_varDecl) {
  ...
  for (auto const &variable : _varDecl.variables) {
    auto &var =
        boost::get<Scope::Variable>(m_scope->identifiers.at(variable.name));
...
void CodeTransform::operator()(Label const &_label) {
  ...
  Scope::Label &label =
      boost::get<Scope::Label>(m_scope->identifiers.at(_label.name));
...
void CodeTransform::operator()(FunctionCall const &_call) {
  ...
  solAssert(m_scope->lookup(_call.functionName.name,
                            Scope::NonconstVisitor(
                                [=](Scope::Variable &) {
                                  solAssert(false, "Expected function name.");
                                },
                                [=](Scope::Label &) {
                                  solAssert(false, "Expected function name.");
                                },
                                [&](Scope::Function &_function) {
                                  function = &_function;
                                })),
  ...
  if (m_evm15)
    m_assembly.appendJumpsub(
        ...
        function->arguments.size(), function->returns.size());
  else {
    m_assembly.appendJumpTo(functionEntryID(_call.functionName.name, *function),
...
void CodeTransform::operator()(FunctionalInstruction const &_instruction) {
  if (m_evm15 && (_instruction.instruction == solidity::Instruction::JUMP ||
    ...
    auto label = labelFromIdentifier(
        boost::get<assembly::Identifier>(_instruction.arguments.at(0)));
    if (isJumpI)
...
void CodeTransform::operator()(assembly::Identifier const &_identifier) {
          ...
          Scope::NonconstVisitor(
              [=](Scope::Variable &_var) {
                if (int heightDiff = variableHeightDiff(_var, false))
                  m_assembly.appendInstruction(
                      solidity::dupInstruction(heightDiff));
                else
  ...
  solAssert(m_identifierAccess.generateCode,
            "Identifier not found and no external access available.");
  m_identifierAccess.generateCode(_identifier, IdentifierContext::RValue,
                                  m_assembly);
...
void CodeTransform::operator()(assembly::Literal const &_literal) {
  ...
  if (_literal.kind == assembly::LiteralKind::Number)
    ...
    m_assembly.appendConstant(
        u256(h256(_literal.value, h256::FromBinary, h256::AlignLeft)));
  }
  checkStackHeight(&_literal);
}

void CodeTransform::operator()(assembly::Instruction const &_instruction) {
  solAssert(!m_evm15 || _instruction.instruction != solidity::Instruction::JUMP,
            "Bare JUMP instruction used for EVM1.5");
  solAssert(!m_evm15 ||
                _instruction.instruction != solidity::Instruction::JUMPI,
            "Bare JUMPI instruction used for EVM1.5");
  m_assembly.setSourceLocation(_instruction.location);
  m_assembly.appendInstruction(_instruction.instruction);
  checkStackHeight(&_instruction);
}
...
void CodeTransform::operator()(FunctionDefinition const &_function) {
  ...
  Scope::Function &function =
      boost::get<Scope::Function>(m_scope->identifiers.at(_function.name));

  int const localStackAdjustment = m_evm15 ? 0 : 1;
  int height = localStackAdjustment;
  solAssert(m_info.scopes.at(&_function.body), "");
  Scope *varScope =
      m_info.scopes.at(m_info.virtualBlocks.at(&_function).get()).get();
  solAssert(varScope, "");
  ...
  if (m_evm15) {
    ...
    m_assembly.appendBeginsub(functionEntryID(_function.name, function),
  ...
  CodeTransform(m_assembly, m_info, m_julia, m_evm15, m_identifierAccess,
                m_useNamedLabelsForFunctions, localStackAdjustment,
                m_context)(_function.body);

  {
    ...
    if (!m_evm15)
      stackLayout.push_back(
          _function.returnVariables.size()); // Move return label to the top
    stackLayout +=
        vector<int>(_function.parameters.size(), -1); // discard all arguments
    ...
    while (!stackLayout.empty() &&
           stackLayout.back() != int(stackLayout.size() - 1))
      if (stackLayout.back() < 0) {
        m_assembly.appendInstruction(solidity::Instruction::POP);
        stackLayout.pop_back();
      } else {
        m_assembly.appendInstruction(
            swapInstruction(stackLayout.size() - stackLayout.back() - 1));
  ...
  if (m_evm15)
    m_assembly.appendReturnsub(_function.returnVariables.size(),
                               stackHeightBefore);
  else
...
void CodeTransform::operator()(ForLoop const &_forLoop) {
  ...
  // TODO: When we implement break and continue, the labels and the stack
  // heights at that point have to be stored in a stack.
...
CodeTransform::labelFromIdentifier(Identifier const &_identifier) {
  AbstractAssembly::LabelID label = AbstractAssembly::LabelID(-1);
  if (!m_scope->lookup(
          _identifier.name,
          Scope::NonconstVisitor(
              [=](Scope::Variable &) { solAssert(false, "Expected label"); },
              [&](Scope::Label &_label) { label = labelID(_label); },
              [=](Scope::Function &) {
                solAssert(false, "Expected label");
              }))) {
...
AbstractAssembly::LabelID
CodeTransform::functionEntryID(string const &_name,
...
void CodeTransform::finalizeBlock(Block const &_block,
                                  int blockStartStackHeight) {
...
void CodeTransform::generateMultiAssignment(
    vector<Identifier> const &_variableNames) {
  solAssert(m_scope, "");
...
void CodeTransform::generateAssignment(Identifier const &_variableName) {
  ...
  if (var) {
    ...
    solAssert(m_identifierAccess.generateCode,
              "Identifier not found and no external access available.");
    m_identifierAccess.generateCode(_variableName, IdentifierContext::LValue,
                                    m_assembly);
  }
}

int CodeTransform::variableHeightDiff(
    solidity::assembly::Scope::Variable const &_var, bool _forSwap) {
  solAssert(m_context->variableStackHeights.count(&_var), "");
  int heightDiff =
      m_assembly.stackHeight() - m_context->variableStackHeights[&_var];
  if (heightDiff <= (_forSwap ? 1 : 0) || heightDiff > (_forSwap ? 17 : 16)) {
    solUnimplemented("Variable inaccessible, too deep inside stack (" +
                     boost::lexical_cast<string>(heightDiff) + ")");
    return 0;
  } else
    return heightDiff;
}
...
void CodeTransform::expectDeposit(int _deposit, int _oldHeight) {
  solAssert(m_assembly.stackHeight() == _oldHeight + _deposit,
            "Invalid stack deposit.");
}

void CodeTransform::checkStackHeight(void const *_astElement) {
  solAssert(m_info.stackHeightInfo.count(_astElement),
            "Stack height for AST element not found.");
  solAssert(m_info.stackHeightInfo.at(_astElement) ==
                m_assembly.stackHeight() - m_stackAdjustment,
