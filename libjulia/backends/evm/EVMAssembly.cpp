...
namespace {
/// Size of labels in bytes. Four-byte labels are required by some EVM1.5
/// instructions.
...
void EVMAssembly::appendInstruction(solidity::Instruction _instr) {
	...
	m_stackHeight += solidity::instructionInfo(_instr).ret -
	                 solidity::instructionInfo(_instr).args;
}
...
void EVMAssembly::appendLabelReference(LabelID _labelId) {
	...
	// @TODO we now always use labelReferenceSize for all labels, it could be
	// shortened for some of them.
...
void EVMAssembly::appendJumpsub(LabelID _labelId, int _arguments,
                                int _returns) {
