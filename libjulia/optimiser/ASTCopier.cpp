...
Statement ASTCopier::operator()(FunctionCall const &_call) {
	return FunctionCall {_call.location, translate(_call.functionName),
		             translateVector(_call.arguments)};
}

Statement ASTCopier::operator()(FunctionalInstruction const &_instruction) {
	return FunctionalInstruction {_instruction.location, _instruction.instruction,
		                      translateVector(_instruction.arguments)};
}

Statement ASTCopier::operator()(Identifier const &_identifier) {
	return Identifier {_identifier.location,
		           translateIdentifier(_identifier.name)};
}
...
Statement ASTCopier::operator()(Switch const &_switch) {
	return Switch {_switch.location, translate(_switch.expression),
		       translateVector(_switch.cases)};
}
...
Statement ASTCopier::operator()(FunctionDefinition const &_function) {
	...
	ScopeGuard g([&]() {
		this->leaveFunction(_function);
	});

	return FunctionDefinition {_function.location, move(translatedName),
...
Statement ASTCopier::operator()(ForLoop const &_forLoop) {
	...
	ScopeGuard g([&]() {
		this->leaveScope(_forLoop.pre);
	});

	return ForLoop {_forLoop.location, translate(_forLoop.pre),
		        translate(_forLoop.condition), translate(_forLoop.post),
...
TypedName ASTCopier::translate(TypedName const &_typedName) {
	return TypedName {_typedName.location, translateIdentifier(_typedName.name),
		          _typedName.type};
}
