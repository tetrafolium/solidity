...
protected:
  template <class T> void walkVector(T const &_statements) {
...
protected:
  template <class T> void walkVector(T &&_statements) {
    for (auto &st : _statements)
      visit(st);
  }
  virtual void visit(Statement &_st) { boost::apply_visitor(*this, _st); }
};
