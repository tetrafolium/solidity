...
decshake(128) decshake(256) decsha3(224) decsha3(256) decsha3(384) decsha3(512)
    deckeccak(224) deckeccak(256) deckeccak(384) deckeccak(512)

    /******** The Keccak-f[1600] permutation ********/

    /*** Constants. ***/
    static const uint8_t rho[24] = {1,  3,  6,  10, 15, 21, 28, 36,
...
#define defkeccak(bits)                                                        \
  int keccak##bits(uint8_t *out, size_t outlen, const uint8_t *in,             \
                   size_t inlen) {                                             \
    if (outlen > (bits / 8)) {                                                 \
      return -1;                                                               \
    }                                                                          \
...
bool keccak256(bytesConstRef _input, bytesRef o_output) {
  ...
  //	keccak::keccak(ret.data(), 32, (uint64_t const*)_input.data(),
  //_input.size());
  return true;
}
