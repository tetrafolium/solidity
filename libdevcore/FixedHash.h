...
/// this will assume the data contained in the hash is big-endian.
...
explicit FixedHash(bytes const &_b,
...
/// Explicitly construct, copying from a byte array.
explicit FixedHash(bytesConstRef _b,
                   ConstructFromHashType _t = FailIfDifferent) {
	if (_b.size() == N)
		memcpy(m_data.data(), _b.data(), std::min<unsigned>(_b.size(), N));
	else {
...
explicit FixedHash(std::string const &_s,
	...
	: FixedHash(_t == FromHex ? fromHex(_s, WhenError::Throw)
		    : dev::asBytes(_s),
	            _ht) {
}
...
/// Returns the index of the first bit set to one, or size() * 8 if no bits
/// are set.
