...
#pragma warning( \
        disable : 597)// will not be called for implicit or explicit conversions
#endif
...
using string_type =
	      typename std::conditional<std::is_const<_T>::value, std::string const,
	                                std::string>::type;
using vector_type = typename std::conditional<
	      std::is_const<_T>::value,
	      std::vector<typename std::remove_const<_T>::type> const,
	      std::vector<_T> >::type;

static_assert(std::is_pod<value_type>::value, "vector_ref can only be used "
              "with PODs due to its "
              "low-level treatment of data.");

vector_ref() : m_data(nullptr), m_count(0) {
}
/// Creates a new vector_ref to point to @a _count elements starting at @a
/// _data.
...
template <class _T2> explicit operator vector_ref<_T2>() const {
	assert(m_count * sizeof(_T) / sizeof(_T2) * sizeof(_T2) / sizeof(_T) ==
	       m_count);
	return vector_ref<_T2>(reinterpret_cast<_T2 *>(m_data),
	                       m_count * sizeof(_T) / sizeof(_T2));
}
...
/// @returns the number of elements referenced (not necessarily number of
/// bytes).
size_t size() const {
	return m_count;
}
...
/// @returns a new vector_ref which is a shifted and shortened view of the
/// original data. If this goes out of bounds in any way, returns an empty
/// vector_ref. If @a _count is ~size_t(0), extends the view to the end of the
/// data.
vector_ref<_T> cropped(size_t _begin, size_t _count) const {
	if (m_data && _begin <= m_count && _count <= m_count &&
	    _begin + _count <= m_count)
		return vector_ref<_T>(m_data + _begin,
		                      _count == ~size_t(0) ? m_count - _begin : _count);
	else
