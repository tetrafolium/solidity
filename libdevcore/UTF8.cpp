...
bool validateUTF8(const unsigned char *_input, size_t _length,
                  size_t &_invalidPosition) {
  bool valid = true;
  size_t i = 0;
...
bool validateUTF8(std::string const &_input, size_t &_invalidPosition) {
  return validateUTF8(reinterpret_cast<unsigned char const *>(_input.c_str()),
                      _input.length(), _invalidPosition);
}
