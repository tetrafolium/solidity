...
wrapReadCallback(CStyleReadFileCallback _readCallback = nullptr) {
...
string compile(StringMap const &_sources, bool _optimize,
  ...
  if (ret.isMember("errors")) {
    ...
    for (auto const &error : ret["errors"])
      output["errors"].append(!error["formattedMessage"].empty()
  ...
  if (ret.isMember("sources")) {
    ...
    for (auto const &sourceName : ret["sources"].getMemberNames()) {
      ...
      output["sources"][sourceName]["AST"] =
          ret["sources"][sourceName]["legacyAST"];
    }
  }
  ...
  if (ret.isMember("contracts")) {
    ...
    for (auto const &sourceName : ret["contracts"].getMemberNames())
      for (auto const &contractName :
           ret["contracts"][sourceName].getMemberNames()) {
        Json::Value contractInput = ret["contracts"][sourceName][contractName];
        Json::Value contractOutput = Json::objectValue;
        contractOutput["interface"] =
            dev::jsonCompactPrint(contractInput["abi"]);
        contractOutput["metadata"] = contractInput["metadata"];
        contractOutput["functionHashes"] =
            contractInput["evm"]["methodIdentifiers"];
        contractOutput["gasEstimates"] =
            translateGasEstimates(contractInput["evm"]["gasEstimates"]);
        ...
        contractOutput["srcmap"] =
            contractInput["evm"]["bytecode"]["sourceMap"];
        contractOutput["runtimeBytecode"] =
            contractInput["evm"]["deployedBytecode"]["object"];
        contractOutput["srcmapRuntime"] =
            contractInput["evm"]["deployedBytecode"]["sourceMap"];
...
string compileMulti(string const &_input, bool _optimize,
                    CStyleReadFileCallback _readCallback = nullptr) {
  Json::Reader reader;
  Json::Value input;
  if (!reader.parse(_input, input, false)) {
    Json::Value errors(Json::arrayValue);
    errors.append("Error parsing input JSON: " +
...
string compileStandardInternal(string const &_input,
                               CStyleReadFileCallback _readCallback = nullptr) {
  StandardCompiler compiler(wrapReadCallback(_readCallback));
...
extern char const *version() { return VersionString.c_str(); }
...
extern char const *compileJSONCallback(char const *_input, bool _optimize,
                                       CStyleReadFileCallback _readCallback) {
  s_outputBuffer = compileMulti(_input, _optimize, _readCallback);
  return s_outputBuffer.c_str();
}
extern char const *compileStandard(char const *_input,
                                   CStyleReadFileCallback _readCallback) {
