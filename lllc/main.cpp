...
static string const VersionString =
    ...
    (string(SOL_VERSION_PRERELEASE).empty()
         ? ""
         : "-" + string(SOL_VERSION_PRERELEASE)) +
    (string(SOL_VERSION_BUILDINFO).empty()
         ? ""
         : "+" + string(SOL_VERSION_BUILDINFO));

static void help() {
  cout << "Usage lllc [OPTIONS] <file>" << endl
       << "Options:" << endl
       << "    -b,--binary  Parse, compile and assemble; output byte code in "
          "binary."
       << endl
...
int main(int argc, char **argv) {
  ...
  if (src.empty()) {
    ...
    auto bs =
        compileLLL(src, optimise ? true : false, &errors, readFileAsString);
    if (mode == Hex)
      cout << toHex(bs) << endl;
    else if (mode == Binary)
    ...
    cout << compileLLLToAsm(src, optimise ? true : false, &errors,
                            readFileAsString)
         << endl;
  }
