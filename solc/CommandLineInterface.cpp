...
static void version() {
	cout << "solc, the solidity compiler commandline interface" << endl
...
static bool needsHumanTargetedStdout(po::variables_map const &_args) {
	...
	for (string const &arg :
	     {g_argAbi, g_argAsm, g_argAsmJson, g_argAstJson, g_argBinary,
	      g_argBinaryRuntime, g_argCloneBinary, g_argFormal, g_argMetadata,
	      g_argNatspecUser, g_argNatspecDev, g_argOpcodes, g_argSignatureHashes})
		if (_args.count(arg))
			return true;
	return false;
}
...
void CommandLineInterface::handleBinary(string const &_contract) {
	if (m_args.count(g_argBinary)) {
		if (m_args.count(g_argOutputDir))
			createFile(m_compiler->filesystemFriendlyName(_contract) + ".bin",
			           m_compiler->object(_contract).toHex());
		else {
			cout << "Binary: " << endl;
	...
	if (m_args.count(g_argCloneBinary)) {
		if (m_args.count(g_argOutputDir))
			createFile(m_compiler->filesystemFriendlyName(_contract) + ".clone_bin",
			           m_compiler->cloneObject(_contract).toHex());
		else {
	...
	if (m_args.count(g_argBinaryRuntime)) {
		if (m_args.count(g_argOutputDir))
			createFile(m_compiler->filesystemFriendlyName(_contract) + ".bin-runtime",
			           m_compiler->runtimeObject(_contract).toHex());
		else {
...
void CommandLineInterface::handleOpcode(string const &_contract) {
	if (m_args.count(g_argOutputDir))
		createFile(m_compiler->filesystemFriendlyName(_contract) + ".opcode",
		           solidity::disassemble(m_compiler->object(_contract).bytecode));
	else {
		cout << "Opcodes: " << endl;
...
void CommandLineInterface::handleBytecode(string const &_contract) {
	...
	if (m_args.count(g_argBinary) || m_args.count(g_argCloneBinary) ||
	    m_args.count(g_argBinaryRuntime))
...
void CommandLineInterface::handleSignatureHashes(string const &_contract) {
	...
	if (m_args.count(g_argOutputDir))
		createFile(m_compiler->filesystemFriendlyName(_contract) + ".signatures",
		           out);
	else
...
void CommandLineInterface::handleMetadata(string const &_contract) {
	...
	if (m_args.count(g_argOutputDir))
		createFile(m_compiler->filesystemFriendlyName(_contract) + "_meta.json",
		           data);
	else
...
void CommandLineInterface::handleNatspec(bool _natspecDev,
	...
	if (m_args.count(argName)) {
		std::string output =
			dev::jsonPrettyPrint(_natspecDev ? m_compiler->natspecDev(_contract)
					     : m_compiler->natspecUser(_contract));

		if (m_args.count(g_argOutputDir))
			createFile(m_compiler->filesystemFriendlyName(_contract) + suffix,
			           output);
		else {
			cout << title << endl;
			cout << output << endl;
		...
		for (string path : m_args[g_argInputFile].as<vector<string> >()) {
			...
			if (eq != path.end())
				...
				if (!boost::filesystem::exists(infile)) {
					cerr << "Skipping non-existent input file \"" << infile << "\""
					     << endl;
					continue;
				}
...
bool CommandLineInterface::parseLibraryOption(string const &_input) {
	...
	boost::split(libraries, data, boost::is_space() || boost::is_any_of(","),
	             boost::token_compress_on);
	for (string const &lib : libraries)
		if (!lib.empty()) {
			// search for last colon in string as our binaries output placeholders in
			// the form of file:Name  so we need to search for the second `:` in the
			// string
			auto colon = lib.rfind(':');
			if (colon == string::npos) {
				cerr << "Colon separator missing in library address specifier \"" << lib
				     << "\"" << endl;
				return false;
			}
			...
			if (!passesAddressChecksum(addrString, false)) {
				cerr << "Invalid checksum on library address \"" << libName
				     << "\": " << addrString << endl;
				return false;
			}
			...
			if (binAddr.size() > 20 || address == h160()) {
				cerr << "Invalid address for library \"" << libName
				     << "\": " << addrString << endl;
				return false;
			}
...
void CommandLineInterface::createFile(string const &_fileName,
                                      string const &_data) {
	namespace fs = boost::filesystem;
	...
	if (fs::exists(pathName) && !m_args.count(g_strOverwrite)) {
		cerr << "Refusing to overwrite existing file \"" << pathName
		     << "\" (use --overwrite to force)." << endl;
		m_error = true;
		return;
	}
	...
	if (!outFile)
		BOOST_THROW_EXCEPTION(
			FileError() << errinfo_comment("Could not write to file: " + pathName));
}

void CommandLineInterface::createJson(string const &_fileName,
                                      string const &_json) {
...
Allowed options)",
	...
	desc.add_options()(g_argHelp.c_str(), "Show help message and exit.")(
		...
		g_argPrettyJson.c_str(), "Output JSON in pretty format. Currently it "
		"only works with the combined JSON output.")(
		g_argLibraries.c_str(), po::value<vector<string> >()->value_name("libs"),
		"Direct string or file containing library addresses. Syntax: "
		"<libraryName>: <address> [, or whitespace] ...\n"
		"Address is interpreted as a hex string optionally prefixed by 0x.")(
		(g_argOutputDir + ",o").c_str(), po::value<string>()->value_name("path"),
		...
		"It reads from standard input and provides the result on the standard "
		"output.")(g_argAssemble.c_str(), "Switch to assembly mode, ignoring all "
	                   "options except --machine and assumes "
	                   "input is assembly.")(
		g_argJulia.c_str(), "Switch to JULIA mode, ignoring all options except "
		...
		"Store referenced sources are literal data in the metadata output.")(
		g_argAllowPaths.c_str(), po::value<string>()->value_name("path(s)"),
		"Allow a given path for imports. A list of paths can be supplied by "
		"separating them with a comma.");
	po::options_description outputComponents("Output Components");
	outputComponents.add_options()(g_argAst.c_str(), "AST of all source files.")(
	...
	if (m_args.count(g_argCombinedJson)) {
		...
		for (string const &item :
		     boost::split(requests, m_args[g_argCombinedJson].as<string>(),
		                  boost::is_any_of(",")))
...
bool CommandLineInterface::processInput() {
						    ...
						    try {
							    ...
							    for (auto const &allowedDir : m_allowedDirectories) {
								    ...
								    if (std::distance(allowedDir.begin(), allowedDir.end()) <=
								        ...
								        std::equal(allowedDir.begin(), allowedDir.end(),
								                   canonicalPath.begin())) {
									    isAllowed = true;
									    break;
								    }
							    ...
							    return ReadCallback::Result {
									   false, "Exception in read callback: " +
	...
	if (m_args.count(g_argAllowPaths)) {
		vector<string> paths;
		for (string const &path :
		     boost::split(paths, m_args[g_argAllowPaths].as<string>(),
		                  boost::is_any_of(","))) {
	...
	if (m_args.count(g_argAssemble) || m_args.count(g_argJulia)) {
		...
		Input inputLanguage =
			m_args.count(g_argJulia) ? Input::JULIA : Input::Assembly;
	...
	auto scannerFromSourceName =
		[&](string const &_sourceName) -> solidity::Scanner const & {
	...
	try {
		...
		SourceReferenceFormatter::printExceptionInformation(
			cerr, _exception, "Compiler error", scannerFromSourceName);
		return false;
		...
		if (_error.type() == Error::Type::DocstringParsingError)
			cerr << "Documentation parsing error: "
			     << *boost::get_error_info<errinfo_comment>(_error) << endl;
		else
			SourceReferenceFormatter::printExceptionInformation(
		...
		cerr << "Exception during compilation: "
...
void CommandLineInterface::handleCombinedJSON() {
	...
	for (string const &contractName : contracts) {
		...
		if (requests.count(g_strAbi))
			contractData[g_strAbi] =
				dev::jsonCompactPrint(m_compiler->contractABI(contractName));
		...
		if (requests.count(g_strBinaryRuntime))
			contractData[g_strBinaryRuntime] =
				m_compiler->runtimeObject(contractName).toHex();
		if (requests.count(g_strCloneBinary))
			contractData[g_strCloneBinary] =
				m_compiler->cloneObject(contractName).toHex();
		if (requests.count(g_strOpcodes))
			contractData[g_strOpcodes] =
				solidity::disassemble(m_compiler->object(contractName).bytecode);
		if (requests.count(g_strAsm))
			contractData[g_strAsm] =
				m_compiler->assemblyJSON(contractName, m_sourceCodes);
		...
		if (requests.count(g_strSignatureHashes))
			contractData[g_strSignatureHashes] =
				m_compiler->methodIdentifiers(contractName);
		if (requests.count(g_strNatspecDev))
			contractData[g_strNatspecDev] =
				dev::jsonCompactPrint(m_compiler->natspecDev(contractName));
		if (requests.count(g_strNatspecUser))
			contractData[g_strNatspecUser] =
				dev::jsonCompactPrint(m_compiler->natspecUser(contractName));
	}

	bool needsSourceList = requests.count(g_strAst) ||
	                       requests.count(g_strSrcMap) ||
	                       requests.count(g_strSrcMapRuntime);
	if (needsSourceList) {
		// Indices into this array are used to abbreviate source names in source
		// locations.
	...
	if (requests.count(g_strAst)) {
		...
		for (auto const &sourceCode : m_sourceCodes) {
			...
			output[g_strSources][sourceCode.first]["AST"] =
				converter.toJson(m_compiler->ast(sourceCode.first));
		}
	}

	string json = m_args.count(g_argPrettyJson) ? dev::jsonPrettyPrint(output)
		      : dev::jsonCompactPrint(output);
...
void CommandLineInterface::handleAst(string const &_argStr) {
	...
	if (_argStr == g_argAst)
		...
		BOOST_THROW_EXCEPTION(InternalCompilerError()
		                      << errinfo_comment("Illegal argStr for AST"));
	...
	if (m_args.count(_argStr)) {
		...
		if (m_compiler->runtimeAssemblyItems(m_compiler->lastContractName()))
			gasCosts = GasEstimator::breakToStatementLevel(
				GasEstimator::structuralEstimation(
		...
		if (m_args.count(g_argOutputDir)) {
			for (auto const &sourceCode : m_sourceCodes) {
				...
				string postfix = "";
				if (_argStr == g_argAst) {
					ASTPrinter printer(m_compiler->ast(sourceCode.first),
					                   sourceCode.second);
					printer.print(data);
				} else {
			...
			for (auto const &sourceCode : m_sourceCodes) {
				...
				if (_argStr == g_argAst) {
					ASTPrinter printer(m_compiler->ast(sourceCode.first),
					                   sourceCode.second, gasCosts);
					printer.print(cout);
				} else
					ASTJsonConverter(legacyFormat, m_compiler->sourceIndices())
					.print(cout, m_compiler->ast(sourceCode.first));
			}
		}
	}
}
...
bool CommandLineInterface::link() {
	// Map from how the libraries will be named inside the bytecode to their
	// addresses.
	map<string, h160> librariesReplacements;
	int const placeholderSize = 40; // 20 bytes or 40 hex characters
	for (auto const &library : m_libraries) {
		string const &name = library.first;
		// Library placeholders are 40 hex digits (20 bytes) that start and end with
		// '__'. This leaves 36 characters for the library name, while too short
		// library names are padded on the right with '_' and too long names are
		// truncated.
	...
	for (auto &src : m_sourceCodes) {
		...
		for (auto it = src.second.begin(); it != end; ) {
			...
			if (end - it < placeholderSize) {
				cerr << "Error in binary object file " << src.first << " at position "
				     << (end - src.second.begin()) << endl;
				return false;
			}
			...
			if (librariesReplacements.count(name)) {
				...
				cerr << "Reference \"" << name << "\" in file \"" << src.first
				     << "\" still unresolved." << endl;
			it += placeholderSize;
		}
	}
	return true;
...
bool CommandLineInterface::assemble(AssemblyStack::Language _language,
	...
	for (auto const &src : m_sourceCodes) {
		string machine = _targetMachine == AssemblyStack::Machine::EVM
		                 ? "EVM"
				 : _targetMachine == AssemblyStack::Machine::EVM15
		                 ? "EVM 1.5"
				 : "eWasm";
		cout << endl
		     << "======= " << src.first << " (" << machine << ") =======" << endl;
		...
		try {
			...
			cerr << "Exception while assembling: "
			     << boost::diagnostic_information(_exception) << endl;
			return false;
		} catch (...) {
	...
	for (string const &contract : contracts) {
		...
		if (m_args.count(g_argAsm) || m_args.count(g_argAsmJson)) {
			...
			if (m_args.count(g_argAsmJson))
				ret = dev::jsonPrettyPrint(
					m_compiler->assemblyJSON(contract, m_sourceCodes));
			else
				ret = m_compiler->assemblyString(contract, m_sourceCodes);

			if (m_args.count(g_argOutputDir)) {
				createFile(m_compiler->filesystemFriendlyName(contract) +
				           (m_args.count(g_argAsmJson) ? "_evm.json" : ".evm"),
				           ret);
			} else {
