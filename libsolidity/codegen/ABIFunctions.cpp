 ...
 * @date 2017
 * Routines that generate JULIA code related to ABI encoding, decoding and type
 * conversions.
 */
...
string ABIFunctions::tupleEncoder(TypePointers const &_givenTypes,
  ...
  return createFunction(functionName, [&]() {
    ...
    // Note that the values are in reverse due to the difference in calling
    // semantics.
    Whiskers templ(R"(
    ...
    for (size_t i = 0; i < _givenTypes.size(); ++i) {
      ...
      Whiskers elementTempl(dynamic ? string(R"(
      ...
      elementTempl("abiEncode",
                   abiEncodingFunction(*_givenTypes[i], *_targetTypes[i],
                                       _encodeAsLibraryTypes, true));
...
string ABIFunctions::tupleDecoder(TypePointers const &_types,
                                  bool _fromMemory) {
  ...
  return createFunction(functionName, [&]() {
    ...
    for (size_t i = 0; i < _types.size(); ++i) {
      ...
      elementTempl("pos", to_string(headPos));
      elementTempl("abiDecode",
                   abiDecodingFunction(*_types[i], _fromMemory, true));
...
string ABIFunctions::cleanupFunction(Type const &_type, bool _revertOnFailure) {
  string functionName = string("cleanup_") +
                        (_revertOnFailure ? "revert_" : "assert_") +
                        _type.identifier();
  return createFunction(functionName, [&]() {
    Whiskers templ(R"(
    ...
    case Type::Category::Integer: {
      ...
      if (type.numBits() == 256)
        ...
        templ("body", "cleaned := signextend(" +
                          to_string(type.numBits() / 8 - 1) + ", value)");
      else
        templ("body",
              "cleaned := and(value, " +
                  toCompactHexWithPrefix((u256(1) << type.numBits()) - 1) +
                  ")");
      break;
    }
    ...
    case Type::Category::Struct:
      solAssert(_type.dataStoredIn(DataLocation::Storage),
                "Cleanup requested for non-storage reference type.");
      templ("body", "cleaned := value");
      break;
    case Type::Category::FixedBytes: {
      ...
      if (type.numBytes() == 32)
        ...
        templ("body",
              "cleaned := and(value, " + toCompactHexWithPrefix(mask) + ")");
      }
      break;
    }
    ...
    case Type::Category::Enum: {
      ...
      Whiskers w(
          "if iszero(lt(value, <members>)) { <failure> } cleaned := value");
...
string ABIFunctions::conversionFunction(Type const &_from, Type const &_to) {
  string functionName =
      "convert_" + _from.identifier() + "_to_" + _to.identifier();
  return createFunction(functionName, [&]() {
    Whiskers templ(R"(
    ...
    case Type::Category::RationalNumber:
    case Type::Category::Contract: {
      if (RationalNumberType const *rational =
              dynamic_cast<RationalNumberType const *>(&_from))
        solUnimplementedAssert(!rational->isFractional(),
                               "Not yet implemented - FixedPointType.");
      if (toCategory == Type::Category::FixedBytes) {
        solAssert(fromCategory == Type::Category::Integer ||
                      fromCategory == Type::Category::RationalNumber,
                  "Invalid conversion to FixedBytesType requested.");
        FixedBytesType const &toBytesType =
            dynamic_cast<FixedBytesType const &>(_to);
        body = Whiskers("converted := <shiftLeft>(<clean>(value))")(
                   "shiftLeft",
                   shiftLeftFunction(256 - toBytesType.numBytes() * 8))(
                   "clean", cleanupFunction(_from))
                   .render();
      } else if (toCategory == Type::Category::Enum) {
        solAssert(_from.mobileType(), "");
        body = Whiskers("converted := <cleanEnum>(<cleanInt>(value))")(
        ...
        solAssert(toCategory == Type::Category::Integer ||
                      toCategory == Type::Category::Contract,
                  "");
        IntegerType const addressType(160, IntegerType::Modifier::Address);
        IntegerType const &to = toCategory == Type::Category::Integer
        ...
        body = Whiskers("converted := <cleanInt>(value)")(
    ...
    case Type::Category::Bool: {
      ...
      body = Whiskers("converted := <clean>(value)")("clean",
                                                     cleanupFunction(_from))
                 .render();
      break;
    }
    case Type::Category::FixedPoint:
      ...
      if (toCategory == Type::Category::Integer)
        body =
            Whiskers("converted := <convert>(<shift>(value))")(
                ...
                "convert",
                conversionFunction(IntegerType(from.numBytes() * 8), _to))
                .render();
      else {
        // clear for conversion to longer bytes
        solAssert(toCategory == Type::Category::FixedBytes,
                  "Invalid type conversion requested.");
        body = Whiskers("converted := <clean>(value)")("clean",
                                                       cleanupFunction(from))
                   .render();
      }
      break;
    }
    ...
    return Whiskers(R"(
			function <functionName>(addr_and_selector) -> cleaned {
				cleaned := <clean>(addr_and_selector)
                    ...
                    functionName)("clean", cleanupFunction(FixedBytesType(24)))
        .render();
  });
}
  ...
  return createFunction(functionName, [&]() {
    return Whiskers(R"(
			function <functionName>(addr, selector) -> combined {
               ...
               "shl32", shiftLeftFunction(32))("shl64", shiftLeftFunction(64))
        .render();
  });
}
    ...
    return Whiskers(R"(
			function <functionName>(combined) -> addr, selector {
				...
				addr := <shr32>(combined)
                    ...
                    functionName)("shr32", shiftRightFunction(32, false))(
               "shr64", shiftRightFunction(64, false))
        .render();
  });
}

string ABIFunctions::abiEncodingFunction(Type const &_from, Type const &_to,
                                         bool _encodeAsLibraryTypes,
                                         bool _fromStack) {
  ...
  if (_from.category() == Type::Category::StringLiteral)
    ...
    if (fromArray.location() == DataLocation::CallData)
      ...
      return abiEncodingFunctionCompactStorageArray(fromArray, *toArray,
                                                    _encodeAsLibraryTypes);
    else
      solAssert(false, "");
...
string ABIFunctions::abiEncodingFunctionCalldataArray(
  ...
  solAssert(*fromArrayType.copyForLocation(DataLocation::Memory, true) ==
  ...
  return createFunction(functionName, [&]() {
    solUnimplementedAssert(
        ...
        "Only byte arrays can be encoded from calldata currently.");
    ...
    templ("storeLength", _to.isDynamicallySized()
                             ? "mstore(pos, length) pos := add(pos, 0x20)"
                             : "");
    templ("functionName", functionName);
...
string ABIFunctions::abiEncodingFunctionSimpleArray(
  ...
  solAssert(_from.dataStoredIn(DataLocation::Memory) ||
                _from.baseType()->storageBytes() > 16,
            "");
  ...
  return createFunction(functionName, [&]() {
    ...
    templ("elementEncodedSize",
          toCompactHexWithPrefix(_to.baseType()->calldataEncodedSize()));
    templ("encodeToMemoryFun",
          abiEncodingFunction(*_from.baseType(), *_to.baseType(),
                              _encodeAsLibraryTypes, false));
    templ("arrayElementAccess",
          inMemory
              ? "mload(srcPtr)"
              : _from.baseType()->isValueType() ? "sload(srcPtr)" : "srcPtr");
...
string ABIFunctions::abiEncodingFunctionCompactStorageArray(
  ...
  return createFunction(functionName, [&]() {
    if (_from.isByteArray()) {
      ...
      string elementEncodedSize =
          toCompactHexWithPrefix(_to.baseType()->calldataEncodedSize());
      templ("elementEncodedSize", elementEncodedSize);
      string encodeToMemoryFun = abiEncodingFunction(
          *_from.baseType(), *_to.baseType(), _encodeAsLibraryTypes, false);
      templ("encodeToMemoryFun", encodeToMemoryFun);
      std::vector<std::map<std::string, std::string>> items(itemsPerSlot);
      for (size_t i = 0; i < itemsPerSlot; ++i)
        items[i]["shiftRightFun"] =
            shiftRightFunction(i * storageBytes * 8, false);
      templ("items", items);
      return templ.render();
    }
  });
...
string ABIFunctions::abiEncodingFunctionStruct(StructType const &_from,
  ...
  solUnimplementedAssert(!_from.dataStoredIn(DataLocation::CallData),
                         "Encoding struct from calldata is not yet supported.");
  ...
  return createFunction(functionName, [&]() {
    ...
    for (auto const &member : _to.members(nullptr)) {
      ...
      solUnimplementedAssert(
          member.type->mobileType() &&
              ...
              member.type->mobileType()
                  ->interfaceType(_encodeAsLibraryTypes)
                  ->encodingType(),
          "Encoding type \"" + member.type->toString() +
              "\" not yet implemented.");
      auto memberTypeTo = member.type->mobileType()
                              ->interfaceType(_encodeAsLibraryTypes)
                              ->encodingType();
      ...
      if (fromStorage) {
        ...
        tie(storageSlotOffset, intraSlotOffset) =
            _from.storageOffsetsOfMember(member.name);
        if (memberTypeFrom->isValueType()) {
          if (storageSlotOffset != previousSlotOffset) {
            memberTempl("preprocess",
                        "slotValue := sload(add(value, " +
                            toCompactHexWithPrefix(storageSlotOffset) + "))");
            previousSlotOffset = storageSlotOffset;
          } else
            memberTempl("preprocess", "");
          memberTempl("retrieveValue",
                      shiftRightFunction(intraSlotOffset * 8, false) +
                          "(slotValue)");
        } else {
        ...
        memberTempl("preprocess", "");
        string sourceOffset =
            toCompactHexWithPrefix(_from.memoryOffsetOfMember(member.name));
        memberTempl("retrieveValue", "mload(add(value, " + sourceOffset + "))");
      }
      memberTempl("encodingOffset", toCompactHexWithPrefix(encodingOffset));
      encodingOffset +=
          dynamicMember ? 0x20 : memberTypeTo->calldataEncodedSize();
      memberTempl("abiEncode",
                  abiEncodingFunction(*memberTypeFrom, *memberTypeTo,
                                      _encodeAsLibraryTypes, false));
...
string ABIFunctions::abiEncodingFunctionStringLiteral(
  ...
  return createFunction(functionName, [&]() {
    ...
    if (_to.isDynamicallySized()) {
      ...
      // TODO this can make use of CODECOPY for large strings once we have that
      // in JULIA
      ...
      for (size_t i = 0; i < words; ++i) {
        ...
        wordParams[i]["wordValue"] =
            "0x" + h256(value.substr(32 * i, 32), h256::AlignLeft).hex();
      }
...
string ABIFunctions::abiEncodingFunctionFunctionType(FunctionType const &_from,
                                                     Type const &_to,
                                                     bool _encodeAsLibraryTypes,
                                                     bool _fromStack) {
  ...
  string functionName = "abi_encode_" + _from.identifier() + "_to_" +
                        _to.identifier() + (_fromStack ? "_fromStack" : "") +
                        (_encodeAsLibraryTypes ? "_library" : "");
...
string ABIFunctions::abiDecodingFunction(Type const &_type, bool _fromMemory,
                                         bool _forUseOnStack) {
  // The decoding function has to perform bounds checks unless it decodes a
  // value type. Conversely, bounds checks have to be performed before the
  // decoding function of a value type is called.
  ...
  } else if (auto const *structType =
                 dynamic_cast<StructType const *>(decodingType.get()))
    return abiDecodingFunctionStruct(*structType, _fromMemory);
  else if (auto const *functionType =
               dynamic_cast<FunctionType const *>(decodingType.get()))
...
string ABIFunctions::abiDecodingFunctionValueType(const Type &_type,
                                                  bool _fromMemory) {
  TypePointer decodingType = _type.decodingType();
  solAssert(decodingType, "");
  ...
  string functionName =
      "abi_decode_" + _type.identifier() + (_fromMemory ? "_fromMemory" : "");
  return createFunction(functionName, [&]() {
    Whiskers templ(R"(
...
string ABIFunctions::abiDecodingFunctionArray(ArrayType const &_type,
                                              bool _fromMemory) {
  ...
  return createFunction(functionName, [&]() {
    ...
    templ("retrieveLength", !_type.isDynamicallySized()
                                ? toCompactHexWithPrefix(_type.length())
                                : load + "(offset)");
    templ("allocate", allocationFunction());
    templ("allocationSize", arrayAllocationSizeFunction(_type));
    if (_type.isDynamicallySized())
      templ("storeLength", "mstore(array, length) offset := add(offset, 0x20) "
                           "dst := add(dst, 0x20)");
    else
      templ("storeLength", "");
    if (dynamicBase) {
      ...
      string baseEncodedSize =
          toCompactHexWithPrefix(_type.baseType()->calldataEncodedSize());
      templ("staticBoundsCheck", "switch gt(add(src, mul(length, " +
                                     baseEncodedSize +
                                     ")), end) case 1 { revert(0, 0) }");
      templ("retrieveElementPos", "src");
      templ("baseEncodedSize", baseEncodedSize);
    }
    templ("decodingFun",
          abiDecodingFunction(*_type.baseType(), _fromMemory, false));
    return templ.render();
  });
}
...
string ABIFunctions::abiDecodingFunctionCalldataArray(ArrayType const &_type) {
  ...
  solAssert(
      _type.baseType()->calldataEncodedSize() < u256("0xffffffffffffffff"), "");

  string functionName = "abi_decode_" + _type.identifier();
  return createFunction(functionName, [&]() {
    string templ;
    if (_type.isDynamicallySized())
      templ = R"(
    ...
    w("baseEncodedSize",
      toCompactHexWithPrefix(
          _type.isByteArray() ? 1 : _type.baseType()->calldataEncodedSize()));
    w("length", _type.isDynamicallyEncoded()
                    ? "length"
                    : toCompactHexWithPrefix(_type.length()));
    return w.render();
  });
...
string ABIFunctions::abiDecodingFunctionByteArray(ArrayType const &_type,
                                                  bool _fromMemory) {
  solAssert(_type.dataStoredIn(DataLocation::Memory), "");
  solAssert(_type.isByteArray(), "");
...
string ABIFunctions::abiDecodingFunctionStruct(StructType const &_type,
                                               bool _fromMemory) {
  string functionName =
      "abi_decode_" + _type.identifier() + (_fromMemory ? "_fromMemory" : "");
  ...
  return createFunction(functionName, [&]() {
    ...
    for (auto const &member : _type.members(nullptr)) {
      ...
      memberTempl("memoryOffset", toCompactHexWithPrefix(
                                      _type.memoryOffsetOfMember(member.name)));
      memberTempl("abiDecode",
                  abiDecodingFunction(*member.type, _fromMemory, false));
...
string ABIFunctions::abiDecodingFunctionFunctionType(FunctionType const &_type,
                                                     bool _fromMemory,
                                                     bool _forUseOnStack) {
...
string ABIFunctions::copyToMemoryFunction(bool _fromCalldata) {
  string functionName =
      "copy_" + string(_fromCalldata ? "calldata" : "memory") + "_to_memory";
  return createFunction(functionName, [&]() {
    if (_fromCalldata) {
      return Whiskers(R"(
...
string ABIFunctions::shiftRightFunction(size_t _numBits, bool _signed) {
  string functionName = "shift_right_" + to_string(_numBits) +
                        (_signed ? "_signed" : "_unsigned");
  return createFunction(functionName, [&]() {
    solAssert(_numBits < 256, "");
    return Whiskers(R"(
			function <functionName>(value) -> newValue {
				newValue := <div>(value, <multiplier>)
                            ...
                            functionName)("div", _signed ? "sdiv" : "div")(
               "multiplier", toCompactHexWithPrefix(u256(1) << _numBits))
        .render();
  });
}
    ...
    if (!_type.isDynamicallySized())
      ...
      case DataLocation::Storage:
        if (_type.isByteArray()) {
          ...
          // long strings it computes (x & (-1)) / 2, which is equivalent to
          // just x / 2.
          body = R"(
						length := sload(value)
...
string ABIFunctions::nextArrayElementFunction(ArrayType const &_type) {
  solAssert(!_type.isByteArray(), "");
  solAssert(_type.location() == DataLocation::Memory ||
    ...
    return Whiskers(R"(
			function <functionName>(size) -> memPtr {
				...
				mstore(<freeMemoryPointer>, newFreePtr)
                    ...
                    to_string(CompilerUtils::freeMemoryPointer))("functionName",
                                                                 functionName)
        .render();
  });
}

string ABIFunctions::createFunction(string const &_name,
