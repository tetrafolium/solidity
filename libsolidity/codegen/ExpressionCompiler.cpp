...
void ExpressionCompiler::appendStateVariableInitialization(
  ...
  if (_varDecl.annotation().type->dataStoredIn(DataLocation::Storage)) {
    // reference type, only convert value to mobile type and do final conversion
    // in storeValue.
...
void ExpressionCompiler::appendConstStateVariableAccessor(
    VariableDeclaration const &_varDecl) {
  solAssert(_varDecl.isConstant(), "");
  _varDecl.value()->accept(*this);
  utils().convertType(*_varDecl.value()->annotation().type,
...
void ExpressionCompiler::appendStateVariableAccessor(
    VariableDeclaration const &_varDecl) {
  solAssert(!_varDecl.isConstant(), "");
  CompilerContext::LocationSetter locationSetter(m_context, _varDecl);
  ...
  for (size_t i = 0; i < paramTypes.size(); ++i) {
    if (auto mappingType =
      ...
      solUnimplementedAssert(!paramTypes[i]->isDynamicallySized(),
                             "Accessors for mapping with dynamically-sized "
      ...
      solAssert(
          false,
          "Index access is allowed only for \"mapping\" and \"array\" types.");
  }
  ...
  if (StructType const *structType =
          dynamic_cast<StructType const *>(returnType.get())) {
    // remove offset
    m_context << Instruction::POP;
    ...
    for (size_t i = 0; i < names.size(); ++i) {
      if (returnTypes[i]->category() == Type::Category::Mapping)
        continue;
      if (auto arrayType =
              dynamic_cast<ArrayType const *>(returnTypes[i].get()))
        if (!arrayType->isByteArray())
          continue;
      pair<u256, unsigned> const &offsets =
          structType->storageOffsetsOfMember(names[i]);
      m_context << Instruction::DUP1 << u256(offsets.first) << Instruction::ADD
                << u256(offsets.second);
...
bool ExpressionCompiler::visit(Conditional const &_condition) {
  ...
  utils().convertType(*_condition.falseExpression().annotation().type,
                      *_condition.annotation().type);
  ...
  utils().convertType(*_condition.trueExpression().annotation().type,
                      *_condition.annotation().type);
  m_context << endTag;
  return false;
}
...
bool ExpressionCompiler::visit(Assignment const &_assignment) {
  ...
  Token::Value binOp =
      op == Token::Assign ? op : Token::AssignmentToBinaryOp(op);
  ...
  // Perform some conversion already. This will convert storage types to memory
  // and literals to their actual type, but will not convert e.g. memory to
  // storage.
  TypePointer rightIntermediateType;
  if (op != Token::Assign && Token::isShiftOp(binOp))
    rightIntermediateType =
        _assignment.rightHandSide().annotation().type->mobileType();
  else
    rightIntermediateType =
        _assignment.rightHandSide().annotation().type->closestTemporaryType(
            _assignment.leftHandSide().annotation().type);
  solAssert(rightIntermediateType, "");
  utils().convertType(*_assignment.rightHandSide().annotation().type,
                      *rightIntermediateType, cleanupNeeded);
  ...
  {
    solAssert(leftType.isValueType(),
    ...
    m_currentLValue->storeValue(*_assignment.annotation().type,
...
bool ExpressionCompiler::visit(TupleExpression const &_tuple) {
  if (_tuple.isInlineArray()) {
    ArrayType const &arrayType =
        dynamic_cast<ArrayType const &>(*_tuple.annotation().type);

    solAssert(!arrayType.isDynamicallySized(),
              "Cannot create dynamically sized inline array.");
    ...
    for (auto const &component : _tuple.components()) {
      ...
      utils().convertType(*component->annotation().type, *arrayType.baseType(),
                          true);
...
bool ExpressionCompiler::visit(UnaryOperation const &_unaryOperation) {
  ...
  if (_unaryOperation.annotation().type->category() ==
  ...
  case Token::Dec: // -- (pre- or postfix)
    ...
    if (!_unaryOperation.isPrefixOperation()) {
      ...
      solUnimplementedAssert(_unaryOperation.annotation().type->sizeOnStack() ==
                                 1,
                             "Stack size != 1 not implemented.");
      m_context << Instruction::DUP1;
    ...
    for (unsigned i = m_currentLValue->sizeOnStack(); i > 0; --i)
      m_context << swapInstruction(i);
    m_currentLValue->storeValue(*_unaryOperation.annotation().type,
                                _unaryOperation.location(),
...
bool ExpressionCompiler::visit(BinaryOperation const &_binaryOperation) {
  ...
  if (c_op == Token::And || c_op == Token::Or) // special case: short-circuiting
    ...
    TypePointer rightTargetType =
        Token::isShiftOp(c_op) ? rightExpression.annotation().type->mobileType()
                               : commonType;
    solAssert(rightTargetType, "");

    // for commutative operators, push the literal as late as possible to allow
    // improved optimization
    auto isLiteral = [](Expression const &_e) {
      return dynamic_cast<Literal const *>(&_e) ||
             _e.annotation().type->category() == Type::Category::RationalNumber;
    };
    bool swap = m_optimize && Token::isCommutativeOp(c_op) &&
                isLiteral(rightExpression) && !isLiteral(leftExpression);
    if (swap) {
      leftExpression.accept(*this);
      utils().convertType(*leftExpression.annotation().type, *leftTargetType,
                          cleanupNeeded);
      rightExpression.accept(*this);
      utils().convertType(*rightExpression.annotation().type, *rightTargetType,
                          cleanupNeeded);
    } else {
      rightExpression.accept(*this);
      utils().convertType(*rightExpression.annotation().type, *rightTargetType,
...
bool ExpressionCompiler::visit(FunctionCall const &_functionCall) {
  ...
  if (_functionCall.annotation().kind == FunctionCallKind::TypeConversion) {
    ...
    utils().convertType(*firstArgument.annotation().type,
  ...
  if (_functionCall.annotation().kind ==
    ...
    auto const &type = dynamic_cast<TypeType const &>(
        *_functionCall.expression().annotation().type);
    auto const &structType =
        dynamic_cast<StructType const &>(*type.actualType());
    functionType = structType.constructorType();
  } else
    functionType = dynamic_pointer_cast<FunctionType const>(
        _functionCall.expression().annotation().type);

  TypePointers parameterTypes = functionType->parameterTypes();
  vector<ASTPointer<Expression const>> const &callArguments =
      _functionCall.arguments();
  vector<ASTPointer<ASTString>> const &callArgumentNames =
      _functionCall.names();
  ...
  if (_functionCall.annotation().kind ==
      FunctionCallKind::StructConstructorCall) {
    TypeType const &type = dynamic_cast<TypeType const &>(
        *_functionCall.expression().annotation().type);
    auto const &structType =
        dynamic_cast<StructType const &>(*type.actualType());
    ...
    for (unsigned i = 0; i < arguments.size(); ++i) {
      ...
      utils().convertType(*arguments[i]->annotation().type,
    ...
    if (function.bound())
      // Only delegatecall and internal functions can be bound, this might be
      // lifted later.
      solAssert(function.kind() == FunctionType::Kind::DelegateCall ||
    ...
    case FunctionType::Kind::Internal: {
      ...
      unsigned parameterSize =
          CompilerUtils::sizeOnStack(function.parameterTypes());
      if (function.bound()) {
      ...
      unsigned returnParametersSize =
          CompilerUtils::sizeOnStack(function.returnParameterTypes());
    ...
    case FunctionType::Kind::Creation: {
      ...
      ContractDefinition const *contract =
          &dynamic_cast<ContractType const &>(
               *function.returnParameterTypes().front())
               .contractDefinition();
      m_context.callLowLevelFunction(
          "$copyContractCreationCodeToMemory_" + contract->type()->identifier(),
          0, 1, [contract](CompilerContext &_context) {
            ...
            // pushes size
            auto subroutine =
                _context.addSubroutine(make_shared<eth::Assembly>(assembly));
    ...
    case FunctionType::Kind::SetGas: {
      ...
      utils().convertType(*arguments.front()->annotation().type,
                          IntegerType(256), true);
      // Note that function is not the original function, but the ".gas"
      // function. Its values of gasSet and valueSet is equal to the original
      // function's though.
      unsigned stackDepth =
          (function.gasSet() ? 1 : 0) + (function.valueSet() ? 1 : 0);
      if (stackDepth > 0)
    ...
    case FunctionType::Kind::SetValue:
      // stack layout: contract_address function_id [gas] [value]
      _functionCall.expression().accept(*this);
      // Note that function is not the original function, but the ".value"
      // function. Its values of gasSet and valueSet is equal to the original
      // function's though.
      if (function.valueSet())
        m_context << Instruction::POP;
    ...
    case FunctionType::Kind::Transfer:
      _functionCall.expression().accept(*this);
      // Provide the gas stipend manually at first because we may send zero
      // ether. Will be zeroed if we send more than zero ether.
      ...
      m_context << Instruction::ISZERO << Instruction::MUL
                << Instruction::SWAP1;
      appendExternalFunctionCall(
          FunctionType(TypePointers{}, TypePointers{}, strings(), strings(),
                       FunctionType::Kind::BareCall, false,
                       StateMutability::NonPayable, nullptr, true, true),
          {});
    ...
    case FunctionType::Kind::Log4: {
      ...
      for (unsigned arg = logNumber; arg > 0; --arg) {
        ...
        utils().convertType(*arguments[arg]->annotation().type,
                            *function.parameterTypes()[arg], true);
      }
      arguments.front()->accept(*this);
      utils().fetchFreeMemoryPointer();
      utils().packedEncode({arguments.front()->annotation().type},
    ...
    case FunctionType::Kind::Event: {
      ...
      for (unsigned arg = arguments.size(); arg > 0; --arg)
        if (event.parameters()[arg - 1]->isIndexed()) {
          ...
          if (auto const &arrayType = dynamic_pointer_cast<ArrayType const>(
                  function.parameterTypes()[arg - 1])) {
            utils().fetchFreeMemoryPointer();
            utils().packedEncode({arguments[arg - 1]->annotation().type},
                                 {arrayType});
            utils().toSizeAfterFreeMemoryPointer();
            m_context << Instruction::KECCAK256;
          } else
            utils().convertType(*arguments[arg - 1]->annotation().type,
                                *function.parameterTypes()[arg - 1], true);
        }
      if (!event.isAnonymous()) {
        m_context << u256(
            h256::Arith(dev::keccak256(function.externalSignature())));
        ++numIndexed;
      }
      solAssert(numIndexed <= 4, "Too many indexed arguments.");
      // Copy all non-indexed arguments to memory (data)
      // Memory position is only a hack and should be removed once we have free
      // memory pointer.
    ...
    case FunctionType::Kind::MulMod: {
      for (unsigned i = 0; i < 3; i++) {
        ...
        utils().convertType(*arguments[2 - i]->annotation().type,
                            IntegerType(256));
      }
    ...
    case FunctionType::Kind::RIPEMD160: {
      _functionCall.expression().accept(*this);
      static const map<FunctionType::Kind, u256> contractAddresses{
    ...
    case FunctionType::Kind::ArrayPush: {
      ...
      TypePointer type = arguments[0]->annotation().type->closestTemporaryType(
          arrayType->baseType());
      solAssert(type, "");
      ...
      if (function.kind() == FunctionType::Kind::ArrayPush)
        StorageItem(m_context, *paramType)
            .storeValue(*type, _functionCall.location(), true);
      else
        StorageByteArrayElement(m_context).storeValue(
            *type, _functionCall.location(), true);
      break;
    }
    case FunctionType::Kind::ObjectCreation: {
      // Will allocate at the end of memory (MSIZE) and not write at all unless
      // the base type is dynamically sized.
      ArrayType const &arrayType =
      ...
      if (arrayType.isByteArray())
        ...
        m_context << u256(31) << Instruction::ADD << u256(31)
    ...
    case FunctionType::Kind::Require: {
      arguments.front()->accept(*this);
      utils().convertType(*arguments.front()->annotation().type,
                          *function.parameterTypes().front(), false);
...
bool ExpressionCompiler::visit(MemberAccess const &_memberAccess) {
  ...
  if (auto funType = dynamic_cast<FunctionType const *>(
          _memberAccess.annotation().type.get()))
    if (funType->bound()) {
      _memberAccess.expression().accept(*this);
      utils().convertType(*_memberAccess.expression().annotation().type,
                          *funType->selfType(), true);
      if (funType->kind() == FunctionType::Kind::Internal) {
        FunctionDefinition const &funDef =
            dynamic_cast<decltype(funDef)>(funType->declaration());
        utils().pushCombinedFunctionEntryLabel(funDef);
        utils().moveIntoStack(funType->selfType()->sizeOnStack(), 1);
      } else {
        solAssert(funType->kind() == FunctionType::Kind::DelegateCall, "");
        auto contract = dynamic_cast<ContractDefinition const *>(
            funType->declaration().scope());
  ...
  // Special processing for TypeType because we do not want to visit the library
  // itself for internal functions, or enum/struct definitions.
  if (TypeType const *type = dynamic_cast<TypeType const *>(
          _memberAccess.expression().annotation().type.get())) {
    if (dynamic_cast<ContractType const *>(type->actualType().get())) {
      ...
      if (auto funType = dynamic_cast<FunctionType const *>(
        ...
        switch (funType->kind()) {
        case FunctionType::Kind::Internal:
          // We do not visit the expression here on purpose, because in the case
          // of an internal library function call, this would push the library
          // address forcing us to link against it although we actually do not
          // need it.
          ...
          break;
        case FunctionType::Kind::Event:
          if (!dynamic_cast<EventDefinition const *>(
                  _memberAccess.annotation().referencedDeclaration))
      ...
      } else if (dynamic_cast<TypeType const *>(
                     _memberAccess.annotation().type.get())) {
        // no-op
      } else if (auto variable = dynamic_cast<VariableDeclaration const *>(
                     _memberAccess.annotation().referencedDeclaration))
        appendVariable(*variable,
                       static_cast<Expression const &>(_memberAccess));
      else
        _memberAccess.expression().accept(*this);
    } else if (auto enumType =
                   dynamic_cast<EnumType const *>(type->actualType().get())) {
      _memberAccess.expression().accept(*this);
  ...
  case Type::Category::Integer: {
    ...
    if (_memberAccess.expression().annotation().type->category() ==
        Type::Category::Contract) {
      ContractType const &type = dynamic_cast<ContractType const &>(
          *_memberAccess.expression().annotation().type);
      if (type.isSuper()) {
        solAssert(!!_memberAccess.annotation().referencedDeclaration,
                  "Referenced declaration not resolved.");
        ...
        if (Declaration const *declaration =
                _memberAccess.annotation().referencedDeclaration) {
          u256 identifier;
          ...
          utils().convertType(
              type, IntegerType(160, IntegerType::Modifier::Address), true);
  ...
  case Type::Category::Struct: {
    StructType const &type = dynamic_cast<StructType const &>(
        *_memberAccess.expression().annotation().type);
  ...
  case Type::Category::Enum: {
    EnumType const &type = dynamic_cast<EnumType const &>(
        *_memberAccess.expression().annotation().type);
    m_context << type.memberValue(_memberAccess.memberName());
    break;
  }
  case Type::Category::Array: {
    auto const &type = dynamic_cast<ArrayType const &>(
        *_memberAccess.expression().annotation().type);
    if (member == "length") {
  ...
  case Type::Category::FixedBytes: {
    auto const &type = dynamic_cast<FixedBytesType const &>(
        *_memberAccess.expression().annotation().type);
    utils().popStackElement(type);
    if (member == "length")
...
bool ExpressionCompiler::visit(IndexAccess const &_indexAccess) {
  ...
  if (baseType.category() == Type::Category::Mapping) {
    ...
    _indexAccess.indexExpression()->accept(*this);
    utils().convertType(*_indexAccess.indexExpression()->annotation().type,
                        IntegerType(256), true);
    ...
    case DataLocation::Storage:
      if (arrayType.isByteArray()) {
        solAssert(!arrayType.isString(),
                  "Index access to string is not allowed.");
        setLValue<StorageByteArrayElement>(_indexAccess);
      } else
        setLValueToStorageItem(_indexAccess);
      break;
    case DataLocation::Memory:
      setLValue<MemoryItem>(_indexAccess, *_indexAccess.annotation().type,
                            !arrayType.isByteArray());
      break;
    case DataLocation::CallData:
      //@todo if we implement this, the value in calldata has to be added to the
      //base offset
      solUnimplementedAssert(!arrayType.baseType()->isDynamicallySized(),
                             "Nested arrays not yet implemented.");
      if (arrayType.baseType()->isValueType())
        CompilerUtils(m_context).loadFromMemoryDynamic(
            *arrayType.baseType(), true, !arrayType.isByteArray(), false);
      break;
    }
  } else if (baseType.category() == Type::Category::FixedBytes) {
    FixedBytesType const &fixedBytesType =
...
void ExpressionCompiler::endVisit(Identifier const &_identifier) {
  ...
  if (MagicVariableDeclaration const *magicVar =
    ...
    utils().pushCombinedFunctionEntryLabel(
        m_context.resolveVirtualFunction(*functionDef));
  else if (auto variable =
               dynamic_cast<VariableDeclaration const *>(declaration))
    appendVariable(*variable, static_cast<Expression const &>(_identifier));
  else if (auto contract =
               dynamic_cast<ContractDefinition const *>(declaration)) {
...
void ExpressionCompiler::endVisit(Literal const &_literal) {
  ...
  default:
    solUnimplemented(
        "Only integer, boolean and string literals implemented for now.");
  }
}

void ExpressionCompiler::appendAndOrOperatorCode(
    BinaryOperation const &_binaryOperation) {
  Token::Value const c_op = _binaryOperation.getOperator();
...
void ExpressionCompiler::appendCompareOperatorCode(Token::Value _operator,
                                                   Type const &_type) {
  ...
  if (_operator == Token::Equal || _operator == Token::NotEqual) {
    if (FunctionType const *funType = dynamic_cast<decltype(funType)>(&_type)) {
      if (funType->kind() == FunctionType::Kind::Internal) {
        // We have to remove the upper bits (construction time value) because
        // they might be "unknown" in one of the operands and not in the other.
        m_context << ((u256(1) << 32) - 1) << Instruction::AND;
        m_context << Instruction::SWAP1;
        m_context << ((u256(1) << 32) - 1) << Instruction::AND;
      }
...
void ExpressionCompiler::appendOrdinaryBinaryOperatorCode(
    Token::Value _operator, Type const &_type) {
...
void ExpressionCompiler::appendArithmeticOperatorCode(Token::Value _operator,
                                                      Type const &_type) {
...
void ExpressionCompiler::appendShiftOperatorCode(Token::Value _operator,
  ...
  if (auto valueType = dynamic_cast<IntegerType const *>(&_valueType))
    ...
    solAssert(dynamic_cast<FixedBytesType const *>(&_valueType),
              "Only integer and fixed bytes type supported for shifts.");

  // The amount can be a RationalNumberType too.
  bool c_amountSigned = false;
  if (auto amountType =
          dynamic_cast<RationalNumberType const *>(&_shiftAmountType)) {
    // This should be handled by the type checker.
    solAssert(amountType->integerType(), "");
    solAssert(!amountType->integerType()->isSigned(), "");
  } else if (auto amountType =
                 dynamic_cast<IntegerType const *>(&_shiftAmountType))
  ...
  switch (_operator) {
  case Token::SHL:
    m_context << Instruction::SWAP1 << u256(2) << Instruction::EXP
              << Instruction::MUL;
    break;
  case Token::SAR:
    m_context << Instruction::SWAP1 << u256(2) << Instruction::EXP
              << Instruction::SWAP1
              << (c_valueSigned ? Instruction::SDIV : Instruction::DIV);
    break;
  case Token::SHR:
  default:
...
void ExpressionCompiler::appendExternalFunctionCall(
  ...
  unsigned selfSize =
      _functionType.bound() ? _functionType.selfType()->sizeOnStack() : 0;
  unsigned gasValueSize =
      (_functionType.gasSet() ? 1 : 0) + (_functionType.valueSet() ? 1 : 0);
  unsigned contractStackPos = m_context.currentToBaseStackOffset(
      1 + gasValueSize + selfSize + (_functionType.isBareCall() ? 0 : 1));
  unsigned gasStackPos = m_context.currentToBaseStackOffset(gasValueSize);
  unsigned valueStackPos = m_context.currentToBaseStackOffset(1);

  // move self object to top
  if (_functionType.bound())
    utils().moveToStackTop(gasValueSize,
                           _functionType.selfType()->sizeOnStack());

  auto funKind = _functionType.kind();
  bool returnSuccessCondition = funKind == FunctionType::Kind::BareCall ||
                                funKind == FunctionType::Kind::BareCallCode ||
                                funKind == FunctionType::Kind::BareDelegateCall;
  bool isCallCode = funKind == FunctionType::Kind::BareCallCode ||
                    funKind == FunctionType::Kind::CallCode;
  bool isDelegateCall = funKind == FunctionType::Kind::BareDelegateCall ||
                        funKind == FunctionType::Kind::DelegateCall;
  ...
  if (returnSuccessCondition)
    ...
    for (auto const &retType : _functionType.returnParameterTypes()) {
      solAssert(!retType->isDynamicallySized(),
                "Unable to return dynamic type from external call.");
  ...
  if ((funKind == FunctionType::Kind::BareCall ||
    ...
    manualFunctionId =
        _arguments.front()
            ->annotation()
            .type->mobileType()
            ->calldataEncodedSize(false) == CompilerUtils::dataStartOffset;
  }
  if (manualFunctionId) {
    // If we have a Bare* and the first type has exactly 4 bytes, use it as
    // function identifier.
    _arguments.front()->accept(*this);
    utils().convertType(*_arguments.front()->annotation().type,
                        IntegerType(8 * CompilerUtils::dataStartOffset), true);
  ...
  if (funKind == FunctionType::Kind::ECRecover) {
    // Clears 32 bytes of currently free memory and advances free memory
    // pointer. Output area will be "start of input area" - 32. The reason is
    // that a failing ECRecover cannot be detected, it will just return zero
    // bytes (which we cannot detect).
  ...
  // Touch the end of the output area so that we do not pay for memory resize
  // during the call (which we would have to subtract from the gas left) We
  // could also just use MLOAD; POP right before the gas calculation, but the
  // optimizer would remove that, so we use MSTORE here.
  ...
  if (!_functionType.isBareCall() || manualFunctionId) {
    m_context << dupInstruction(2 + gasValueSize +
                                CompilerUtils::sizeOnStack(argumentTypes));
    utils().storeInMemoryDynamic(
        IntegerType(8 * CompilerUtils::dataStartOffset), false);
  }
  // If the function takes arbitrary parameters, copy dynamic length data in
  // place. Move arguments to memory, will not update the free memory pointer
  // (but will update the memory pointer on the stack).
  utils().encodeToMemory(
      argumentTypes, parameterTypes, _functionType.padArguments(),
      _functionType.takesArbitraryParameters(), isCallCode || isDelegateCall);
  ...
  // put on stack: <size of output> <memory pos of output> <size of input>
  // <memory pos of input>
  m_context << u256(retSize);
  utils().fetchFreeMemoryPointer(); // This is the start of input
  if (funKind == FunctionType::Kind::ECRecover) {
    // In this case, output is 32 bytes before input and has already been
    // cleared.
    m_context << u256(32) << Instruction::DUP2 << Instruction::SUB
              << Instruction::SWAP1;
  ...
  if (isDelegateCall)
    ...
    m_context << dupInstruction(
        m_context.baseToCurrentStackOffset(valueStackPos));
  else
    m_context << u256(0);
  m_context << dupInstruction(
      m_context.baseToCurrentStackOffset(contractStackPos));

  bool existenceChecked = false;
  // Check the the target contract exists (has code) for non-low-level calls.
  if (funKind == FunctionType::Kind::External ||
      funKind == FunctionType::Kind::CallCode ||
      funKind == FunctionType::Kind::DelegateCall) {
  ...
  if (_functionType.gasSet())
    m_context << dupInstruction(
        m_context.baseToCurrentStackOffset(gasStackPos));
  ...
  unsigned remainsSize = 2 + // contract address, input_memory_end
                         _functionType.valueSet() + _functionType.gasSet() +
...
void ExpressionCompiler::appendExpressionCopyToMemory(
    Type const &_expectedType, Expression const &_expression) {
  solUnimplementedAssert(_expectedType.isValueType(),
                         "Not implemented for non-value types.");
  _expression.accept(*this);
  utils().convertType(*_expression.annotation().type, _expectedType, true);
  utils().storeInMemoryDynamic(_expectedType);
}

void ExpressionCompiler::appendVariable(VariableDeclaration const &_variable,
                                        Expression const &_expression) {
  if (!_variable.isConstant())
    setLValueFromDeclaration(_variable, _expression);
  else {
    _variable.value()->accept(*this);
    utils().convertType(*_variable.value()->annotation().type,
                        *_variable.annotation().type);
  }
}

void ExpressionCompiler::setLValueFromDeclaration(
    Declaration const &_declaration, Expression const &_expression) {
  if (m_context.isLocalVariable(&_declaration))
    setLValue<StackVariable>(
        _expression, dynamic_cast<VariableDeclaration const &>(_declaration));
  else if (m_context.isStateVariable(&_declaration))
    setLValue<StorageItem>(
        _expression, dynamic_cast<VariableDeclaration const &>(_declaration));
  else
    BOOST_THROW_EXCEPTION(
        ...
        << errinfo_comment(
               "Identifier type not supported or identifier not found."));
}
...
bool ExpressionCompiler::cleanupNeededForOp(Type::Category _type,
                                            Token::Value _op) {
  if (Token::isCompareOp(_op) || Token::isShiftOp(_op))
    return true;
  else if (_type == Type::Category::Integer &&
           (_op == Token::Div || _op == Token::Mod))
    return true;
  else
    return false;
}

CompilerUtils ExpressionCompiler::utils() { return CompilerUtils(m_context); }
