...
StackVariable::StackVariable(CompilerContext &_compilerContext,
...
void StackVariable::retrieveValue(SourceLocation const &_location, bool) const {
  ...
  if (stackPos + 1 >
      16) //@todo correct this by fetching earlier or moving to memory
...
void StackVariable::storeValue(Type const &, SourceLocation const &_location,
                               bool _move) const {
  unsigned stackDiff =
      m_context.baseToCurrentStackOffset(m_baseStackOffset) - m_size + 1;
  if (stackDiff > 16)
    BOOST_THROW_EXCEPTION(
        CompilerError()
        << errinfo_sourceLocation(_location)
...
MemoryItem::MemoryItem(CompilerContext &_compilerContext, Type const &_type,
                       bool _padded)
...
void MemoryItem::retrieveValue(SourceLocation const &, bool _remove) const {
  if (m_dataType->isValueType()) {
    ...
    CompilerUtils(m_context).loadFromMemoryDynamic(*m_dataType, false, m_padded,
                                                   false);
  } else
    m_context << Instruction::MLOAD;
}

void MemoryItem::storeValue(Type const &_sourceType, SourceLocation const &,
                            bool _move) const {
  ...
  if (m_dataType->isValueType()) {
    ...
    if (!_move) {
      ...
      utils.copyToStackTop(1 + m_dataType->sizeOnStack(),
                           m_dataType->sizeOnStack());
    }
    if (!m_padded) {
      solAssert(m_dataType->calldataEncodedSize(false) == 1,
                "Invalid non-padded type.");
...
void MemoryItem::setToZero(SourceLocation const &,
                           bool _removeReference) const {
  CompilerUtils utils(m_context);
  if (!_removeReference)
    m_context << Instruction::DUP1;
...
StorageItem::StorageItem(CompilerContext &_compilerContext,
                         VariableDeclaration const &_declaration)
...
void StorageItem::retrieveValue(SourceLocation const &, bool _remove) const {
  ...
  if (m_dataType->storageBytes() == 32)
    ...
    m_context << Instruction::SWAP1 << Instruction::SLOAD << Instruction::SWAP1
              << u256(0x100) << Instruction::EXP << Instruction::SWAP1
              << Instruction::DIV;
    if (m_dataType->category() == Type::Category::FixedPoint)
      // implementation should be very similar to the integer case.
      solUnimplemented("Not yet implemented - FixedPointType.");
    if (m_dataType->category() == Type::Category::FixedBytes) {
      CompilerUtils(m_context).leftShiftNumberOnStack(
          256 - 8 * m_dataType->storageBytes());
      cleaned = true;
    } else if (m_dataType->category() == Type::Category::Integer &&
               dynamic_cast<IntegerType const &>(*m_dataType).isSigned()) {
      m_context << u256(m_dataType->storageBytes() - 1)
                << Instruction::SIGNEXTEND;
      cleaned = true;
    } else if (FunctionType const *fun =
                   dynamic_cast<decltype(fun)>(m_dataType)) {
    ...
    if (!cleaned) {
      solAssert(m_dataType->sizeOnStack() == 1, "");
      m_context << ((u256(0x1) << (8 * m_dataType->storageBytes())) - 1)
                << Instruction::AND;
    }
  }
}

void StorageItem::storeValue(Type const &_sourceType,
  ...
  if (m_dataType->isValueType()) {
    ...
    if (m_dataType->storageBytes() == 32) {
      ...
      utils.copyToStackTop(3 + m_dataType->sizeOnStack(),
                           m_dataType->sizeOnStack());
      // stack: value storage_ref cleared_value multiplier value
      if (FunctionType const *fun = dynamic_cast<decltype(fun)>(m_dataType)) {
        solAssert(_sourceType == *m_dataType,
        ...
        CompilerUtils(m_context).rightShiftNumberOnStack(
            256 - 8 * dynamic_cast<FixedBytesType const &>(*m_dataType)
                          .numBytes(),
            false);
      } else {
        solAssert(m_dataType->sizeOnStack() == 1,
                  "Invalid stack size for opaque type.");
    ...
    solAssert(_sourceType.category() == m_dataType->category(),
    ...
    if (m_dataType->category() == Type::Category::Array) {
      ...
      solAssert(structType.structDefinition() == sourceType.structDefinition(),
                "Struct assignment with conversion.");
      solAssert(sourceType.location() != DataLocation::CallData,
      ...
      for (auto const &member : structType.members(nullptr)) {
        ...
        if (sourceType.location() == DataLocation::Storage) {
          ...
          StorageItem(m_context, *sourceMemberType)
              .retrieveValue(_location, true);
          ...
          MemoryItem(m_context, *sourceMemberType)
              .retrieveValue(_location, true);
        ...
        m_context << dupInstruction(1 + stackSize) << offsets.first
                  << Instruction::ADD;
        m_context << u256(offsets.second);
        // stack: source_ref target_ref target_off source_value...
        // target_member_ref target_member_byte_off
        StorageItem(m_context, *memberType)
            .storeValue(*sourceMemberType, _location, true);
      }
      // stack layout: source_ref target_ref
...
void StorageItem::setToZero(SourceLocation const &,
                            bool _removeReference) const {
  if (m_dataType->category() == Type::Category::Array) {
    if (!_removeReference)
      CompilerUtils(m_context).copyToStackTop(sizeOnStack(), sizeOnStack());
    ArrayUtils(m_context).clearArray(
        dynamic_cast<ArrayType const &>(*m_dataType));
  } else if (m_dataType->category() == Type::Category::Struct) {
    // stack layout: storage_key storage_offset
    // @todo this can be improved: use StorageItem for non-value types, and just
    // store 0 in all slots that contain value types later.
    auto const &structType = dynamic_cast<StructType const &>(*m_dataType);
    for (auto const &member : structType.members(nullptr)) {
    ...
    solAssert(m_dataType->isValueType(),
              "Clearing of unsupported type requested: " +
                  m_dataType->toString());
    if (!_removeReference)
      CompilerUtils(m_context).copyToStackTop(sizeOnStack(), sizeOnStack());
    if (m_dataType->storageBytes() == 32) {
      // offset should be zero
      m_context << Instruction::POP << u256(0) << Instruction::SWAP1
                << Instruction::SSTORE;
    } else {
      m_context << u256(0x100) << Instruction::EXP;
...
StorageByteArrayElement::StorageByteArrayElement(
    CompilerContext &_compilerContext)
    : LValue(_compilerContext, &byteType) {}

void StorageByteArrayElement::retrieveValue(SourceLocation const &,
                                            bool _remove) const {
...
void StorageByteArrayElement::storeValue(Type const &, SourceLocation const &,
                                         bool _move) const {
...
void StorageByteArrayElement::setToZero(SourceLocation const &,
                                        bool _removeReference) const {
  // stack: ref byte_number
  if (!_removeReference)
...
StorageArrayLength::StorageArrayLength(CompilerContext &_compilerContext,
                                       const ArrayType &_arrayType)
    : LValue(_compilerContext, _arrayType.memberType("length").get()),
      m_arrayType(_arrayType) {
  solAssert(m_arrayType.isDynamicallySized(), "");
}

void StorageArrayLength::retrieveValue(SourceLocation const &,
                                       bool _remove) const {
...
void StorageArrayLength::storeValue(Type const &, SourceLocation const &,
                                    bool _move) const {
  if (_move)
    m_context << Instruction::SWAP1;
  else
...
void StorageArrayLength::setToZero(SourceLocation const &,
                                   bool _removeReference) const {
  if (!_removeReference)
    m_context << Instruction::DUP1;
...
TupleObject::TupleObject(CompilerContext &_compilerContext,
...
void TupleObject::retrieveValue(SourceLocation const &_location,
                                bool _remove) const {
    ...
    if (lv) {
      ...
      if (lv->sizeOnStack() > 0) {
        if (_remove && depth > lv->sizeOnStack())
          CompilerUtils(m_context).moveToStackTop(depth,
                                                  depth - lv->sizeOnStack());
...
void TupleObject::storeValue(Type const &_sourceType,
                             SourceLocation const &_location, bool) const {
  // values are below the lvalue references
  unsigned valuePos = sizeOnStack();
  TypePointers const &valueTypes =
      dynamic_cast<TupleType const &>(_sourceType).components();
  ...
  for (size_t i = 0; i < m_lvalues.size(); ++i) {
    ...
    CompilerUtils(m_context).moveToStackTop(valType->sizeOnStack(),
                                            lvalue->sizeOnStack());
    lvalue->storeValue(*valType, _location, true);
    valuePos += m_context.stackHeight() - stackHeight;
  }
  // As the type of an assignment to a tuple type is the empty tuple, we always
  // move.
  CompilerUtils(m_context).popStackElement(_sourceType);
}

void TupleObject::setToZero(SourceLocation const &_location,
                            bool _removeReference) const {
  if (_removeReference) {
    for (size_t i = 0; i < m_lvalues.size(); ++i)
