...
/**
 * Simple helper class to ensure that the stack height is the same at certain
 * places in the code.
 */
...
public:
  ...
  void check() {
    solAssert(m_context.stackHeight() == stackHeight,
              std::string("I sense a disturbance in the stack: ") +
                  std::to_string(m_context.stackHeight()) + " vs " +
                  std::to_string(stackHeight));
  }
...
size_t ContractCompiler::compileClone(
  ...
  m_context << Instruction::DUP1 << runtimeSub << u256(0)
...
void ContractCompiler::initializeContext(
    ContractDefinition const &_contract,
    map<ContractDefinition const *, eth::Assembly const *> const
        &_compiledContracts) {
  m_context.setExperimentalFeatures(
      _contract.sourceUnit().annotation().experimentalFeatures);
  m_context.setCompiledContracts(_compiledContracts);
  m_context.setInheritanceHierarchy(
...
void ContractCompiler::appendInitAndConstructorCode(
    ContractDefinition const &_contract) {
  CompilerContext::LocationSetter locationSetter(m_context, _contract);
  // Determine the arguments that are used for the base constructors.
  std::vector<ContractDefinition const *> const &bases =
      _contract.annotation().linearizedBaseContracts;
      ...
      for (auto const &modifier : constructor->modifiers()) {
        ...
        if (baseContract)
          if (m_baseArguments.count(baseContract->constructor()) == 0)
            m_baseArguments[baseContract->constructor()] =
                &modifier->arguments();
      }
...
size_t
ContractCompiler::packIntoContractCreator(ContractDefinition const &_contract) {
  solAssert(!!m_runtimeCompiler, "");

  appendInitAndConstructorCode(_contract);

  // We jump to the deploy routine because we first have to append all missing
  // functions, which can cause further functions to be added to the runtime
  // context.
  eth::AssemblyItem deployRoutine = m_context.appendJumpToNew();

  // We have to include copies of functions in the construction time and runtime
  // context because of absolute jumps.
  appendMissingFunctions();
...
void ContractCompiler::appendBaseConstructor(
  ...
  if (!constructorType.parameterTypes().empty()) {
    ...
    std::vector<ASTPointer<Expression>> const *arguments =
        m_baseArguments[&_constructor];
    solAssert(arguments, "");
    for (unsigned i = 0; i < arguments->size(); ++i)
      compileExpression(*(arguments->at(i)),
                        constructorType.parameterTypes()[i]);
  }
  _constructor.accept(*this);
}

void ContractCompiler::appendConstructor(
    FunctionDefinition const &_constructor) {
  CompilerContext::LocationSetter locationSetter(m_context, _constructor);
  if (!_constructor.isPayable())
    appendCallValueCheck();

  // copy constructor arguments from code to memory and then to stack, they are
  // supplied after the actual program
...
void ContractCompiler::appendFunctionSelector(
    ContractDefinition const &_contract) {
  map<FixedHash<4>, FunctionTypePointer> interfaceFunctions =
      _contract.interfaceFunctions();
  map<FixedHash<4>, const eth::AssemblyItem> callDataUnpackerEntryPoints;

  FunctionDefinition const *fallback = _contract.fallbackFunction();
  eth::AssemblyItem notFound = m_context.newTag();
  // directly jump to fallback if the data is too short to contain a function
  ...
  if (!interfaceFunctions.empty())
    CompilerUtils(m_context).loadFromMemory(
        0, IntegerType(CompilerUtils::dataStartOffset * 8), true);

  // stack now is: 1 0 <funhash>
  for (auto const &it : interfaceFunctions) {
    callDataUnpackerEntryPoints.insert(
        std::make_pair(it.first, m_context.newTag()));
    m_context << dupInstruction(1) << u256(FixedHash<4>::Arith(it.first))
              << Instruction::EQ;
  ...
  for (auto const &it : interfaceFunctions) {
    ...
    CompilerContext::LocationSetter locationSetter(m_context,
    ...
    m_context.adjustStackOffset(
        CompilerUtils(m_context).sizeOnStack(
...
void ContractCompiler::appendCalldataUnpacker(
    TypePointers const &_typeParameters, bool _fromMemory) {
  ...
  if (m_context.experimentalFeatureActive(ExperimentalFeature::ABIEncoderV2)) {
    ...
    solAssert(m_context.stackHeight() - stackHeightBefore ==
                  CompilerUtils(m_context).sizeOnStack(_typeParameters) - 1,
              "");
    return;
  }
  ...
  for (TypePointer const &parameterType : _typeParameters) {
    ...
    if (type->category() == Type::Category::Array) {
      ...
      if (_fromMemory) {
        ...
        // @todo If base type is an array or struct, it is still calldata-style
        // encoded, so we would have to convert it like below.
        ...
        if (arrayType.isDynamicallySized()) {
          ...
          m_context << u256(arrayType.calldataEncodedSize(true))
                    << Instruction::ADD;
        }
      } else {
        // first load from calldata and potentially convert to memory if
        // arrayType is memory
        TypePointer calldataType =
            arrayType.copyForLocation(DataLocation::CallData, false);
        if (calldataType->isDynamicallySized()) {
          // put on stack: data_pointer length
          CompilerUtils(m_context).loadFromMemoryDynamic(IntegerType(256),
                                                         !_fromMemory);
          // stack: base_offset data_offset next_pointer
          m_context << Instruction::SWAP1 << Instruction::DUP3
                    << Instruction::ADD;
        ...
        if (arrayType.location() == DataLocation::Memory) {
          ...
          CompilerUtils(m_context).convertType(*calldataType, arrayType, false,
                                               false, true);
      ...
      solAssert(!type->isDynamicallySized(),
                "Unknown dynamically sized type: " + type->toString());
...
void ContractCompiler::appendReturnValuePacker(
    TypePointers const &_typeParameters, bool _isLibrary) {
  CompilerUtils utils(m_context);
  if (_typeParameters.empty())
    m_context << Instruction::STOP;
  else {
    utils.fetchFreeMemoryPointer();
    //@todo optimization: if we return a single memory array, there should be
    //enough space before
...
void ContractCompiler::registerStateVariables(
    ContractDefinition const &_contract) {
  for (auto const &var : ContractType(_contract).stateVariables())
    m_context.addStateVariable(*get<0>(var), get<1>(var), get<2>(var));
}

void ContractCompiler::initializeStateVariables(
    ContractDefinition const &_contract) {
  for (VariableDeclaration const *variable : _contract.stateVariables())
    if (variable->value() && !variable->isConstant())
      ExpressionCompiler(m_context, m_optimise)
          .appendStateVariableInitialization(*variable);
}

bool ContractCompiler::visit(VariableDeclaration const &_variableDeclaration) {
  solAssert(_variableDeclaration.isStateVariable(),
...
bool ContractCompiler::visit(FunctionDefinition const &_function) {
  ...
  // reserve additional slots: [retarg0] ... [retargm] [localvar0] ...
  // [localvarp]
  ...
  for (ASTPointer<VariableDeclaration const> const &variable :
  ...
  if (_function.isConstructor())
    if (auto c = m_context.nextConstructor(
  ...
  // Now we need to re-shuffle the stack. For this we keep a record of the stack
  // layout that shows the target positions of the elements, where "-1" denotes
  // that this element needs to be removed from the stack. Note that the fact
  // that the return arguments are of increasing index is vital for this
  // algorithm to work.

  unsigned const c_argumentsSize =
      CompilerUtils::sizeOnStack(_function.parameters());
  unsigned const c_returnValuesSize =
      CompilerUtils::sizeOnStack(_function.returnParameters());
  unsigned const c_localVariablesSize =
      CompilerUtils::sizeOnStack(_function.localVariables());
...
bool ContractCompiler::visit(InlineAssembly const &_inlineAssembly) {
  ...
  identifierAccess.resolve = [&](assembly::Identifier const &_identifier,
                                 julia::IdentifierContext, bool) {
    auto ref =
  ...
  identifierAccess.generateCode = [&](assembly::Identifier const &_identifier,
    ...
    if (_context == julia::IdentifierContext::RValue) {
      ...
      if (FunctionDefinition const *functionDef =
        ...
        auto functionEntryLabel =
            m_context.functionEntryLabel(*functionDef).pushTag();
        solAssert(functionEntryLabel.data() <=
                      std::numeric_limits<size_t>::max(),
                  "");
        _assembly.appendLabelReference(size_t(functionEntryLabel.data()));
        // If there is a runtime context, we have to merge both labels into the
        // same stack slot in case we store it in storage.
        if (CompilerContext *rtc = m_context.runtimeContext()) {
          _assembly.appendConstant(u256(1) << 32);
          _assembly.appendInstruction(Instruction::MUL);
          auto runtimeEntryLabel =
              rtc->functionEntryLabel(*functionDef)
                  .toSubAssemblyTag(m_context.runtimeSub());
          solAssert(runtimeEntryLabel.data() <=
                        std::numeric_limits<size_t>::max(),
                    "");
        ...
        if (m_context.isStateVariable(decl)) {
          ...
          int stackDiff = _assembly.stackHeight() -
      ...
      solAssert(_assembly.stackHeight() - depositBefore ==
                    int(ref->second.valueSize),
                "");
    } else {
      // lvalue context
      solAssert(!ref->second.isOffset && !ref->second.isSlot, "");
      auto variable = dynamic_cast<VariableDeclaration const *>(decl);
      solAssert(!!variable && m_context.isLocalVariable(variable),
      ...
      if (stackDiff > 16 || stackDiff < 1)
        BOOST_THROW_EXCEPTION(
            CompilerError()
            << errinfo_sourceLocation(_inlineAssembly.location())
            << errinfo_comment("Stack too deep(" + to_string(stackDiff) +
  ...
  assembly::CodeGenerator::assemble(
      _inlineAssembly.operations(), *_inlineAssembly.annotation().analysisInfo,
      m_context.nonConstAssembly(), identifierAccess);
...
bool ContractCompiler::visit(Return const &_return) {
  ...
  if (Expression const *expression = _return.expression()) {
    solAssert(_return.annotation().functionReturnParameters,
...
bool ContractCompiler::visit(
    PlaceholderStatement const &_placeholderStatement) {
  StackHeightChecker checker(m_context);
  CompilerContext::LocationSetter locationSetter(m_context,
...
void ContractCompiler::appendMissingFunctions() {
  while (Declaration const *function = m_context.nextFunctionToCompile()) {
    ...
    solAssert(m_context.nextFunctionToCompile() != function,
              "Compiled the wrong function?");
  }
  ...
  if (m_modifierDepth >= m_currentFunction->modifiers().size()) {
    ...
    if (dynamic_cast<ContractDefinition const *>(
      ...
      ModifierDefinition const &modifier =
          m_context.functionModifier(modifierInvocation->name()->name());
      CompilerContext::LocationSetter locationSetter(m_context, modifier);
      solAssert(modifier.parameters().size() ==
                    modifierInvocation->arguments().size(),
                "");
      for (unsigned i = 0; i < modifier.parameters().size(); ++i) {
        m_context.addVariable(*modifier.parameters()[i]);
        addedVariables.push_back(modifier.parameters()[i].get());
        compileExpression(*modifierInvocation->arguments()[i],
                          modifier.parameters()[i]->annotation().type);
      }
      for (VariableDeclaration const *localVariable :
           modifier.localVariables()) {
...
void ContractCompiler::compileExpression(Expression const &_expression,
                                         TypePointer const &_targetType) {
  ExpressionCompiler expressionCompiler(m_context, m_optimise);
  expressionCompiler.compile(_expression);
  if (_targetType)
    CompilerUtils(m_context).convertType(*_expression.annotation().type,
                                         *_targetType);
}
...
eth::AssemblyPointer ContractCompiler::cloneRuntime() {
  ...
  a << u256(eth::GasCosts::callGas + 10) << Instruction::GAS
    << Instruction::SUB;
