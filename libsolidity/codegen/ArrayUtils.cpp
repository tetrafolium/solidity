...
void ArrayUtils::copyArrayToStorage(ArrayType const &_targetType,
                                    ArrayType const &_sourceType) const {
  ...
  TypePointer targetBaseType =
      _targetType.isByteArray() ? uint256 : _targetType.baseType();
  TypePointer sourceBaseType =
      _sourceType.isByteArray() ? uint256 : _sourceType.baseType();
  ...
  bool directCopy = sourceIsStorage && sourceBaseType->isValueType() &&
                    *sourceBaseType == *targetBaseType;
  bool haveByteOffsetSource =
      !directCopy && sourceIsStorage && sourceBaseType->storageBytes() <= 16;
  bool haveByteOffsetTarget =
      !directCopy && targetBaseType->storageBytes() <= 16;
  unsigned byteOffsetSize =
      (haveByteOffsetSource ? 1 : 0) + (haveByteOffsetTarget ? 1 : 0);
  ...
  if (_sourceType.location() != DataLocation::CallData ||
      !_sourceType.isDynamicallySized())
  ...
  m_context.callLowLevelFunction(
      "$copyArrayToStorage_" + sourceType->identifier() + "_to_" +
          targetType->identifier(),
      3, 1, [=](CompilerContext &_context) {
        ArrayUtils utils(_context);
        ArrayType const &_sourceType =
            dynamic_cast<ArrayType const &>(*sourceType);
        ArrayType const &_targetType =
            dynamic_cast<ArrayType const &>(*targetType);
        // stack: target_ref source_ref source_length
        ...
        if (_targetType.isDynamicallySized())
          ...
          if (!_targetType.isByteArray())
            ...
            _context << Instruction::DUP3 << Instruction::DUP3
                     << Instruction::SSTORE;
        ...
        // stack: target_ref source_ref source_length target_length
        // target_data_pos
        _context << Instruction::SWAP1;
        utils.convertLengthToSize(_targetType);
        _context << Instruction::DUP2 << Instruction::ADD;
        // stack: target_ref source_ref source_length target_data_pos
        // target_data_end
        _context << Instruction::SWAP3;
        // stack: target_ref target_data_end source_length target_data_pos
        // source_ref

        eth::AssemblyItem copyLoopEndWithoutByteOffset = _context.newTag();

        // special case for short byte arrays: Store them together with their
        // length.
        if (_targetType.isByteArray()) {
          // stack: target_ref target_data_end source_length target_data_pos
          // source_ref
          ...
          if (_sourceType.location() == DataLocation::Storage) {
            ...
            CompilerUtils(_context).loadFromMemoryDynamic(
                *sourceBaseType, fromCalldata, true, false);
            // stack: target_ref target_data_end source_length target_data_pos
            // source_ref value clear the lower-order byte - which will hold the
            // length
            _context << u256(0xff) << Instruction::NOT << Instruction::AND;
            // fetch the length and shift it left by one
            _context << Instruction::DUP4 << Instruction::DUP1
                     << Instruction::ADD;
            // combine value and length and store them
            _context << Instruction::OR << Instruction::DUP6
                     << Instruction::SSTORE;
          }
          ...
          _context << Instruction::DUP3 << Instruction::DUP1
                   << Instruction::ADD;
        ...
        if (sourceBaseType->category() == Type::Category::Array) {
          ...
          // Note that we have to copy each element on its own in case
          // conversion is involved. We might copy too much if there is padding
          // at the last element, but this way end checking is easier. stack:
          // target_ref target_data_end source_data_pos target_data_pos
          // source_data_end [target_byte_offset] [source_byte_offset]
          _context << dupInstruction(3 + byteOffsetSize);
          if (_sourceType.location() == DataLocation::Storage) {
            if (haveByteOffsetSource)
              ...
              _context << u256(0);
            StorageItem(_context, *sourceBaseType)
                .retrieveValue(SourceLocation(), true);
          } else if (sourceBaseType->isValueType())
            CompilerUtils(_context).loadFromMemoryDynamic(
                *sourceBaseType, fromCalldata, true, false);
          else
            solUnimplemented("Copying of type " + _sourceType.toString(false) +
                             " to storage not yet supported.");
          // stack: target_ref target_data_end source_data_pos target_data_pos
          // source_data_end [target_byte_offset] [source_byte_offset]
          // <source_value>...
          solAssert(2 + byteOffsetSize + sourceBaseType->sizeOnStack() <= 16,
                    "Stack too deep, try removing local variables.");
          // fetch target storage reference
          _context << dupInstruction(2 + byteOffsetSize +
                                     sourceBaseType->sizeOnStack());
          if (haveByteOffsetTarget)
            _context << dupInstruction(1 + byteOffsetSize +
                                       sourceBaseType->sizeOnStack());
          else
            _context << u256(0);
          StorageItem(_context, *targetBaseType)
              .storeValue(*sourceBaseType, SourceLocation(), true);
        }
        // stack: target_ref target_data_end source_data_pos target_data_pos
        // source_data_end [target_byte_offset] [source_byte_offset] increment
        // source
        if (haveByteOffsetSource)
          utils.incrementByteOffset(sourceBaseType->storageBytes(), 1,
                                    haveByteOffsetTarget ? 5 : 4);
        else {
          ...
          _context << Instruction::ADD << swapInstruction(2 + byteOffsetSize);
        }
        // increment target
        if (haveByteOffsetTarget)
          utils.incrementByteOffset(targetBaseType->storageBytes(),
                                    byteOffsetSize, byteOffsetSize + 2);
        else
          _context << swapInstruction(1 + byteOffsetSize)
                   << targetBaseType->storageSize() << Instruction::ADD
                   << swapInstruction(1 + byteOffsetSize);
        _context.appendJumpTo(copyLoopStart);
        _context << copyLoopEnd;
        if (haveByteOffsetTarget) {
          // clear elements that might be left over in the current slot in
          // target stack: target_ref target_data_end source_data_pos
          // target_data_pos source_data_end target_byte_offset
          // [source_byte_offset]
          _context << dupInstruction(byteOffsetSize) << Instruction::ISZERO;
          eth::AssemblyItem copyCleanupLoopEnd =
              _context.appendConditionalJump();
          _context << dupInstruction(2 + byteOffsetSize)
                   << dupInstruction(1 + byteOffsetSize);
          StorageItem(_context, *targetBaseType)
              .setToZero(SourceLocation(), true);
          utils.incrementByteOffset(targetBaseType->storageBytes(),
                                    byteOffsetSize, byteOffsetSize + 2);
          _context.appendJumpTo(copyLoopEnd);

          _context << copyCleanupLoopEnd;
          _context << Instruction::POP; // might pop the source, but then target
                                        // is popped next
        }
        if (haveByteOffsetSource)
          _context << Instruction::POP;
        ...
        // stack: target_ref target_data_end source_data_pos
        // target_data_pos_updated source_data_end
...
void ArrayUtils::copyArrayToMemory(ArrayType const &_sourceType,
                                   bool _padToWordBoundaries) const {
  solUnimplementedAssert(!_sourceType.baseType()->isDynamicallySized(),
  ...
  if (_sourceType.location() == DataLocation::CallData) {
    ...
    if (!_sourceType.baseType()->isValueType()) {
      ...
      MemoryItem(m_context, *_sourceType.baseType(), true)
          .retrieveValue(SourceLocation(), true);
      if (auto baseArray =
              dynamic_cast<ArrayType const *>(_sourceType.baseType().get()))
    ...
    if (_sourceType.isDynamicallySized())
      ...
      paddingNeeded = _padToWordBoundaries &&
                      (((_sourceType.length() * baseSize) % 32) != 0);
    if (paddingNeeded) {
      // stack: <target> <size>
    ...
    // stack here: memory_end_offset storage_data_offset [storage_byte_offset]
    // memory_offset
    ...
    if (_sourceType.isByteArray()) {
      ...
      StorageItem(m_context, *_sourceType.baseType())
          .retrieveValue(SourceLocation(), true);
      if (auto baseArray =
              dynamic_cast<ArrayType const *>(_sourceType.baseType().get()))
        copyArrayToMemory(*baseArray, _padToWordBoundaries);
    ...
    if (_padToWordBoundaries && baseSize % 32 != 0) {
      // memory_end_offset - start is the actual length (we want to compute the
      // ceil of). memory_offset - start is its next multiple of 32, but it
      // might be off by 32. so we compute: memory_end_offset += (memory_offset
      // - memory_end_offest) & 31
...
void ArrayUtils::clearArray(ArrayType const &_typeIn) const {
  ...
  m_context.callLowLevelFunction(
      "$clearArray_" + _typeIn.identifier(), 2, 0,
      [type](CompilerContext &_context) {
        ...
        if (_type.baseType()->storageBytes() < 32) {
          solAssert(_type.baseType()->isValueType(),
                    "Invalid storage size for non-value type.");
          solAssert(_type.baseType()->storageSize() <= 1,
                    "Invalid storage size for type.");
        }
        if (_type.baseType()->isValueType())
          solAssert(_type.baseType()->storageSize() <= 1,
                    "Invalid size for value type.");
        ...
        if (_type.isDynamicallySized())
          ...
          // unroll loop for small arrays @todo choose a good value
          solAssert(_type.baseType()->storageBytes() >= 32,
                    "Invalid storage size.");
          for (unsigned i = 1; i < _type.length(); ++i) {
            _context << u256(0);
            StorageItem(_context, *_type.baseType())
                .setToZero(SourceLocation(), false);
            _context << Instruction::POP
                     << u256(_type.baseType()->storageSize())
                     << Instruction::ADD;
          }
          _context << u256(0);
          StorageItem(_context, *_type.baseType())
...
void ArrayUtils::resizeDynamicArray(ArrayType const &_typeIn) const {
  TypePointer type = _typeIn.shared_from_this();
  m_context.callLowLevelFunction(
      "$resizeDynamicArray_" + _typeIn.identifier(), 2, 0,
      [type](CompilerContext &_context) {
        ArrayType const &_type = dynamic_cast<ArrayType const &>(*type);
        solAssert(_type.location() == DataLocation::Storage, "");
        solAssert(_type.isDynamicallySized(), "");
        ...
        // Special case for short byte arrays, they are stored together with
        // their length
        if (_type.isByteArray()) {
          eth::AssemblyItem regularPath = _context.newTag();
          // We start by a large case-distinction about the old and new length
          // of the byte array.
          ...
          _context.adjustStackOffset(
              1); // we have to do that because of the jumps
          // Here: short -> long
          ...
          _context << Instruction::DUP2 << Instruction::DUP1
                   << Instruction::ADD;
          _context << u256(1) << Instruction::ADD;
          ...
          // Here: long -> short
          // Read the first word of the data and store it on the stack. Clear
          // the data location and then jump to the short -> short case.
          ...
          _context << Instruction::DUP2 << Instruction::ADD
                   << Instruction::SWAP1;
          // stack: ref new_length current_length first_word data_location_end
          // data_location
        ...
        // Change of length for a regular array (i.e. length at location, data
        // at KECCAK256(location)). stack: ref new_length old_length store new
        // length
        _context << Instruction::DUP2;
        if (_type.isByteArray())
          // For a "long" byte array, store length as 2*length+1
          _context << Instruction::DUP1 << Instruction::ADD << u256(1)
                   << Instruction::ADD;
        _context << Instruction::DUP4 << Instruction::SSTORE;
...
void ArrayUtils::clearStorageLoop(TypePointer const &_type) const {
  m_context.callLowLevelFunction(
      "$clearStorageLoop_" + _type->identifier(), 2, 1,
      [_type](CompilerContext &_context) {
        ...
        _context << loopStart;
        // check for loop condition
        _context << Instruction::DUP1 << Instruction::DUP3 << Instruction::GT
                 << Instruction::ISZERO;
...
void ArrayUtils::convertLengthToSize(ArrayType const &_arrayType,
                                     bool _pad) const {
  if (_arrayType.location() == DataLocation::Storage) {
    if (_arrayType.baseType()->storageSize() <= 1) {
      ...
      if (baseBytes == 0)
        ...
        m_context << u256(itemsPerSlot - 1) << Instruction::ADD
                  << u256(itemsPerSlot) << Instruction::SWAP1
                  << Instruction::DIV;
      }
    } else
    ...
    if (!_arrayType.isByteArray()) {
      ...
      m_context << u256(31) << Instruction::ADD << u256(32) << Instruction::DUP1
                << Instruction::SWAP2 << Instruction::DIV << Instruction::MUL;
  }
}

void ArrayUtils::retrieveLength(ArrayType const &_arrayType,
                                unsigned _stackDepth) const {
  if (!_arrayType.isDynamicallySized())
    ...
    case DataLocation::Storage:
      ...
      if (_arrayType.isByteArray()) {
        ...
        // i.e. for short strings (x & 1 == 0) it does (x & 0xff) / 2 and for
        // long strings it computes (x & (-1)) / 2, which is equivalent to just
        // x / 2.
        m_context << u256(1) << Instruction::DUP2 << u256(1)
                  << Instruction::AND;
...
void ArrayUtils::accessIndex(ArrayType const &_arrayType,
                             bool _doBoundsCheck) const {
  ...
  case DataLocation::Storage: {
    ...
    if (_arrayType.baseType()->storageBytes() <= 16) {
      ...
      // goal:
      // <ref> <byte_number> = <base_ref + index / itemsPerSlot> <(index %
      // itemsPerSlot) * byteSize>
      unsigned byteSize = _arrayType.baseType()->storageBytes();
      solAssert(byteSize != 0, "");
      unsigned itemsPerSlot = 32 / byteSize;
      m_context << u256(itemsPerSlot) << Instruction::SWAP2;
      // stack: itemsPerSlot index data_ref
      m_context << Instruction::DUP3 << Instruction::DUP3 << Instruction::DIV
                << Instruction::ADD
                // stack: itemsPerSlot index (data_ref + index / itemsPerSlot)
                << Instruction::SWAP2 << Instruction::SWAP1 << Instruction::MOD;
      if (byteSize != 1)
...
void ArrayUtils::incrementByteOffset(unsigned _byteSize,
                                     unsigned _byteOffsetPosition,
                                     unsigned _storageOffsetPosition) const {
  ...
  if (_byteOffsetPosition > 1)
    m_context << swapInstruction(_byteOffsetPosition - 1);
  // compute, X := (byteOffset + byteSize - 1) / 32, should be 1 iff byteOffset
  // + bytesize > 32
  m_context << u256(32) << dupInstruction(1 + _byteOffsetPosition)
            << u256(_byteSize - 1) << Instruction::ADD << Instruction::DIV;
  // increment storage offset if X == 1 (just add X to it)
  // stack: X
  m_context << swapInstruction(_storageOffsetPosition)
            << dupInstruction(_storageOffsetPosition + 1) << Instruction::ADD
            << swapInstruction(_storageOffsetPosition);
  // stack: X
  // set source_byte_offset to zero if X == 1 (using source_byte_offset *= 1 -
  // X)
