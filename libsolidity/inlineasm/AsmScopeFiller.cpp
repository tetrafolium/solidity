...
bool ScopeFiller::operator()(Label const &_item) {
  if (!m_currentScope->registerLabel(_item.name)) {
    ...
    m_errorReporter.declarationError(_item.location,
                                     "Label name " + _item.name +
...
bool ScopeFiller::operator()(assembly::FunctionDefinition const &_funDef) {
  ...
  if (!m_currentScope->registerFunction(_funDef.name, arguments, returns)) {
    //@TODO secondary location
    m_errorReporter.declarationError(_funDef.location,
                                     "Function name " + _funDef.name +
                                         " already taken in this scope.");
    success = false;
  }
...
bool ScopeFiller::registerVariable(TypedName const &_name,
                                   SourceLocation const &_location,
                                   Scope &_scope) {
  if (!_scope.registerVariable(_name.name, _name.type)) {
    //@TODO secondary location
    m_errorReporter.declarationError(_location,
                                     "Variable name " + _name.name +
                                         " already taken in this scope.");
    return false;
  }
  return true;
