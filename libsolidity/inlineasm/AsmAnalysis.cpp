...
set<string> const builtinTypes{"bool", "u8",   "s8",   "u32",  "s32", "u64",
                               "s64",  "u128", "s128", "u256", "s256"};
}
...
bool AsmAnalyzer::operator()(assembly::Literal const &_literal) {
  ...
  if (_literal.kind == assembly::LiteralKind::String &&
      _literal.value.size() > 32) {
    m_errorReporter.typeError(
        _literal.location,
        "String literal too long (" +
            boost::lexical_cast<std::string>(_literal.value.size()) + " > 32)");
    return false;
  }
...
bool AsmAnalyzer::operator()(assembly::Identifier const &_identifier) {
  ...
  bool success = true;
  if (m_currentScope->lookup(_identifier.name,
                             Scope::Visitor(
                                   ...
                                   if (!m_activeVariables.count(&_var)) {
                                     m_errorReporter.declarationError(
                                         ...
                                         "Variable " + _identifier.name +
                                             " used before it was declared.");
                                     success = false;
                                   }
                                   ++m_stackHeight;
                                 },
                                 [&](Scope::Label const &) { ++m_stackHeight; },
    ...
    if (stackSize == size_t(-1)) {
      ...
      if (numErrorsBefore == m_errorReporter.errors().size())
        m_errorReporter.declarationError(_identifier.location,
...
bool AsmAnalyzer::operator()(assembly::VariableDeclaration const &_varDecl) {
  ...
  for (auto const &variable : _varDecl.variables) {
    ...
    m_activeVariables.insert(&boost::get<Scope::Variable>(
...
bool AsmAnalyzer::operator()(assembly::FunctionDefinition const &_funDef) {
  ...
  for (auto const &var : _funDef.parameters + _funDef.returnVariables) {
    ...
    m_activeVariables.insert(
        &boost::get<Scope::Variable>(varScope.identifiers.at(var.name)));
  }
  ...
  for (auto const &_case : _switch.cases) {
    if (_case.value) {
      ...
      if (!cases.insert(val).second) {
        m_errorReporter.declarationError(_case.location,
...
bool AsmAnalyzer::expectDeposit(int _deposit, int _oldHeight,
...
bool AsmAnalyzer::checkAssignment(assembly::Identifier const &_variable,
  ...
  if (Scope::Identifier const *var = m_currentScope->lookup(_variable.name)) {
    ...
    if (var->type() != typeid(Scope::Variable)) {
      ...
      m_errorReporter.declarationError(_variable.location,
    ...
    bool insideFunction = m_currentScope->insideFunction();
    variableSize =
        m_resolver(_variable, julia::IdentifierContext::LValue, insideFunction);
  }
  if (variableSize == size_t(-1)) {
    // Only add message if the callback did not.
    if (numErrorsBefore == m_errorReporter.errors().size())
      m_errorReporter.declarationError(
          _variable.location, "Variable not found or variable not lvalue.");
    success = false;
  }
  ...
  if (_valueSize != variableSize && variableSize != size_t(-1)) {
    m_errorReporter.typeError(_variable.location,
                              "Variable size (" + to_string(variableSize) +
                                  ") and value size (" + to_string(_valueSize) +
                                  ") do not match.");
    success = false;
  }
  return success;
...
Scope &AsmAnalyzer::scope(Block const *_block) {
  solAssert(m_info.scopes.count(_block) == 1,
            "Scope requested but not present.");
  auto scopePtr = m_info.scopes.at(_block);
  solAssert(scopePtr, "Scope requested but not present.");
  return *scopePtr;
}
void AsmAnalyzer::expectValidType(string const &type,
                                  SourceLocation const &_location) {
  if (!m_julia)
    return;

  if (!builtinTypes.count(type))
    m_errorReporter.typeError(_location, "\"" + type +
                                             "\" is not a valid type (user "
                                             "defined types are not yet "
                                             "supported).");
}

void AsmAnalyzer::warnOnInstructions(solidity::Instruction _instr,
                                     SourceLocation const &_location) {
  static set<solidity::Instruction> futureInstructions{
      solidity::Instruction::CREATE2, solidity::Instruction::RETURNDATACOPY,
      solidity::Instruction::RETURNDATASIZE, solidity::Instruction::STATICCALL};
  if (futureInstructions.count(_instr))
    m_errorReporter.warning(
        _location, "The \"" +
                       boost::to_lower_copy(instructionInfo(_instr).name) +
                       "\" instruction is only available after " +
                       "the Metropolis hard fork. Before that it acts as an "
                       "invalid instruction.");

  if (_instr == solidity::Instruction::JUMP ||
      _instr == solidity::Instruction::JUMPI ||
      _instr == solidity::Instruction::JUMPDEST)
    m_errorReporter.warning(
        _location,
        "Jump instructions and labels are low-level EVM features that can lead "
        "to "
        "incorrect stack access. Because of that they are discouraged. "
        "Please consider using \"switch\", \"if\" or \"for\" statements "
        "instead.");
}
