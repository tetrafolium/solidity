...
shared_ptr<assembly::Block>
Parser::parse(std::shared_ptr<Scanner> const &_scanner) {
  m_recursionDepth = 0;
  try {
    m_scanner = _scanner;
...
assembly::Statement Parser::parseStatement() {
  ...
  case Token::If: {
    ...
    if (_if.condition->type() == typeid(assembly::Instruction))
      fatalParserError("Instructions are not supported as conditions for if - "
                       "try to append \"()\".");
    _if.body = parseBlock();
    return _if;
  }
  case Token::Switch: {
    ...
    if (_switch.expression->type() == typeid(assembly::Instruction))
      fatalParserError("Instructions are not supported as expressions for "
                       "switch - try to append \"()\".");
  ...
  case Token::Assign: {
    if (m_julia)
      break;
    assembly::StackAssignment assignment =
        createWithLocation<assembly::StackAssignment>();
    advance();
  ...
  case Token::Comma: {
    ...
    if (statement.type() != typeid(assembly::Identifier))
      fatalParserError("Label name / variable name must precede \",\" "
                       "(multiple assignment).");
    assembly::Identifier const &identifier =
        boost::get<assembly::Identifier>(statement);
    ...
    do {
      ...
      assignment.variableNames.emplace_back(
          boost::get<assembly::Identifier>(statement));
  ...
  case Token::Colon: {
    ...
    // while identifier:= (being followed by a non-colon) as identifier :=
    // (assignment).
    if (currentToken() == Token::Assign && peekNextToken() != Token::Colon) {
      assembly::Assignment assignment =
          createWithLocation<assembly::Assignment>(identifier.location);
...
assembly::Case Parser::parseCase() {
  ...
  if (m_scanner->currentToken() == Token::Default)
    ...
    _case.value = make_shared<Literal>(
        std::move(boost::get<assembly::Literal>(statement)));
  } else
    ...
    if (args > 0 && currentToken() != Token::LParen)
      fatalParserError(
          string("Expected token \"(\" (\"" +
                 instructionNames().at(instr.instruction) + "\" expects " +
                 boost::lexical_cast<string>(args) + " arguments)"));
  }
...
std::map<string, dev::solidity::Instruction> const &Parser::instructions() {
  ...
  if (s_instructions.empty()) {
    for (auto const &instruction : solidity::c_instructions) {
      if (instruction.second == solidity::Instruction::JUMPDEST ||
...
assembly::Statement Parser::parseElementaryOperation(bool _onlySinglePusher) {
  ...
  case Token::Address: {
    ...
    if (!m_julia && instructions().count(literal)) {
      ...
      if (_onlySinglePusher) {
        ...
        if (info.ret != 1)
          fatalParserError("Instruction \"" + literal +
  ...
  case Token::FalseLiteral: {
    ...
    Literal literal{location(), kind, currentLiteral(), ""};
    advance();
    if (m_julia) {
      expectToken(Token::Colon);
  ...
  default:
    fatalParserError(m_julia ? "Literal or identifier expected."
...
assembly::Statement Parser::parseCall(assembly::Statement &&_instruction) {
  ...
  if (_instruction.type() == typeid(Instruction)) {
    ...
    Instruction const &instruction =
        std::move(boost::get<Instruction>(_instruction));
    ...
    if (solidity::isSwapInstruction(instr))
      fatalParserError(
          "SWAPi instructions not allowed for functional notation");
    expectToken(Token::LParen);
    ...
    for (unsigned i = 0; i < args; ++i) {
      ...
      if (currentToken() == Token::RParen)
        fatalParserError(string(
            "Expected expression (\"" + instructionNames().at(instr) +
            "\" expects " + boost::lexical_cast<string>(args) + " arguments)"));

      ret.arguments.emplace_back(parseExpression());
      if (i != args - 1) {
        if (currentToken() != Token::Comma)
          fatalParserError(string("Expected comma (\"" +
                                  instructionNames().at(instr) + "\" expects " +
    ...
    if (currentToken() == Token::Comma)
      fatalParserError(string(
          "Expected ')' (\"" + instructionNames().at(instr) + "\" expects " +
          boost::lexical_cast<string>(args) + " arguments)"));
    expectToken(Token::RParen);
    return ret;
    ...
    fatalParserError(m_julia ? "Function name expected."
                             : "Assembly instruction or function name required "
                               "in front of \"(\")");
