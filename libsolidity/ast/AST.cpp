...
ASTNode::ASTNode(SourceLocation const &_location)
    : m_id(IDDispenser::next()), m_location(_location) {}

ASTNode::~ASTNode() { delete m_annotation; }

void ASTNode::resetID() { IDDispenser::reset(); }
...
set<SourceUnit const *>
SourceUnit::referencedSourceUnits(bool _recurse,
                                  set<SourceUnit const *> _skipList) const {
  set<SourceUnit const *> sourceUnits;
  for (ImportDirective const *importDirective :
       filteredNodes<ImportDirective>(nodes())) {
...
SourceUnit const &Declaration::sourceUnit() const {
  ...
  while (dynamic_cast<Declaration const *>(scope) &&
...
map<FixedHash<4>, FunctionTypePointer>
ContractDefinition::interfaceFunctions() const {
...
FunctionDefinition const *ContractDefinition::fallbackFunction() const {
  for (ContractDefinition const *contract :
       annotation().linearizedBaseContracts)
...
vector<EventDefinition const *> const &
ContractDefinition::interfaceEvents() const {
...
vector<pair<FixedHash<4>, FunctionTypePointer>> const &
ContractDefinition::interfaceFunctionList() const {
  if (!m_interfaceFunctionList) {
    set<string> signaturesSeen;
    m_interfaceFunctionList.reset(
        new vector<pair<FixedHash<4>, FunctionTypePointer>>());
    for (ContractDefinition const *contract :
         annotation().linearizedBaseContracts) {
...
vector<Declaration const *> const &
ContractDefinition::inheritableMembers() const {
  if (!m_inheritableMembers) {
    set<string> memberSeen;
    m_inheritableMembers.reset(new vector<Declaration const *>());
    auto addInheritableMember = [&](Declaration const *_decl) {
      solAssert(_decl, "addInheritableMember got a nullpointer.");
      if (memberSeen.count(_decl->name()) == 0 &&
          _decl->isVisibleInDerivedContracts()) {
...
FunctionDefinition::functionType(bool _internal) const {
  if (_internal) {
    switch (visibility()) {
...
EventDefinition::functionType(bool _internal) const {
  if (_internal)
    return make_shared<FunctionType>(*this);
  else
    return {};
...
shared_ptr<FunctionType>
VariableDeclaration::functionType(bool _internal) const {
  if (_internal)
    return {};
  switch (visibility()) {
  case Declaration::Visibility::Default:
...
VariableDeclarationStatement::annotation() const {
  if (!m_annotation)
