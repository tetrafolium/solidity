...
struct TypeDeclarationAnnotation : ASTAnnotation {
  /// The name of this type, prefixed by proper namespaces if globally
  /// accessible.
...
struct ContractDefinitionAnnotation : TypeDeclarationAnnotation,
  ...
  /// List of functions without a body. Can also contain functions from base
  /// classes.
  std::vector<FunctionDefinition const *> unimplementedFunctions;
  /// List of all (direct and indirect) base contracts in order from derived to
  /// base, including the contract itself.
  std::vector<ContractDefinition const *> linearizedBaseContracts;
  /// List of contracts this contract creates, i.e. which need to be compiled
  /// first. Also includes all contracts from @a linearizedBaseContracts.
...
struct InlineAssemblyAnnotation : StatementAnnotation {
  struct ExternalIdentifierInfo {
    ...
    bool isOffset = false; ///< Whether the intra-slot offset of a storage
                           ///< variable is queried.
    size_t valueSize = size_t(-1);
  };

  /// Mapping containing resolved references to external identifiers and their
  /// value size
  std::map<assembly::Identifier const *, ExternalIdentifierInfo>
      externalReferences;
...
struct TypeNameAnnotation : ASTAnnotation {
  /// Type declared by this type name, i.e. type of a variable where this type
  /// name is used. Set during reference resolution stage.
  TypePointer type;
};
...
struct UserDefinedTypeNameAnnotation : TypeNameAnnotation {
  ...
  /// This is needed because types of base contracts change depending on the
  /// context.
  ContractDefinition const *contractScope = nullptr;
};

struct VariableDeclarationStatementAnnotation : StatementAnnotation {
  /// Information about which component of the value is assigned to which
  /// variable. The pointer can be null to signify that the component is
  /// discarded.
...
struct ExpressionAnnotation : ASTAnnotation {
  ...
  bool isLValue = false;
  /// Whether the expression is used in a context where the LValue is actually
  /// required.
...
struct BinaryOperationAnnotation : ExpressionAnnotation {
  /// The common type that is used for the operation, not necessarily the result
  /// type (which e.g. for comparisons is bool).
