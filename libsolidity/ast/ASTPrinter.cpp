 ...
 * @date 2014
 * Pretty-printer for the abstract syntax tree (the "pretty" is arguable), used
 * for debugging.
 */
...
bool ASTPrinter::visit(UserDefinedTypeName const &_node) {
  writeLine("UserDefinedTypeName \"" +
            boost::algorithm::join(_node.namePath(), ".") + "\"");
  printSourcePart(_node);
  return goDeeper();
}
...
bool ASTPrinter::visit(Assignment const &_node) {
  writeLine(string("Assignment using operator ") +
...
bool ASTPrinter::visit(UnaryOperation const &_node) {
  writeLine(string("UnaryOperation (") +
...
bool ASTPrinter::visit(BinaryOperation const &_node) {
  writeLine(string("BinaryOperation using operator ") +
            Token::toString(_node.getOperator()));
  printType(_node);
  printSourcePart(_node);
  return goDeeper();
...
bool ASTPrinter::visit(ElementaryTypeNameExpression const &_node) {
  writeLine(string("ElementaryTypeNameExpression ") +
            _node.typeName().toString());
  printType(_node);
  printSourcePart(_node);
  return goDeeper();
}
...
bool ASTPrinter::visit(Literal const &_node) {
  ...
  writeLine(string("Literal, token: ") + tokenString +
            " value: " + _node.value());
  printType(_node);
  printSourcePart(_node);
  return goDeeper();
...
void ASTPrinter::endVisit(PragmaDirective const &) { m_indentation--; }
...
void ASTPrinter::endVisit(InheritanceSpecifier const &) { m_indentation--; }
...
void ASTPrinter::endVisit(VariableDeclaration const &) { m_indentation--; }
...
void ASTPrinter::endVisit(FunctionTypeName const &) { m_indentation--; }
...
void ASTPrinter::endVisit(Block const &) { m_indentation--; }

void ASTPrinter::endVisit(PlaceholderStatement const &) { m_indentation--; }
...
void ASTPrinter::endVisit(Break const &) { m_indentation--; }
...
void ASTPrinter::endVisit(Assignment const &) { m_indentation--; }

void ASTPrinter::endVisit(TupleExpression const &) { m_indentation--; }

void ASTPrinter::endVisit(UnaryOperation const &) { m_indentation--; }
...
void ASTPrinter::printSourcePart(ASTNode const &_node) {
  if (m_gasCosts.count(&_node))
    *m_ostream << indentation() << "   Gas costs: " << m_gasCosts.at(&_node)
               << endl;
  if (!m_source.empty()) {
    ...
    *m_ostream << indentation() << "   Source: "
               << Json::valueToQuotedString(
                      m_source
                          .substr(location.start, location.end - location.start)
                          .c_str())
               << endl;
  }
}
...
void ASTPrinter::printType(Expression const &_expression) {
  if (_expression.annotation().type)
    *m_ostream << indentation()
               << "   Type: " << _expression.annotation().type->toString()
               << "\n";
  else
