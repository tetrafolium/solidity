...
/**
 * Pretty-printer for the abstract syntax tree (the "pretty" is arguable) for
 * debugging purposes.
 */
class ASTPrinter : public ASTConstVisitor {
public:
  /// Create a printer for the given abstract syntax tree. If the source is
  /// specified, the corresponding parts of the source are printed with each
  /// node.
  ASTPrinter(ASTNode const &_ast, std::string const &_source = std::string(),
             GasEstimator::ASTGasConsumption const &_gasCosts =
                 GasEstimator::ASTGasConsumption());
