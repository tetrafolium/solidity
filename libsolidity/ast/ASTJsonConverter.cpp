...
ASTJsonConverter::ASTJsonConverter(bool _legacy,
                                   map<string, unsigned> _sourceIndices)
    : m_legacy(_legacy), m_sourceIndices(_sourceIndices) {}

void ASTJsonConverter::setJsonNode(
    ASTNode const &_node, string const &_nodeName,
...
void ASTJsonConverter::setJsonNode(
    ASTNode const &_node, string const &_nodeType,
  ...
  if (!m_legacy) {
    ...
    if (
        ...
        (_nodeType == "ParameterList") || (_nodeType == "Block") ||
        (_nodeType == "InlineAssembly") || (_nodeType == "Throw"))
      m_currentValue["children"] = Json::arrayValue;

    for (auto &e : _attributes) {
      if ((!e.second.isNull()) &&
          ((e.second.isObject() && e.second.isMember("name")) ||
           (e.second.isArray() && e.second[0].isObject() &&
            e.second[0].isMember("name")) ||
           (e.first == "declarations") // (in the case (_,x)= ... there's a
                                       // nullpointer at [0]
           )) {
...
string ASTJsonConverter::sourceLocationToString(
    SourceLocation const &_location) const {
  int sourceIndex{-1};
...
string
ASTJsonConverter::namePathToString(std::vector<ASTString> const &_namePath) {
...
Json::Value ASTJsonConverter::typePointerToJson(TypePointer _tp) {
  ...
  typeDescriptions["typeString"] =
      _tp ? Json::Value(_tp->toString()) : Json::nullValue;
  typeDescriptions["typeIdentifier"] =
      _tp ? Json::Value(_tp->identifier()) : Json::nullValue;
  return typeDescriptions;
}
Json::Value ASTJsonConverter::typePointerToJson(
    std::shared_ptr<std::vector<TypePointer>> _tps) {
  if (_tps) {
...
Json::Value ASTJsonConverter::inlineAssemblyIdentifierToJson(
    pair<assembly::Identifier const *,
         InlineAssemblyAnnotation::ExternalIdentifierInfo>
        _info) const {
...
bool ASTJsonConverter::visit(ImportDirective const &_node) {
  std::vector<pair<string, Json::Value>> attributes = {
      ...
      make_pair(m_legacy ? "SourceUnit" : "sourceUnit",
                nodeId(*_node.annotation().sourceUnit)),
  ...
  for (auto const &symbolAlias : _node.symbolAliases()) {
    ...
    tuple["local"] =
        symbolAlias.second ? Json::Value(*symbolAlias.second) : Json::nullValue;
...
bool ASTJsonConverter::visit(ContractDefinition const &_node) {
  setJsonNode(
      ...
      {make_pair("name", _node.name()),
       make_pair("documentation", _node.documentation()
                                      ? Json::Value(*_node.documentation())
                                      : Json::nullValue),
       make_pair("contractKind", contractKind(_node.contractKind())),
       make_pair("fullyImplemented",
                 _node.annotation().unimplementedFunctions.empty()),
       make_pair("linearizedBaseContracts",
                 getContainerIds(_node.annotation().linearizedBaseContracts)),
...
bool ASTJsonConverter::visit(UsingForDirective const &_node) {
  setJsonNode(
      ...
      {make_pair("libraryName", toJson(_node.libraryName())),
       make_pair("typeName", _node.typeName() ? toJson(*_node.typeName())
                                              : Json::nullValue)});
  return false;
}
  ...
  setJsonNode(_node, "StructDefinition",
              {make_pair("name", _node.name()),
               make_pair("visibility",
                         Declaration::visibilityToString(_node.visibility())),
...
bool ASTJsonConverter::visit(FunctionDefinition const &_node) {
  std::vector<pair<string, Json::Value>> attributes = {
      ...
      make_pair("stateMutability",
                stateMutabilityToString(_node.stateMutability())),
      ...
      make_pair("body",
                _node.isImplemented() ? toJson(_node.body()) : Json::nullValue),
...
bool ASTJsonConverter::visit(VariableDeclaration const &_node) {
  std::vector<pair<string, Json::Value>> attributes = {
      ...
      make_pair("visibility",
                Declaration::visibilityToString(_node.visibility())),
      make_pair("value",
                _node.value() ? toJson(*_node.value()) : Json::nullValue),
      make_pair("scope", idOrNull(_node.scope())),
      make_pair("typeDescriptions",
                typePointerToJson(_node.annotation().type))};
  if (m_inEvent)
...
bool ASTJsonConverter::visit(UserDefinedTypeName const &_node) {
  setJsonNode(
      _node, "UserDefinedTypeName",
      {make_pair("name", namePathToString(_node.namePath())),
       make_pair("referencedDeclaration",
                 idOrNull(_node.annotation().referencedDeclaration)),
...
bool ASTJsonConverter::visit(FunctionTypeName const &_node) {
  setJsonNode(_node, "FunctionTypeName",
              {make_pair("payable", _node.isPayable()),
               ...
               make_pair("returnParameterTypes",
                         toJson(*_node.returnParameterTypeList())),
               make_pair("typeDescriptions",
                         typePointerToJson(_node.annotation().type))});
  return false;
}
...
bool ASTJsonConverter::visit(InlineAssembly const &_node) {
  ...
  setJsonNode(_node, "InlineAssembly",
              {make_pair("operations", Json::Value(assembly::AsmPrinter()(
                                           _node.operations()))),
...
bool ASTJsonConverter::visit(WhileStatement const &_node) {
  setJsonNode(_node, _node.isDoWhile() ? "DoWhileStatement" : "WhileStatement",
              {make_pair("condition", toJson(_node.condition())),
               make_pair("body", toJson(_node.body()))});
  return false;
}
...
bool ASTJsonConverter::visit(ForStatement const &_node) {
  setJsonNode(
      _node, "ForStatement",
      {make_pair("initializationExpression",
                 toJsonOrNull(_node.initializationExpression())),
...
bool ASTJsonConverter::visit(Return const &_node) {
  setJsonNode(
      ...
      {make_pair("expression", toJsonOrNull(_node.expression())),
       make_pair("functionReturnParameters",
                 idOrNull(_node.annotation().functionReturnParameters))});
  return false;
}
...
bool ASTJsonConverter::visit(FunctionCall const &_node) {
  ...
  std::vector<pair<string, Json::Value>> attributes = {
      ...
      make_pair("arguments", toJson(_node.arguments()))};
  if (m_legacy) {
    attributes.push_back(make_pair(
        "isStructConstructorCall",
        _node.annotation().kind == FunctionCallKind::StructConstructorCall));
    attributes.push_back(
        make_pair("type_conversion",
                  _node.annotation().kind == FunctionCallKind::TypeConversion));
  } else
    attributes.push_back(
        make_pair("kind", functionCallKind(_node.annotation().kind)));
  appendExpressionAttributes(attributes, _node.annotation());
...
void ASTJsonConverter::endVisit(EventDefinition const &) { m_inEvent = false; }
...
string ASTJsonConverter::type(Expression const &_expression) {
  return _expression.annotation().type
             ? _expression.annotation().type->toString()
             : "Unknown";
}

string ASTJsonConverter::type(VariableDeclaration const &_varDecl) {
  return _varDecl.annotation().type ? _varDecl.annotation().type->toString()
                                    : "Unknown";
}
