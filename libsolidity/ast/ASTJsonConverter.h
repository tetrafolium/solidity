...
public:
  ...
  explicit ASTJsonConverter(bool _legacy,
...
private:
  void setJsonNode(
      ASTNode const &_node, std::string const &_nodeName,
      std::initializer_list<std::pair<std::string, Json::Value>> &&_attributes);
  void
  setJsonNode(ASTNode const &_node, std::string const &_nodeName,
  ...
  Json::Value inlineAssemblyIdentifierToJson(
      std::pair<assembly::Identifier const *,
  ...
  static Json::Value
  typePointerToJson(std::shared_ptr<std::vector<TypePointer>> _tps);
