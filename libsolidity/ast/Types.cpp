...
void StorageOffsets::computeOffsets(TypePointers const &_types) {
  ...
  for (size_t i = 0; i < _types.size(); ++i) {
    ...
    if (slotOffset >= bigint(1) << 256)
      BOOST_THROW_EXCEPTION(Error(Error::Type::TypeError) << errinfo_comment(
                                "Object too large for storage."));
  ...
  if (slotOffset >= bigint(1) << 256)
    BOOST_THROW_EXCEPTION(Error(Error::Type::TypeError)
                          << errinfo_comment("Object too large for storage."));
...
pair<u256, unsigned> const *
MemberList::memberStorageOffset(string const &_name) const {
  if (!m_storageOffsets) {
...
template <class Range> string identifierList(Range const &&_list) {
...
string parenthesizeUserIdentifier(string const &_internal) {
  return parenthesizeIdentifier(
      boost::algorithm::replace_all_copy(_internal, "$", "$$$"));
}
...
TypePointer Type::fromElementaryTypeName(ElementaryTypeNameToken const &_type) {
  ...
  case Token::UFixedMxN:
    return make_shared<FixedPointType>(m, n,
  ...
  case Token::Fixed:
    return make_shared<FixedPointType>(128, 19,
  ...
  // no types found
  default:
    solAssert(false, "Unable to convert elementary typename " +
                         _type.toString() + " to type.");
  }
}
...
TypePointer Type::fromElementaryTypeName(string const &_name) {
  ...
  return fromElementaryTypeName(
      ElementaryTypeNameToken(token, firstNum, secondNum));
}
...
TypePointer Type::forLiteral(Literal const &_literal) {
  ...
  case Token::FalseLiteral:
    return make_shared<BoolType>();
  case Token::Number: {
    tuple<bool, rational> validLiteral =
        RationalNumberType::isValidLiteral(_literal);
...
MemberList const &Type::members(ContractDefinition const *_currentScope) const {
  if (!m_members[_currentScope]) {
    ...
    m_members[_currentScope] =
        unique_ptr<MemberList>(new MemberList(move(members)));
  }
  return *m_members[_currentScope];
}

MemberList::MemberMap Type::boundFunctions(Type const &_type,
                                           ContractDefinition const &_scope) {
  // Normalise data location of type.
  TypePointer type = ReferenceType::copyForLocationIfReference(
      DataLocation::Storage, _type.shared_from_this());
  set<Declaration const *> seenFunctions;
  MemberList::MemberMap members;
  for (ContractDefinition const *contract :
       _scope.annotation().linearizedBaseContracts)
    for (UsingForDirective const *ufd : contract->usingForDirectives()) {
      if (ufd->typeName() &&
          *type !=
              *ReferenceType::copyForLocationIfReference(
                  DataLocation::Storage, ufd->typeName()->annotation().type))
        continue;
      ...
      for (FunctionDefinition const *function : library.definedFunctions()) {
        ...
        if (auto fun = funType.asMemberFunction(true, true))
          if (_type.isImplicitlyConvertibleTo(*fun->selfType()))
            members.push_back(
                MemberList::Member(function->name(), fun, function));
      }
    }
  return members;
}
...
bool isValidShiftAndAmountType(Token::Value _operator,
                               Type const &_shiftAmountType) {
  // Disable >>> here.
  if (_operator == Token::SHR)
    return false;
  else if (IntegerType const *otherInt =
               dynamic_cast<decltype(otherInt)>(&_shiftAmountType))
    return !otherInt->isAddress();
  else if (RationalNumberType const *otherRat =
               dynamic_cast<decltype(otherRat)>(&_shiftAmountType))
...
bool IntegerType::isImplicitlyConvertibleTo(Type const &_convertTo) const {
  if (_convertTo.category() == category()) {
    IntegerType const &convertTo =
        dynamic_cast<IntegerType const &>(_convertTo);
    ...
    FixedPointType const &convertTo =
        dynamic_cast<FixedPointType const &>(_convertTo);

    if (isAddress())
      return false;
    else
      return maxValue() <= convertTo.maxIntegerValue() &&
             minValue() >= convertTo.minIntegerValue();
  } else
    return false;
}
...
TypePointer IntegerType::binaryOperatorResult(Token::Value _operator,
  ...
  auto commonType = Type::commonType(
      shared_from_this(), _other); // might be a integer or fixed point
  if (!commonType)
    return TypePointer();
  ...
  if (auto intType = dynamic_pointer_cast<IntegerType const>(commonType)) {
    ...
    if (Token::Exp == _operator && intType->isSigned())
      return TypePointer();
  } else if (auto fixType =
                 dynamic_pointer_cast<FixedPointType const>(commonType))
...
IntegerType::nativeMembers(ContractDefinition const *) const {
  if (isAddress())
    return {
        {"balance", make_shared<IntegerType>(256)},
        {"call", make_shared<FunctionType>(strings(), strings{"bool"},
                                           FunctionType::Kind::BareCall, true,
                                           StateMutability::Payable)},
        {"callcode", make_shared<FunctionType>(strings(), strings{"bool"},
                                               FunctionType::Kind::BareCallCode,
                                               true, StateMutability::Payable)},
        {"delegatecall",
         make_shared<FunctionType>(strings(), strings{"bool"},
                                   FunctionType::Kind::BareDelegateCall, true)},
        {"send", make_shared<FunctionType>(strings{"uint"}, strings{"bool"},
                                           FunctionType::Kind::Send)},
        {"transfer", make_shared<FunctionType>(strings{"uint"}, strings(),
                                               FunctionType::Kind::Transfer)}};
  else
    return MemberList::MemberMap();
}

FixedPointType::FixedPointType(int _totalBits, int _fractionalDigits,
                               FixedPointType::Modifier _modifier)
    : m_totalBits(_totalBits), m_fractionalDigits(_fractionalDigits),
      m_modifier(_modifier) {
  solAssert(
      8 <= m_totalBits && m_totalBits <= 256 && m_totalBits % 8 == 0 &&
          0 <= m_fractionalDigits && m_fractionalDigits <= 80,
      "Invalid bit number(s) for fixed type: " + dev::toString(_totalBits) +
          "x" + dev::toString(_fractionalDigits));
}

string FixedPointType::identifier() const {
  return "t_" + string(isSigned() ? "" : "u") + "fixed" +
         std::to_string(m_totalBits) + "x" + std::to_string(m_fractionalDigits);
}
...
bool FixedPointType::isImplicitlyConvertibleTo(Type const &_convertTo) const {
  if (_convertTo.category() == category()) {
    ...
    if (convertTo.numBits() < m_totalBits ||
        convertTo.fractionalDigits() < m_fractionalDigits)
      return false;
    else
      return convertTo.maxIntegerValue() >= maxIntegerValue() &&
             convertTo.minIntegerValue() <= minIntegerValue();
  }
  return false;
}
...
bool FixedPointType::operator==(Type const &_other) const {
  ...
  return other.m_totalBits == m_totalBits &&
         other.m_fractionalDigits == m_fractionalDigits &&
         other.m_modifier == m_modifier;
}

string FixedPointType::toString(bool) const {
  string prefix = isSigned() ? "fixed" : "ufixed";
  return prefix + dev::toString(m_totalBits) + "x" +
         dev::toString(m_fractionalDigits);
}
...
TypePointer
FixedPointType::binaryOperatorResult(Token::Value _operator,
                                     TypePointer const &_other) const {
  if (_other->category() != Category::RationalNumber &&
      _other->category() != category() &&
      _other->category() != Category::Integer)
    return TypePointer();
  auto commonType = Type::commonType(shared_from_this(),
                                     _other); // might be fixed point or integer
...
tuple<bool, rational> RationalNumberType::parseRational(string const &_value) {
  ...
  try {
    ...
    if (radixPoint != _value.end()) {
      if (!all_of(radixPoint + 1, _value.end(), ::isdigit) ||
          !all_of(_value.begin(), radixPoint, ::isdigit))
        return make_tuple(false, rational(0));

      // Only decimal notation allowed here, leading zeros would switch to
      // octal.
      auto fractionalBegin = find_if_not(
          radixPoint + 1, _value.end(), [](char const &a) { return a == '0'; });
      ...
      denominator /= boost::multiprecision::pow(
          bigint(10), distance(radixPoint + 1, _value.end()));
...
tuple<bool, rational>
RationalNumberType::isValidLiteral(Literal const &_literal) {
  rational value;
  try {
    ...
    if (boost::starts_with(_literal.value(), "0x")) {
      ...
      if (exp > numeric_limits<int32_t>::max() ||
          exp < numeric_limits<int32_t>::min())
        return make_tuple(false, rational(0));

      // parse the base
      tuple<bool, rational> base =
          parseRational(string(_literal.value().begin(), expPoint));
      if (!get<0>(base))
        return make_tuple(false, rational(0));
      value = get<1>(base);
      ...
      if (exp < 0) {
        exp *= -1;
        value /=
            boost::multiprecision::pow(bigint(10), exp.convert_to<int32_t>());
      } else
        value *=
            boost::multiprecision::pow(bigint(10), exp.convert_to<int32_t>());
    } else {
...
bool RationalNumberType::isImplicitlyConvertibleTo(
  ...
  if (_convertTo.category() == Category::Integer) {
    ...
    if (m_value > rational(0)) {
      if (m_value.numerator() <=
          (u256(-1) >> (256 - targetType.numBits() + forSignBit)))
        return true;
    } else if (targetType.isSigned() &&
               -m_value.numerator() <=
                   (u256(1) << (targetType.numBits() - forSignBit)))
      return true;
    return false;
  } else if (_convertTo.category() == Category::FixedPoint) {
    ...
    FixedBytesType const &fixedBytes =
        dynamic_cast<FixedBytesType const &>(_convertTo);
    if (!isFractional()) {
      if (integerType())
...
bool RationalNumberType::isExplicitlyConvertibleTo(
    Type const &_convertTo) const {
...
TypePointer
RationalNumberType::unaryOperatorResult(Token::Value _operator) const {
  rational value;
  switch (_operator) {
  case Token::BitNot:
    if (isFractional())
...
TypePointer
RationalNumberType::binaryOperatorResult(Token::Value _operator,
                                         TypePointer const &_other) const {
  ...
  RationalNumberType const &other =
      dynamic_cast<RationalNumberType const &>(*_other);
  if (Token::isCompareOp(_operator)) {
    // Since we do not have a "BoolConstantType", we have to do the acutal
    // comparison at runtime and convert to mobile typse first. Such a
    // comparison is not a very common use-case and will be optimized away.
    ...
    switch (_operator) {
    // bit operations will only be enabled for integers and fixed types that
    // resemble integers
    case Token::BitOr:
      if (fractional)
        return TypePointer();
    ...
    case Token::Mod:
      if (other.m_value == rational(0))
        ...
        value = m_value - (tempValue.numerator() / tempValue.denominator()) *
                              other.m_value;
      } else
        value = m_value.numerator() % other.m_value.numerator();
      break;
    ...
    // NOTE: we're using >> (SAR) to denote right shifting. The type of the
    // LValue
...
string RationalNumberType::identifier() const {
  return "t_rational_" + m_value.numerator().str() + "_by_" +
         m_value.denominator().str();
}
...
string RationalNumberType::toString(bool) const {
  ...
  return "rational_const " + m_value.numerator().str() + '/' +
         m_value.denominator().str();
}

u256 RationalNumberType::literalValue(Literal const *) const {
  // We ignore the literal and hope that the type was correctly determined to
  // represent its value.
  ...
  if (!isFractional())
    ...
    shiftedValue = (m_value.numerator() / m_value.denominator()) *
                   pow(bigint(10), fractionalDigits);
  }
...
shared_ptr<FixedPointType const> RationalNumberType::fixedPointType() const {
  ...
  rational maxValue = negative ? rational(bigint(1) << 255, 1)
                               : rational((bigint(1) << 256) - 1, 1);

  while (value * 10 <= maxValue && value.denominator() != 1 &&
         fractionalDigits < 80) {
    value *= 10;
    fractionalDigits++;
  }
  ...
  return make_shared<FixedPointType>(totalBits, fractionalDigits,
                                     negative
                                         ? FixedPointType::Modifier::Signed
                                         : FixedPointType::Modifier::Unsigned);
}
...
bool StringLiteralType::isImplicitlyConvertibleTo(
    Type const &_convertTo) const {
...
string StringLiteralType::identifier() const {
  // Since we have to return a valid identifier and the string itself may
  // contain anything, we hash it.
...
std::string StringLiteralType::toString(bool) const {
  ...
  if (!dev::validateUTF8(m_value, invalidSequence))
    return "literal_string (contains invalid UTF-8 sequence at position " +
           dev::toString(invalidSequence) + ")";
...
shared_ptr<FixedBytesType>
FixedBytesType::smallestTypeForLiteral(string const &_literal) {
...
FixedBytesType::FixedBytesType(int _bytes) : m_bytes(_bytes) {
  solAssert(m_bytes >= 0 && m_bytes <= 32,
            "Invalid byte number for fixed bytes type: " +
...
bool FixedBytesType::isImplicitlyConvertibleTo(Type const &_convertTo) const {
  if (_convertTo.category() != category())
    return false;
  FixedBytesType const &convertTo =
      dynamic_cast<FixedBytesType const &>(_convertTo);
...
TypePointer
FixedBytesType::binaryOperatorResult(Token::Value _operator,
                                     TypePointer const &_other) const {
  if (Token::isShiftOp(_operator)) {
    if (isValidShiftAndAmountType(_operator, *_other))
      return shared_from_this();
  ...
  auto commonType = dynamic_pointer_cast<FixedBytesType const>(
      Type::commonType(shared_from_this(), _other));
  if (!commonType)
    return TypePointer();
...
MemberList::MemberMap
FixedBytesType::nativeMembers(const ContractDefinition *) const {
  return MemberList::MemberMap{
      MemberList::Member{"length", make_shared<IntegerType>(8)}};
}
...
TypePointer BoolType::binaryOperatorResult(Token::Value _operator,
                                           TypePointer const &_other) const {
  if (category() != _other->category())
    return TypePointer();
  if (Token::isCompareOp(_operator) || _operator == Token::And ||
      _operator == Token::Or)
    return _other;
  else
    return TypePointer();
...
bool ContractType::isImplicitlyConvertibleTo(Type const &_convertTo) const {
  ...
  if (_convertTo.category() == Category::Contract) {
    auto const &bases =
        contractDefinition().annotation().linearizedBaseContracts;
    if (m_super && bases.size() <= 1)
      return false;
    return find(m_super ? ++bases.begin() : bases.begin(), bases.end(),
                &dynamic_cast<ContractType const &>(_convertTo)
                     .contractDefinition()) != bases.end();
  }
  return false;
}
...
TypePointer ReferenceType::unaryOperatorResult(Token::Value _operator) const {
  if (_operator != Token::Delete)
    return TypePointer();
  // delete can be used on everything except calldata references or storage
  // pointers (storage references are ok)
  switch (location()) {
...
TypePointer
ReferenceType::copyForLocationIfReference(DataLocation _location,
...
TypePointer
ReferenceType::copyForLocationIfReference(TypePointer const &_type) const {
...
bool ArrayType::isImplicitlyConvertibleTo(const Type &_convertTo) const {
  ...
  if (convertTo.isByteArray() != isByteArray() ||
      convertTo.isString() != isString())
    return false;
  // memory/calldata to storage can be converted, but only to a direct storage
  // reference
  if (convertTo.location() == DataLocation::Storage &&
      location() != DataLocation::Storage && convertTo.isPointer())
    return false;
  if (convertTo.location() == DataLocation::CallData &&
      location() != convertTo.location())
    return false;
  if (convertTo.location() == DataLocation::Storage && !convertTo.isPointer()) {
    ...
    // Conversion to storage pointer or to memory, we de not copy
    // element-for-element here, so require that the base type is the same, not
    // only convertible. This disallows assignment of nested dynamic arrays from
    // storage to memory for now.
    if (*copyForLocationIfReference(location(), baseType()) !=
...
bool ArrayType::operator==(Type const &_other) const {
  ...
  if (!ReferenceType::operator==(other) ||
      other.isByteArray() != isByteArray() || other.isString() != isString() ||
...
bigint ArrayType::unlimitedCalldataEncodedSize(bool _padded) const {
  ...
  bigint size = bigint(length()) *
                (isByteArray() ? 1 : baseType()->calldataEncodedSize(_padded));
  size = ((size + 31) / 32) * 32;
  return size;
}
...
unsigned ArrayType::calldataEncodedSize(bool _padded) const {
  ...
  solAssert(size <= numeric_limits<unsigned>::max(),
            "Array size does not fit unsigned.");
...
u256 ArrayType::storageSize() const {
  ...
  if (size >= bigint(1) << 256)
    BOOST_THROW_EXCEPTION(Error(Error::Type::TypeError)
                          << errinfo_comment("Array too large for storage."));
...
string ArrayType::signatureInExternalFunction(bool _structsByName) const {
  if (isByteArray())
    ...
    solAssert(baseType(), "");
    return baseType()->signatureInExternalFunction(_structsByName) + "[" +
           (isDynamicallySized() ? "" : length().str()) + "]";
  }
}

MemberList::MemberMap
ArrayType::nativeMembers(ContractDefinition const *) const {
  MemberList::MemberMap members;
  if (!isString()) {
    members.push_back({"length", make_shared<IntegerType>(256)});
    if (isDynamicallySized() && location() == DataLocation::Storage)
      members.push_back(
          {"push", make_shared<FunctionType>(
                       TypePointers{baseType()},
                       TypePointers{make_shared<IntegerType>(256)},
                       strings{string()}, strings{string()},
                       isByteArray() ? FunctionType::Kind::ByteArrayPush
...
TypePointer ArrayType::interfaceType(bool _inLibrary) const {
  // Note: This has to fulfill canBeUsedExternally(_inLibrary) ==
  // !!interfaceType(_inLibrary)
...
bool ArrayType::canBeUsedExternally(bool _inLibrary) const {
  ...
  if (_inLibrary && location() == DataLocation::Storage)
    ...
    return false;
  else if (m_baseType->category() == Category::Array &&
           m_baseType->isDynamicallySized())
    return false;
  else
    return true;
}
...
u256 ArrayType::memorySize() const {
  ...
  solAssert(size <= numeric_limits<unsigned>::max(),
            "Array size does not fit u256.");
  return u256(size);
}

TypePointer ArrayType::copyForLocation(DataLocation _location,
                                       bool _isPointer) const {
...
string ContractType::identifier() const {
  return (m_super ? "t_super" : "t_contract") +
         parenthesizeUserIdentifier(m_contract.name()) +
         std::to_string(m_contract.id());
}
...
string ContractType::toString(bool) const {
  return string(m_contract.isLibrary() ? "library " : "contract ") +
         string(m_super ? "super " : "") + m_contract.name();
}
...
MemberList::MemberMap
ContractType::nativeMembers(ContractDefinition const *_contract) const {
  MemberList::MemberMap members;
  solAssert(_contract, "");
  if (m_super) {
    // add the most derived of all functions which are visible in derived
    // contracts
    auto bases = m_contract.annotation().linearizedBaseContracts;
    solAssert(bases.size() >= 1, "linearizedBaseContracts should at least "
                                 "contain the most derived contract.");
    // `sliced(1, ...)` ignores the most derived contract, which should not be
    // searchable from `super`.
    for (ContractDefinition const *base :
         bases | boost::adaptors::sliced(1, bases.size()))
      for (FunctionDefinition const *function : base->definedFunctions()) {
        ...
        for (auto const &member : members) {
          if (member.name != function->name())
            continue;
          auto memberType =
              dynamic_cast<FunctionType const *>(member.type.get());
        ...
        if (!functionWithEqualArgumentsFound)
          members.push_back(
              MemberList::Member(function->name(), functionType, function));
      }
...
void ContractType::addNonConflictingAddressMembers(
    MemberList::MemberMap &_members) {
  MemberList::MemberMap addressMembers =
      IntegerType(160, IntegerType::Modifier::Address).nativeMembers(nullptr);
  for (auto const &addressMember : addressMembers) {
      ...
      if (member.name == addressMember.name &&
          (
              ...
              (member.type->category() == Type::Category::Function &&
               dynamic_cast<FunctionType const &>(*member.type)
                   .hasEqualArgumentTypes(dynamic_cast<FunctionType const &>(
                       *addressMember.type))))) {
        clash = true;
        break;
      }
    ...
    if (!clash)
      _members.push_back(MemberList::Member(
          addressMember.name, addressMember.type, addressMember.declaration));
  }
}
...
vector<tuple<VariableDeclaration const *, u256, unsigned>>
ContractType::stateVariables() const {
  vector<VariableDeclaration const *> variables;
  for (ContractDefinition const *contract : boost::adaptors::reverse(
           m_contract.annotation().linearizedBaseContracts))
  ...
  for (size_t index = 0; index < variables.size(); ++index)
    if (auto const *offset = offsets.offset(index))
      variablesAndOffsets.push_back(
          make_tuple(variables[index], offset->first, offset->second));
...
string StructType::identifier() const {
  return "t_struct" + parenthesizeUserIdentifier(m_struct.name()) +
         std::to_string(m_struct.id()) + identifierLocationSuffix();
}
...
MemberList::MemberMap
StructType::nativeMembers(ContractDefinition const *) const {
  MemberList::MemberMap members;
  for (ASTPointer<VariableDeclaration> const &variable : m_struct.members()) {
    ...
    if (location() != DataLocation::Storage && !type->canLiveOutsideStorage())
      continue;
    members.push_back(MemberList::Member(
        variable->name(), copyForLocationIfReference(type), variable.get()));
  }
  return members;
}
...
TypePointer StructType::interfaceType(bool _inLibrary) const {
  ...
  // Has to fulfill canBeUsedExternally(_inLibrary) ==
  // !!interfaceType(_inLibrary)
  if (_inLibrary && location() == DataLocation::Storage)
    return shared_from_this();
  else
...
bool StructType::canBeUsedExternally(bool _inLibrary) const {
  if (_inLibrary && location() == DataLocation::Storage)
    ...
    // We pass "false" to canBeUsedExternally (_inLibrary), because this struct
    // will be passed by value and thus the encoding does not differ, but it
    // will disallow mappings.
...
TypePointer StructType::copyForLocation(DataLocation _location,
                                        bool _isPointer) const {
...
string StructType::signatureInExternalFunction(bool _structsByName) const {
  if (_structsByName)
    ...
    auto memberTypeStrings =
        memberTypes |
        boost::adaptors::transformed([&](TypePointer _t) -> string {
...
FunctionTypePointer StructType::constructorType() const {
  ...
  for (auto const &member : members(nullptr)) {
    ...
    paramTypes.push_back(
        copyForLocationIfReference(DataLocation::Memory, member.type));
  }
  return make_shared<FunctionType>(
      paramTypes, TypePointers{copyForLocation(DataLocation::Memory, false)},
      paramNames, strings(), FunctionType::Kind::Internal);
}

pair<u256, unsigned> const &
StructType::storageOffsetsOfMember(string const &_name) const {
...
bool StructType::recursive() const {
  if (!m_recursive.is_initialized()) {
    ...
    function<bool(StructType const *)> check =
        [&](StructType const *t) -> bool {
      ...
      for (ASTPointer<VariableDeclaration> const &variable : str->members()) {
        ...
        while (dynamic_cast<ArrayType const *>(memberType))
          memberType =
              dynamic_cast<ArrayType const *>(memberType)->baseType().get();
        if (StructType const *innerStruct =
                dynamic_cast<StructType const *>(memberType))
...
string EnumType::identifier() const {
  return "t_enum" + parenthesizeUserIdentifier(m_enum.name()) +
         std::to_string(m_enum.id());
}
...
size_t EnumType::numberOfMembers() const { return m_enum.members().size(); };
...
bool TupleType::isImplicitlyConvertibleTo(Type const &_other) const {
  if (auto tupleType = dynamic_cast<TupleType const *>(&_other)) {
    ...
    if (components().size() != targets.size() && !targets.front() &&
        !targets.back())
...
TypePointer
TupleType::closestTemporaryType(TypePointer const &_targetType) const {
  solAssert(!!_targetType, "");
  TypePointers const &targetComponents =
      dynamic_cast<TupleType const &>(*_targetType).components();
  bool fillRight = !targetComponents.empty() &&
                   (!targetComponents.back() || targetComponents.front());
  TypePointers tempComponents(targetComponents.size());
  for (size_t i = 0; i < min(targetComponents.size(), components().size());
       ++i) {
    ...
    if (components()[si] && targetComponents[ti]) {
      tempComponents[ti] =
          components()[si]->closestTemporaryType(targetComponents[ti]);
...
FunctionType::FunctionType(VariableDeclaration const &_varDecl)
    : m_kind(Kind::External), m_stateMutability(StateMutability::View),
  ...
  while (true) {
    if (auto mappingType =
            dynamic_cast<MappingType const *>(returnType.get())) {
      paramTypes.push_back(mappingType->keyType());
      paramNames.push_back("");
      returnType = mappingType->valueType();
    } else if (auto arrayType =
                   dynamic_cast<ArrayType const *>(returnType.get())) {
  ...
  if (auto structType = dynamic_cast<StructType const *>(returnType.get())) {
    ...
    retParams.push_back(ReferenceType::copyForLocationIfReference(
        DataLocation::Memory, returnType));
...
FunctionType::FunctionType(EventDefinition const &_event)
    : m_kind(Kind::Event), m_stateMutability(StateMutability::NonPayable),
...
FunctionType::FunctionType(FunctionTypeName const &_typeName)
    : m_kind(_typeName.visibility() == VariableDeclaration::Visibility::External
                 ? Kind::External
                 : Kind::Internal),
  ...
  for (auto const &t : _typeName.parameterTypes()) {
    ...
    if (m_kind == Kind::External)
      solAssert(t->annotation().type->canBeUsedExternally(false),
...
FunctionTypePointer
FunctionType::newExpressionType(ContractDefinition const &_contract) {
  FunctionDefinition const *constructor = _contract.constructor();
  TypePointers parameters;
  strings parameterNames;
  StateMutability stateMutability = StateMutability::NonPayable;

  solAssert(_contract.contractKind() !=
  ...
  return make_shared<FunctionType>(
      parameters, TypePointers{make_shared<ContractType>(_contract)},
      parameterNames, strings{""}, Kind::Creation, false, stateMutability);
}
...
string FunctionType::identifier() const {
  ...
  case Kind::Require:
    ...
    break;
  default:
    solAssert(false, "Unknown function location.");
    break;
  }
  id += "_" + stateMutabilityToString(m_stateMutability);
  id += identifierList(m_parameterTypes) + "returns" +
        identifierList(m_returnParameterTypes);
  if (m_gasSet)
    id += "gas";
  if (m_valueSet)
    id += "value";
...
bool FunctionType::operator==(Type const &_other) const {
  ...
  if (m_kind != other.m_kind || m_stateMutability != other.stateMutability() ||
      m_parameterTypes.size() != other.m_parameterTypes.size() ||
      m_returnParameterTypes.size() != other.m_returnParameterTypes.size())
    return false;

  auto typeCompare = [](TypePointer const &_a, TypePointer const &_b) -> bool {
    return *_a == *_b;
  };
...
TypePointer
FunctionType::binaryOperatorResult(Token::Value _operator,
                                   TypePointer const &_other) const {
  if (_other->category() != category() ||
      !(_operator == Token::Equal || _operator == Token::NotEqual))
    return TypePointer();
  FunctionType const &other = dynamic_cast<FunctionType const &>(*_other);
  if (kind() == Kind::Internal && other.kind() == Kind::Internal &&
      sizeOnStack() == 1 && other.sizeOnStack() == 1)
...
string FunctionType::toString(bool _short) const {
  ...
  for (auto it = m_parameterTypes.begin(); it != m_parameterTypes.end(); ++it)
    name +=
        (*it)->toString(_short) + (it + 1 == m_parameterTypes.end() ? "" : ",");
  name += ")";
  ...
  if (!m_returnParameterTypes.empty()) {
    name += " returns (";
    for (auto it = m_returnParameterTypes.begin();
         it != m_returnParameterTypes.end(); ++it)
      name += (*it)->toString(_short) +
              (it + 1 == m_returnParameterTypes.end() ? "" : ",");
    name += ")";
  }
  return name;
}
...
unsigned FunctionType::sizeOnStack() const {
  ...
  if (m_kind == Kind::SetGas || m_kind == Kind::SetValue) {
    ...
    kind = dynamic_cast<FunctionType const &>(*m_returnParameterTypes.front())
               .m_kind;
  }
...
FunctionTypePointer FunctionType::interfaceFunctionType() const {
  ...
  solAssert(m_declaration,
            "Declaration needed to determine interface function type.");
  bool isLibraryFunction =
      dynamic_cast<ContractDefinition const &>(*m_declaration->scope())
          .isLibrary();
  ...
  return make_shared<FunctionType>(
      paramTypes, retParamTypes, m_parameterNames, m_returnParameterNames,
      m_kind, m_arbitraryParameters, m_stateMutability, m_declaration);
}

MemberList::MemberMap
FunctionType::nativeMembers(ContractDefinition const *) const {
  switch (m_kind) {
  case Kind::External:
  case Kind::Creation:
  case Kind::BareCall:
  ...
  case Kind::BareDelegateCall: {
    ...
    if (m_kind == Kind::External)
      members.push_back(
          MemberList::Member("selector", make_shared<FixedBytesType>(4)));
      ...
      if (isPayable())
        members.push_back(MemberList::Member(
            ...
            make_shared<FunctionType>(
                ...
                TypePointers{copyAndSetGasOrValue(false, true)}, strings(),
                strings(), Kind::SetValue, false, StateMutability::NonPayable,
                nullptr, m_gasSet, m_valueSet)));
    }
    if (m_kind != Kind::Creation)
      members.push_back(MemberList::Member(
          ...
          make_shared<FunctionType>(
              parseElementaryTypeVector({"uint"}),
              TypePointers{copyAndSetGasOrValue(true, false)}, strings(),
              strings(), Kind::SetGas, false, StateMutability::NonPayable,
              nullptr, m_gasSet, m_valueSet)));
    return members;
  }
  default:
...
TypePointer FunctionType::encodingType() const {
  // Only external functions can be encoded, internal functions cannot leave
  // code boundaries.
  if (m_kind == Kind::External)
    return shared_from_this();
  else
...
bool FunctionType::canTakeArguments(TypePointers const &_argumentTypes,
                                    TypePointer const &_selfType) const {
...
bool FunctionType::hasEqualArgumentTypes(FunctionType const &_other) const {
  if (m_parameterTypes.size() != _other.m_parameterTypes.size())
    return false;
  return equal(m_parameterTypes.cbegin(), m_parameterTypes.cend(),
...
string FunctionType::externalSignature() const {
  solAssert(m_declaration != nullptr,
            "External signature of function needs declaration");
  solAssert(!m_declaration->name().empty(),
            "Fallback function has no signature.");

  bool const inLibrary =
      dynamic_cast<ContractDefinition const &>(*m_declaration->scope())
          .isLibrary();
  ...
  return m_declaration->name() + "(" +
         boost::algorithm::join(typeStrings, ",") + ")";
}
...
bool FunctionType::isPure() const {
  ...
  return m_kind == Kind::SHA3 || m_kind == Kind::ECRecover ||
         m_kind == Kind::SHA256 || m_kind == Kind::RIPEMD160 ||
         m_kind == Kind::AddMod || m_kind == Kind::MulMod ||
...
TypePointer FunctionType::copyAndSetGasOrValue(bool _setGas,
                                               bool _setValue) const {
  return make_shared<FunctionType>(
      m_parameterTypes, m_returnParameterTypes, m_parameterNames,
      m_returnParameterNames, m_kind, m_arbitraryParameters, m_stateMutability,
      m_declaration, m_gasSet || _setGas, m_valueSet || _setValue, m_bound);
}

FunctionTypePointer FunctionType::asMemberFunction(bool _inLibrary,
                                                   bool _bound) const {
  ...
  for (auto const &t : m_parameterTypes) {
    ...
    if (refType && refType->location() == DataLocation::CallData)
      parameterTypes.push_back(
          refType->copyForLocation(DataLocation::Memory, true));
    else
  ...
  return make_shared<FunctionType>(
      parameterTypes, returnParameterTypes, m_parameterNames,
      m_returnParameterNames, kind, m_arbitraryParameters, m_stateMutability,
      m_declaration, m_gasSet, m_valueSet, _bound);
}
...
string MappingType::toString(bool _short) const {
  return "mapping(" + keyType()->toString(_short) + " => " +
         valueType()->toString(_short) + ")";
}

string MappingType::canonicalName() const {
  return "mapping(" + keyType()->canonicalName() + " => " +
...
unsigned TypeType::sizeOnStack() const {
  if (auto contractType =
          dynamic_cast<ContractType const *>(m_actualType.get()))
...
MemberList::MemberMap
TypeType::nativeMembers(ContractDefinition const *_currentScope) const {
  MemberList::MemberMap members;
  if (m_actualType->category() == Category::Contract) {
    ContractDefinition const &contract =
        dynamic_cast<ContractType const &>(*m_actualType).contractDefinition();
    bool isBase = false;
    if (_currentScope != nullptr) {
      auto const &currentBases =
          _currentScope->annotation().linearizedBaseContracts;
      isBase = (find(currentBases.begin(), currentBases.end(), &contract) !=
                currentBases.end());
    }
    if (contract.isLibrary())
      for (FunctionDefinition const *function : contract.definedFunctions())
        if (function->isVisibleInDerivedContracts())
          members.push_back(MemberList::Member(
              function->name(), FunctionType(*function).asMemberFunction(true),
              function));
    if (isBase) {
      // We are accessing the type of a base contract, so add all public and
      // protected members. Note that this does not add inherited functions on
      // purpose.
    ...
    EnumDefinition const &enumDef =
        dynamic_cast<EnumType const &>(*m_actualType).enumDefinition();
...
bool ModuleType::operator==(Type const &_other) const {
  if (_other.category() != category())
    return false;
  return &m_sourceUnit ==
         &dynamic_cast<ModuleType const &>(_other).m_sourceUnit;
}

MemberList::MemberMap
ModuleType::nativeMembers(ContractDefinition const *) const {
  MemberList::MemberMap symbols;
  for (auto const &symbolName : m_sourceUnit.annotation().exportedSymbols)
    for (Declaration const *symbol : symbolName.second)
      symbols.push_back(
          MemberList::Member(symbolName.first, symbol->type(), symbol));
  return symbols;
}
...
MemberList::MemberMap
MagicType::nativeMembers(ContractDefinition const *) const {
  switch (m_kind) {
  case Kind::Block:
    return MemberList::MemberMap(
        {{"coinbase",
         ...
         {"blockhash",
          make_shared<FunctionType>(strings{"uint"}, strings{"bytes32"},
                                    FunctionType::Kind::BlockHash, false,
