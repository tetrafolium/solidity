...
enum class CheckResult { SATISFIABLE, UNSATISFIABLE, UNKNOWN, ERROR };
...
public:
explicit Expression(bool _v)
	: name(_v ? "true" : "false"), sort(Sort::Bool) {
}
...
static Expression ite(Expression _condition, Expression _trueValue,
                      Expression _falseValue) {
	solAssert(_trueValue.sort == _falseValue.sort, "");
	return Expression("ite",
	                  std::vector<Expression>{std::move(_condition),
	                                          std::move(_trueValue),
...
Expression operator()(Expression _a) const {
	solAssert(sort == Sort::IntIntFun && arguments.empty(),
...
private:
/// Manual constructor, should only be used by SolverInterface and this class
/// itself.
...
Expression(std::string _name, Expression _arg1, Expression _arg2, Sort _sort)
	: Expression(std::move(_name),
	             std::vector<Expression>{
					std::move(_arg1), std::move(_arg2)
				},
	             _sort) {}
};
...
virtual Expression newFunction(std::string _name, Sort _domain,
                               Sort _codomain) {
	solAssert(_domain == Sort::Int && _codomain == Sort::Int,
	          "Function sort not supported.");
