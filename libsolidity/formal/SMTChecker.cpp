...
SMTChecker::SMTChecker(ErrorReporter &_errorReporter,
                       ReadCallback::Callback const &_readFileCallback)
	:
#ifdef HAVE_Z3
...
void SMTChecker::analyze(SourceUnit const &_source) {
	...
	if (_source.annotation().experimentalFeatures.count(
...
bool SMTChecker::visit(FunctionDefinition const &_function) {
	if (!_function.modifiers().empty() || _function.isConstructor())
		m_errorReporter.warning(_function.location(),
		                        "Assertion checker does not yet support "
		                        "constructors and functions with modifiers.");
	m_currentFunction = &_function;
	// We only handle local variables, so we clear at the beginning of the
	// function. If we add storage variables, those should be cleared differently.
	m_interface->reset();
...
bool SMTChecker::visit(IfStatement const &_node) {
	...
	vector<Declaration const *> touchedVariables =
		m_variableUsage->touchedVariables(_node.trueStatement());
	if (_node.falseStatement()) {
		visitBranch(*_node.falseStatement(), !expr(_node.condition()));
		touchedVariables +=
			m_variableUsage->touchedVariables(*_node.falseStatement());
	}
...
bool SMTChecker::visit(WhileStatement const &_node) {
	...
	if (_node.isDoWhile()) {
		...
		// TODO the assertions generated in the body should still be active in the
		// condition
		_node.condition().accept(*this);
		checkBooleanNotConstant(_node.condition(),
...
bool SMTChecker::visit(ForStatement const &_node) {
	...
	if (_node.loopExpression())
		touchedVariables +=
			m_variableUsage->touchedVariables(*_node.loopExpression());
	// Remove duplicates
	std::sort(touchedVariables.begin(), touchedVariables.end());
	touchedVariables.erase(
		std::unique(touchedVariables.begin(), touchedVariables.end()),
		touchedVariables.end());
	...
	if (_node.condition()) {
		_node.condition()->accept(*this);
		checkBooleanNotConstant(*_node.condition(),
		                        "For loop condition is always $VALUE.");
	}
...
void SMTChecker::endVisit(Assignment const &_assignment) {
	if (_assignment.assignmentOperator() != Token::Value::Assign)
		...
		m_errorReporter.warning(_assignment.location(),
		                        "Assertion checker does not yet implement type " +
		                        _assignment.annotation().type->toString());
	else if (Identifier const *identifier =
...
void SMTChecker::checkUnderOverflow(smt::Expression _value,
                                    IntegerType const &_type,
                                    SourceLocation const &_location) {
	checkCondition(_value < minValue(_type), _location,
	               "Underflow (resulting value less than " +
	               formatNumber(_type.minValue()) + ")",
	               "value", &_value);
	checkCondition(_value > maxValue(_type), _location,
	               "Overflow (resulting value larger than " +
	               formatNumber(_type.maxValue()) + ")",
	               "value", &_value);
}
...
void SMTChecker::endVisit(UnaryOperation const &_op) {
	...
	case Token::Dec: // -- (pre- or postfix)
	{
		...
		if (Identifier const *identifier =
			    dynamic_cast<Identifier const *>(&_op.subExpression())) {
			Declaration const *decl = identifier->annotation().referencedDeclaration;
			if (knownVariable(*decl)) {
				auto innerValue = currentValue(*decl);
				auto newValue =
					_op.getOperator() == Token::Inc ? innerValue + 1 : innerValue - 1;
			...
			m_errorReporter.warning(_op.location(), "Assertion checker does not yet "
			                        "implement such increments / "
			                        "decrements.");
		break;
	}
	case Token::Add: // +
	...
	case Token::Sub: // -
	{
		...
		if (auto intType =
			    dynamic_cast<IntegerType const *>(_op.annotation().type.get()))
...
void SMTChecker::endVisit(FunctionCall const &_funCall) {
	...
	if (funType.kind() == FunctionType::Kind::Assert) {
		...
		solAssert(args[0]->annotation().type->category() == Type::Category::Bool,
		          "");
		checkCondition(!(expr(*args[0])), _funCall.location(),
		               "Assertion violation");
...
void SMTChecker::endVisit(Identifier const &_identifier) {
	...
	} else if (dynamic_cast<IntegerType const *>(
			   _identifier.annotation().type.get()))
		defineExpr(_identifier, currentValue(*decl));
	else if (FunctionType const *fun = dynamic_cast<FunctionType const *>(
			 _identifier.annotation().type.get())) {
...
void SMTChecker::endVisit(Literal const &_literal) {
	...
	if (type.category() == Type::Category::Integer ||
		...
		defineExpr(
			_literal,
			smt::Expression(_literal.token() == Token::TrueLiteral ? true : false));
	else
		m_errorReporter.warning(
			_literal.location(),
...
void SMTChecker::arithmeticOperation(BinaryOperation const &_op) {
	...
	case Token::Div: {
		...
		solAssert(
			_op.annotation().commonType->category() == Type::Category::Integer, "");
		auto const &intType =
			dynamic_cast<IntegerType const &>(*_op.annotation().commonType);
		...
		if (_op.getOperator() == Token::Div) {
			checkCondition(right == 0, _op.location(), "Division by zero", "value",
			               &right);
...
void SMTChecker::compareOperation(BinaryOperation const &_op) {
	...
	if (_op.annotation().commonType->category() == Type::Category::Integer) {
		...
		m_errorReporter.warning(
			_op.location(), "Assertion checker does not yet implement the type " +
			_op.annotation().commonType->toString() +
			" for comparisons");
}
...
void SMTChecker::booleanOperation(BinaryOperation const &_op) {
	...
	if (_op.annotation().commonType->category() == Type::Category::Bool) {
		...
		if (_op.getOperator() == Token::And)
			defineExpr(_op,
			           expr(_op.leftExpression()) && expr(_op.rightExpression()));
		else
			defineExpr(_op,
			           expr(_op.leftExpression()) || expr(_op.rightExpression()));
	} else
...
smt::Expression SMTChecker::division(smt::Expression _left,
                                     smt::Expression _right,
                                     IntegerType const &_type) {
	// Signed division in SMTLIB2 rounds differently for negative division.
	if (_type.isSigned())
		return (smt::Expression::ite(
				_left >= 0,
				smt::Expression::ite(_right >= 0, _left / _right,
				                     0 - (_left / (0 - _right))),
				smt::Expression::ite(_right >= 0, 0 - ((0 - _left) / _right),
				                     (0 - _left) / (0 - _right))));
	else
		return _left / _right;
}

void SMTChecker::assignment(Declaration const &_variable,
...
void SMTChecker::visitBranch(Statement const &_statement,
...
void SMTChecker::checkCondition(smt::Expression _condition,
                                SourceLocation const &_location,
	...
	if (m_conditionalExecutionHappened)
		conditionalComment = "\nNote that some information is erased after "
		                     "conditional execution of parts of the code.\n"
...
void SMTChecker::checkBooleanNotConstant(Expression const &_condition,
                                         string const &_description) {
	...
	if (positiveResult == smt::CheckResult::ERROR ||
	    negatedResult == smt::CheckResult::ERROR)
		m_errorReporter.warning(_condition.location(),
		                        "Error trying to invoke SMT solver.");
	else if (positiveResult == smt::CheckResult::SATISFIABLE &&
	         negatedResult == smt::CheckResult::SATISFIABLE) {
		// everything fine.
	} else if (positiveResult == smt::CheckResult::UNSATISFIABLE &&
	           negatedResult == smt::CheckResult::UNSATISFIABLE)
...
SMTChecker::checkSatisifableAndGenerateModel(
	vector<smt::Expression> const &_expressionsToEvaluate) {
	smt::CheckResult result;
	vector<string> values;
	try {
...
bool SMTChecker::createVariable(VariableDeclaration const &_varDecl) {
	if (dynamic_cast<IntegerType const *>(_varDecl.type().get())) {
		...
		m_variables.emplace(
			&_varDecl, m_interface->newFunction(uniqueSymbol(_varDecl),
			                                    smt::Sort::Int, smt::Sort::Int));
		return true;
	} else {
		m_errorReporter.warning(
			_varDecl.location(),
...
smt::Expression SMTChecker::valueAtSequence(const Declaration &_decl,
                                            int _sequence) {
...
smt::Expression SMTChecker::expr(Expression const &_e) {
	if (!m_expressions.count(&_e)) {
		m_errorReporter.warning(
			_e.location(), "Internal error: Expression undefined for SMT solver.");
		createExpr(_e);
	}
...
void SMTChecker::createExpr(Expression const &_e) {
	if (m_expressions.count(&_e))
		m_errorReporter.warning(
			_e.location(),
			"Internal error: Expression created twice in SMT solver.");
	else {
		solAssert(_e.annotation().type, "");
		switch (_e.annotation().type->category()) {
		case Type::Category::RationalNumber: {
			if (RationalNumberType const *rational =
				    dynamic_cast<RationalNumberType const *>(
					    _e.annotation().type.get()))
				solAssert(!rational->isFractional(), "");
