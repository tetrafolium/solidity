...
Expression SMTLib2Interface::newFunction(string _name, Sort _domain,
                                         Sort _codomain) {
  write("(declare-fun |" + _name + "| (" +
...
pair<CheckResult, vector<string>>
SMTLib2Interface::check(vector<Expression> const &_expressionsToEvaluate) {
  string response =
      querySolver(boost::algorithm::join(m_accumulatedOutput, "\n") +
...
string SMTLib2Interface::checkSatAndGetValuesCommand(
  ...
  if (_expressionsToEvaluate.empty())
    ...
    for (size_t i = 0; i < _expressionsToEvaluate.size(); i++) {
      ...
      solAssert(e.sort == Sort::Int || e.sort == Sort::Bool,
                "Invalid sort for expression to evaluate.");
      command += "(declare-const |EVALEXPR_" + to_string(i) + "| " +
                 (e.sort == Sort::Int ? "Int" : "Bool") + "\n";
      command +=
          "(assert (= |EVALEXPR_" + to_string(i) + "| " + toSExpr(e) + "))\n";
    }
    command += "(check-sat)\n";
    command += "(get-value (";
    for (size_t i = 0; i < _expressionsToEvaluate.size(); i++)
...
vector<string> SMTLib2Interface::parseValues(string::const_iterator _start,
...
string SMTLib2Interface::querySolver(string const &_input) {
  if (!m_queryCallback)
    BOOST_THROW_EXCEPTION(SolverError()
                          << errinfo_comment("No SMT solver available."));

  ReadCallback::Result queryResult = m_queryCallback(_input);
  if (!queryResult.success)
    BOOST_THROW_EXCEPTION(
        SolverError() << errinfo_comment(queryResult.responseOrErrorMessage));
