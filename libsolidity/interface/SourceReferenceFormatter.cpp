 ...
 * @date 2014
 * Formatting functions for errors referencing positions and locations in the
 * source.
 */
...
void SourceReferenceFormatter::printSourceLocation(
	ostream &_stream, SourceLocation const *_location,
	...
	tie(startLine, startColumn) =
		scanner.translatePositionToLineColumn(_location->start);
	int endLine;
	int endColumn;
	tie(endLine, endColumn) =
		scanner.translatePositionToLineColumn(_location->end);
	if (startLine == endLine) {
		string line = scanner.lineAtPosition(_location->start);

		int locationLength = endColumn - startColumn;
		if (locationLength > 150) {
			line = line.substr(0, startColumn + 35) + " ... " +
			       line.substr(endColumn - 35);
		...
		_stream << line << endl;
		for_each(
			line.cbegin(), line.cbegin() + startColumn,
		...
		_stream << scanner.lineAtPosition(_location->start) << endl
		        << string(startColumn, ' ') << "^\n"
...
void SourceReferenceFormatter::printSourceName(
	...
	tie(startLine, startColumn) =
		scanner.translatePositionToLineColumn(_location->start);
	_stream << *_location->sourceName << ":" << (startLine + 1) << ":"
	        << (startColumn + 1) << ": ";
}

void SourceReferenceFormatter::printExceptionInformation(
	ostream &_stream, Exception const &_exception, string const &_name,
	function<Scanner const &(string const &)> const &_scannerFromSourceName) {
	SourceLocation const *location =
		boost::get_error_info<errinfo_sourceLocation>(_exception);
	auto secondarylocation =
		boost::get_error_info<errinfo_secondarySourceLocation>(_exception);
