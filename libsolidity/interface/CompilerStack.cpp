...
void CompilerStack::setRemappings(vector<string> const &_remappings) {
  ...
  for (auto const &remapping : _remappings) {
    ...
    r.prefix =
        colon == eq ? string(remapping.begin(), eq) : string(colon + 1, eq);
...
bool CompilerStack::parse() {
  ...
  if (SemVerVersion{string(VersionString)}.isPrerelease())
    m_errorReporter.warning("This is a pre-release compiler version, please do "
  ...
  for (size_t i = 0; i < sourcesToParse.size(); ++i) {
    ...
    if (!source.ast)
      solAssert(!Error::containsOnlyWarnings(m_errorReporter.errors()),
      ...
      for (auto const &newSource : loadMissingSources(*source.ast, path)) {
        ...
        m_sources[newPath].scanner =
            make_shared<Scanner>(CharStream(newContents), newPath);
...
bool CompilerStack::analyze() {
  ...
  NameAndTypeResolver resolver(m_globalContext->declarations(), m_scopes,
                               m_errorReporter);
  for (Source const *source : m_sourceOrder)
  ...
  for (Source const *source : m_sourceOrder)
    for (ASTPointer<ASTNode> const &node : source->ast->nodes())
      if (ContractDefinition *contract =
        ...
        if (!resolver.updateDeclaration(*m_globalContext->currentThis()))
          return false;
        if (!resolver.updateDeclaration(*m_globalContext->currentSuper()))
          return false;
        if (!resolver.resolveNamesAndTypes(*contract))
          return false;

        // Note that we now reference contracts by their fully qualified names,
        // and thus contracts can only conflict if declared in the same source
        // file.  This already causes a double-declaration error elsewhere, so
        // we do not report an error here and instead silently drop any
        // additional contracts we find.

        if (m_contracts.find(contract->fullyQualifiedName()) ==
            m_contracts.end())
...
bool CompilerStack::parseAndAnalyze() { return parse() && analyze(); }

bool CompilerStack::isRequestedContract(
    ContractDefinition const &_contract) const {
  return m_requestedContractNames.empty() ||
...
vector<string> CompilerStack::contractNames() const {
  if (m_stackState < AnalysisSuccessful)
    BOOST_THROW_EXCEPTION(CompilerError()
                          << errinfo_comment("Parsing was not successful."));
...
eth::AssemblyItems const *
CompilerStack::assemblyItems(string const &_contractName) const {
  Contract const &currentContract = contract(_contractName);
  return currentContract.compiler
             ? &contract(_contractName).compiler->assemblyItems()
             : nullptr;
}

eth::AssemblyItems const *
CompilerStack::runtimeAssemblyItems(string const &_contractName) const {
  Contract const &currentContract = contract(_contractName);
  return currentContract.compiler
             ? &contract(_contractName).compiler->runtimeAssemblyItems()
             : nullptr;
}
...
string const *
CompilerStack::runtimeSourceMapping(string const &_contractName) const {
  Contract const &c = contract(_contractName);
...
std::string const
CompilerStack::filesystemFriendlyName(string const &_contractName) const {
  ...
  for (auto const &contract : m_contracts) {
    if (contract.second.contract->name() == matchContract.contract->name() &&
      ...
      std::string friendlyName =
          boost::algorithm::replace_all_copy(_contractName, "/", "_");
...
eth::LinkerObject const &
CompilerStack::object(string const &_contractName) const {
  return contract(_contractName).object;
}

eth::LinkerObject const &
CompilerStack::runtimeObject(string const &_contractName) const {
  return contract(_contractName).runtimeObject;
}

eth::LinkerObject const &
CompilerStack::cloneObject(string const &_contractName) const {
  return contract(_contractName).cloneObject;
}

/// FIXME: cache this string
string CompilerStack::assemblyString(string const &_contractName,
                                     StringMap _sourceCodes) const {
  Contract const &currentContract = contract(_contractName);
...
Json::Value CompilerStack::assemblyJSON(string const &_contractName,
                                        StringMap _sourceCodes) const {
  Contract const &currentContract = contract(_contractName);
  if (currentContract.compiler)
...
Json::Value const &
CompilerStack::contractABI(string const &_contractName) const {
...
Json::Value const &
CompilerStack::natspecUser(string const &_contractName) const {
...
Json::Value const &CompilerStack::natspecUser(Contract const &_contract) const {
  ...
  if (!_contract.userDocumentation)
    _contract.userDocumentation.reset(
        new Json::Value(Natspec::userDocumentation(*_contract.contract)));

  return *_contract.userDocumentation;
}

Json::Value const &
CompilerStack::natspecDev(string const &_contractName) const {
...
Json::Value const &CompilerStack::natspecDev(Contract const &_contract) const {
  ...
  if (!_contract.devDocumentation)
    _contract.devDocumentation.reset(
        new Json::Value(Natspec::devDocumentation(*_contract.contract)));

  return *_contract.devDocumentation;
}

Json::Value
CompilerStack::methodIdentifiers(string const &_contractName) const {
...
string const &CompilerStack::metadata(string const &_contractName) const {
  if (m_stackState != CompilationSuccessful)
    BOOST_THROW_EXCEPTION(
        CompilerError() << errinfo_comment("Compilation was not successful."));
...
Scanner const &CompilerStack::scanner(string const &_sourceName) const {
  if (m_stackState < SourcesSet)
    BOOST_THROW_EXCEPTION(CompilerError()
                          << errinfo_comment("No sources set."));
...
CompilerStack::contractDefinition(string const &_contractName) const {
  if (m_stackState != CompilationSuccessful)
    BOOST_THROW_EXCEPTION(
        CompilerError() << errinfo_comment("Compilation was not successful."));
...
size_t
CompilerStack::functionEntryPoint(std::string const &_contractName,
...
tuple<int, int, int, int> CompilerStack::positionFromSourceLocation(
    SourceLocation const &_sourceLocation) const {
  int startLine;
  int startColumn;
  int endLine;
  int endColumn;
  tie(startLine, startColumn) =
      scanner(*_sourceLocation.sourceName)
          .translatePositionToLineColumn(_sourceLocation.start);
  tie(endLine, endColumn) =
      scanner(*_sourceLocation.sourceName)
          .translatePositionToLineColumn(_sourceLocation.end);
...
StringMap CompilerStack::loadMissingSources(SourceUnit const &_ast,
  ...
  for (auto const &node : _ast.nodes())
    if (ImportDirective const *import =
            dynamic_cast<ImportDirective *>(node.get())) {
      string importPath = absolutePath(import->path(), _sourcePath);
      // The current value of `path` is the absolute path as seen from this
      // source file. We first have to apply remappings before we can store the
      // actual absolute path as seen globally.
      importPath = applyRemapping(importPath, _sourcePath);
      import->annotation().absolutePath = importPath;
      if (m_sources.count(importPath) || newSources.count(importPath))
        continue;

      ReadCallback::Result result{false,
                                  string("File not supplied initially.")};
      if (m_readFile)
      ...
      if (result.success)
        ...
        m_errorReporter.parserError(
            import->location(),
            string("Source \"" + importPath +
                   "\" not found: " + result.responseOrErrorMessage));
        continue;
      }
    }
  return newSources;
...
string CompilerStack::applyRemapping(string const &_path,
                                     string const &_context) {
  // Try to find the longest prefix match in all remappings that are active in
  // the current context.
...
void CompilerStack::resolveImports() {
  // topological sorting (depth first search) of the import graph, cutting
  // potential cycles
...
string CompilerStack::absolutePath(string const &_path,
                                   string const &_reference) const {
...
bool onlySafeExperimentalFeaturesActivated(
    set<ExperimentalFeature> const &features) {
...
void CompilerStack::compileContract(
    ...
    map<ContractDefinition const *, eth::Assembly const *>
        &_compiledContracts) {
  if (_compiledContracts.count(&_contract) ||
  ...
  shared_ptr<Compiler> compiler =
      make_shared<Compiler>(m_optimize, m_optimizeRuns);
  ...
  if (onlySafeExperimentalFeaturesActivated(
          _contract.sourceUnit().annotation().experimentalFeatures))
    cborEncodedMetadata =
        // CBOR-encoding of {"bzzr0": dev::swarmHash(metadata)}
        bytes{0xa1} + cborEncodedHash;
  else
    cborEncodedMetadata =
        // CBOR-encoding of {"bzzr0": dev::swarmHash(metadata), "experimental":
        // true}
        bytes{0xa2} + cborEncodedHash + bytes{0x6c, 'e', 'x', 'p', 'e',
                                              'r',  'i', 'm', 'e', 'n',
                                              't',  'a', 'l', 0xf5};
...
string const CompilerStack::lastContractName() const {
  if (m_contracts.empty())
    BOOST_THROW_EXCEPTION(CompilerError()
                          << errinfo_comment("No compiled contracts found."));
...
CompilerStack::Contract const &
CompilerStack::contract(string const &_contractName) const {
  if (m_contracts.empty())
    BOOST_THROW_EXCEPTION(CompilerError()
                          << errinfo_comment("No compiled contracts found."));
  ...
  // To provide a measure of backward-compatibility, if a contract is not
  // located by its fully-qualified name, a lookup will be attempted purely on
  // the contract's name to see if anything will satisfy.
  ...
  BOOST_THROW_EXCEPTION(CompilerError() << errinfo_comment(
                            "Contract \"" + _contractName + "\" not found."));
}

CompilerStack::Source const &
CompilerStack::source(string const &_sourceName) const {
  auto it = m_sources.find(_sourceName);
  if (it == m_sources.end())
    BOOST_THROW_EXCEPTION(CompilerError()
                          << errinfo_comment("Given source file not found."));
...
string CompilerStack::createMetadata(Contract const &_contract) const {
  ...
  /// All the source files (including self), which should be included in the
  /// metadata.
  set<string> referencedSources;
  referencedSources.insert(_contract.contract->sourceUnit().annotation().path);
  for (auto const sourceUnit :
       _contract.contract->sourceUnit().referencedSourceUnits(true))
  ...
  for (auto const &s : m_sources) {
    ...
    if (m_metadataLiteralSources)
      ...
      meta["sources"][s.first]["urls"].append(
          "bzzr://" +
          toHex(dev::swarmHash(s.second.scanner->source()).asBytes()));
    }
  }
  ...
  for (auto const &library : m_libraries)
    meta["settings"]["libraries"][library.first] =
        "0x" + toHex(library.second.asBytes());
...
string
CompilerStack::computeSourceMapping(eth::AssemblyItems const &_items) const {
  string ret;
  ...
  for (auto const &item : _items) {
    ...
    int length = location.start != -1 && location.end != -1
  ...
  if (eth::AssemblyItems const *items = runtimeAssemblyItems(_contractName)) {
    ...
    for (auto it : contract.interfaceFunctions()) {
      ...
      externalFunctions[sig] =
          gasToJson(GasEstimator::functionalEstimation(*items, sig));
    }

    if (contract.fallbackFunction())
      /// This needs to be set to an invalid signature in order to trigger the
      /// fallback, without the shortcut (of CALLDATSIZE == 0), and therefore to
      /// receive the upper bound. An empty string ("") would work to trigger
      /// the shortcut only.
      externalFunctions[""] =
          gasToJson(GasEstimator::functionalEstimation(*items, "INVALID"));
    ...
    for (auto const &it : contract.definedFunctions()) {
      /// Exclude externally visible functions, constructor and the fallback
      /// function
