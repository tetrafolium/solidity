...
string const dev::solidity::VersionStringStrict =
	...
	(string(SOL_VERSION_PRERELEASE).empty()
	 ? ""
	 : "-" + string(SOL_VERSION_PRERELEASE)) +
	(string(SOL_VERSION_COMMIT).empty() ? ""
	 : "+" + string(SOL_VERSION_COMMIT));
...
bytes dev::solidity::binaryVersion() {
	...
	auto parseDecimal = [&]() {
				    ...
				    for (; i < VersionString.size() && '0' <= VersionString[i] &&
				         VersionString[i] <= '9';
				         ++i)
	...
	++i;
	ret.push_back(byte(parseDecimal()));
	solAssert(i < VersionString.size() &&
	          (VersionString[i] == '-' || VersionString[i] == '+'),
	          "");
	++i;
