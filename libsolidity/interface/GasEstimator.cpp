...
GasEstimator::ASTGasConsumptionSelfAccumulated
GasEstimator::structuralEstimation(AssemblyItems const &_items,
	...
	auto onNode = [&](ASTNode const &_node) {
			      if (!finestNodes.count(&_node))
				      return true;
			      gasCosts[&_node][0] = gasCosts[&_node][1] =
							    particularCosts[_node.location()];
			      return true;
		      };
...
map<ASTNode const *, GasMeter::GasConsumption>
GasEstimator::breakToStatementLevel(
	...
	// first pass: statementDepth[node] is the distance from the deepend statement
	// to node in direction of the tree root (or undefined if not possible)
	...
	auto onEdgeFirstPass = [&](ASTNode const &_parent, ASTNode const &_child) {
				       if (statementDepth.count(&_child))
					       statementDepth[&_parent] =
						       max(statementDepth[&_parent], statementDepth[&_child] + 1);
			       };
	...
	//  - its statement depth is undefined but the parent's statement depth is at
	//  least 1
	...
	auto onEdgeSecondPass = [&](ASTNode const &_parent, ASTNode const &_child) {
					...
					if (statementDepth.count(&_child))
						...
						useNode =
							statementDepth.count(&_parent) && statementDepth.at(&_parent) > 0;
					if (useNode)
...
GasEstimator::functionalEstimation(AssemblyItems const &_items,
	...
	if (!_signature.empty()) {
		...
		Id hashValue = classes.find(
			u256(FixedHash<4>::Arith(FixedHash<4>(dev::keccak256(_signature)))));
		Id calldata =
			classes.find(Instruction::CALLDATALOAD, Ids {classes.find(u256(0))});
		classes.forceEqual(hashValue, Instruction::DIV,
		                   Ids {calldata, classes.find(u256(1) << (8 * 28))});
	}
...
GasEstimator::functionalEstimation(AssemblyItems const &_items,
	...
	// Store an invalid return value on the stack, so that the path estimator
	// breaks upon reaching the return jump.
...
set<ASTNode const *>
GasEstimator::finestNodesAtLocation(vector<ASTNode const *> const &_roots) {
	map<SourceLocation, ASTNode const *> locations;
	set<ASTNode const *> nodes;
	SimpleASTVisitor visitor(function<bool(ASTNode const &)>(),
	                         [&](ASTNode const &_n) {
