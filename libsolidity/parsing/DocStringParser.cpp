...
string::const_iterator skipLineOrEOS(string::const_iterator _nlPos,
                                     string::const_iterator _end) {
	return (_nlPos == _end) ? _end : ++_nlPos;
}

string::const_iterator firstSpaceOrTab(string::const_iterator _pos,
                                       string::const_iterator _end) {
	return boost::range::find_first_of(make_pair(_pos, _end), " \t");
}

string::const_iterator firstWhitespaceOrNewline(string::const_iterator _pos,
                                                string::const_iterator _end) {
	return boost::range::find_first_of(make_pair(_pos, _end), " \t\n");
}

string::const_iterator skipWhitespace(string::const_iterator _pos,
                                      string::const_iterator _end) {
	auto currPos = _pos;
...
bool DocStringParser::parse(string const &_docString,
	...
	while (currPos != end) {
		...
		if (tagPos != end && tagPos < nlPos) {
			...
			if (tagNameEndPos == end) {
				appendError("End of tag " + string(tagPos, tagNameEndPos) +
				            "not found");
				break;
			}

			currPos = parseDocTag(tagNameEndPos + 1, end,
			                      string(tagPos + 1, tagNameEndPos));
...
DocStringParser::iter DocStringParser::parseDocTagLine(iter _pos, iter _end,
                                                       bool _appending) {
	solAssert(!!m_lastTag, "");
...
DocStringParser::iter DocStringParser::parseDocTag(iter _pos, iter _end,
                                                   string const &_tag) {
