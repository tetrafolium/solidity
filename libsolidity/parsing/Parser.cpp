...
explicit ASTNodeFactory(Parser const &_parser)
	: m_parser(_parser),
	m_location(_parser.position(), -1, _parser.sourceName()) {
}
...
void markEndPosition() {
	m_location.end = m_parser.endPosition();
}
void setLocation(SourceLocation const &_location) {
...
void setEndPositionFromNode(ASTPointer<ASTNode> const &_node) {
	m_location.end = _node->location().end;
}
	...
	try {
		...
		while (m_scanner->currentToken() != Token::EOS) {
			...
			case Token::Library:
				...
				break;
			default:
				fatalParserError(string("Expected pragma, import directive or "
...
ASTPointer<PragmaDirective> Parser::parsePragmaDirective() {
	...
	do {
		...
		if (token == Token::Illegal)
			parserError("Token incompatible with Solidity parser as part of pragma "
			            "directive.");
		else {
		...
		m_scanner->next();
	} while (m_scanner->currentToken() != Token::Semicolon &&
	         m_scanner->currentToken() != Token::EOS);
...
ASTPointer<ImportDirective> Parser::parseImportDirective() {
	...
	nodeFactory.markEndPosition();
	expectToken(Token::Semicolon);
	return nodeFactory.createNode<ImportDirective>(path, unitAlias,
	                                               move(symbolAliases));
}

ContractDefinition::ContractKind
Parser::tokenToContractKind(Token::Value _token) {
	switch (_token) {
	case Token::Interface:
...
ASTPointer<ContractDefinition>
Parser::parseContractDefinition(Token::Value _expectedKind) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
	...
	while (true) {
		...
		if (currentTokenValue == Token::RBrace)
			...
			// complicated to distinguish fallback function from function type state
			// variable)
			subNodes.push_back(
				parseFunctionDefinitionOrFunctionTypeStateVariable(name.get()));
		else if (currentTokenValue == Token::Struct)
			subNodes.push_back(parseStructDefinition());
		else if (currentTokenValue == Token::Enum)
			subNodes.push_back(parseEnumDefinition());
		else if (currentTokenValue == Token::Identifier ||
	...
	return nodeFactory.createNode<ContractDefinition>(
		name, docString, baseContracts, subNodes,
...
Parser::parseFunctionHeader(bool _forceEmptyName, bool _allowModifiers) {
	...
	while (true) {
		...
		if (_allowModifiers && token == Token::Identifier) {
			...
			if (m_scanner->peekNextToken() == Token::Semicolon ||
			...
			if (result.visibility != Declaration::Visibility::Default) {
				parserError(string("Visibility already specified as \"" +
			...
			if (result.stateMutability != StateMutability::NonPayable) {
				parserError(string("State mutability already specified as \"" +
...
ASTPointer<ASTNode> Parser::parseFunctionDefinitionOrFunctionTypeStateVariable(
	...
	if (!header.modifiers.empty() || !header.name->empty() ||
		...
		bool const c_isConstructor =
			(_contractName && *header.name == *_contractName);
		return nodeFactory.createNode<FunctionDefinition>(
			header.name, header.visibility, header.stateMutability, c_isConstructor,
			docstring, header.parameters, header.modifiers, header.returnParameters,
			block);
	} else {
		// this has to be a state variable
		ASTPointer<TypeName> type = nodeFactory.createNode<FunctionTypeName>(
			header.parameters, header.returnParameters, header.visibility,
...
ASTPointer<VariableDeclaration> Parser::parseVariableDeclaration(
	...
	ASTNodeFactory nodeFactory = _lookAheadArrayType
	                             ? ASTNodeFactory(*this, _lookAheadArrayType)
				     : ASTNodeFactory(*this);
	...
	while (true) {
		...
		if (_options.isStateVariable &&
		    Token::isVariableVisibilitySpecifier(token)) {
			...
			if (_options.allowIndexed && token == Token::Indexed)
				...
				isDeclaredConst = true;
			         ...
			         Token::isLocationSpecifier(token)) {
				if (location != VariableDeclaration::Location::Default)
					...
					location =
						(token == Token::Memory ? VariableDeclaration::Location::Memory
	...
	return nodeFactory.createNode<VariableDeclaration>(
		type, identifier, value, visibility, _options.isStateVariable, isIndexed,
		isDeclaredConst, location);
}

ASTPointer<ModifierDefinition> Parser::parseModifierDefinition() {
	RecursionGuard recursionGuard(*this);
	ScopeGuard resetModifierFlag([this]() {
				m_insideModifier = false;
			});
	...
	nodeFactory.setEndPositionFromNode(block);
	return nodeFactory.createNode<ModifierDefinition>(name, docstring, parameters,
	                                                  block);
}
...
ASTPointer<TypeName> Parser::parseTypeNameSuffix(ASTPointer<TypeName> type,
...
ASTPointer<Mapping> Parser::parseMapping() {
	...
	if (!Token::isElementaryTypeName(token))
		fatalParserError(
			string("Expected elementary type name for mapping key type"));
	unsigned firstSize;
	unsigned secondSize;
	tie(firstSize, secondSize) = m_scanner->currentTokenInfo();
	ElementaryTypeNameToken elemTypeName(token, firstSize, secondSize);
...
ASTPointer<InlineAssembly>
Parser::parseInlineAssembly(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
...
ASTPointer<IfStatement>
Parser::parseIfStatement(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
	...
	return nodeFactory.createNode<IfStatement>(_docString, condition, trueBody,
	                                           falseBody);
}

ASTPointer<WhileStatement>
Parser::parseWhileStatement(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
...
ASTPointer<WhileStatement>
Parser::parseDoWhileStatement(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
...
ASTPointer<ForStatement>
Parser::parseForStatement(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	ASTNodeFactory nodeFactory(*this);
	...
	return nodeFactory.createNode<ForStatement>(
		_docString, initExpression, conditionExpression, loopExpression, body);
}

ASTPointer<Statement>
Parser::parseSimpleStatement(ASTPointer<ASTString> const &_docString) {
	RecursionGuard recursionGuard(*this);
	...
	if (m_scanner->currentToken() == Token::Identifier)
		...
		ElementaryTypeNameToken elemToken(m_scanner->currentToken(), firstNum,
		                                  secondNum);
		path.push_back(
			ASTNodeFactory(*this).createNode<ElementaryTypeNameExpression>(
				elemToken));
		m_scanner->next();
	}
	...
	if (m_scanner->currentToken() == Token::Identifier ||
	    Token::isLocationSpecifier(m_scanner->currentToken()))
		return parseVariableDeclarationStatement(
			_docString, typeNameIndexAccessStructure(path, indices));
	else
		return parseExpressionStatement(
			_docString, expressionFromIndexAccessStructure(path, indices));
}

ASTPointer<VariableDeclarationStatement>
Parser::parseVariableDeclarationStatement(
	...
	ASTPointer<Expression> value;
	if (!_lookAheadArrayType && m_scanner->currentToken() == Token::Var &&
	    m_scanner->peekNextToken() == Token::LParen) {
		// Parse `var (a, b, ,, c) = ...` into a single VariableDeclarationStatement
			...
			while (true) {
				...
				if (m_scanner->currentToken() != Token::Comma &&
					...
					var = varDeclNodeFactory.createNode<VariableDeclaration>(
						ASTPointer<TypeName>(), name, ASTPointer<Expression>(),
	...
	return nodeFactory.createNode<VariableDeclarationStatement>(_docString,
	                                                            variables, value);
}
...
ASTPointer<ExpressionStatement> Parser::parseExpressionStatement(
	...
	RecursionGuard recursionGuard(*this);
	ASTPointer<Expression> expression =
		parseExpression(_lookAheadIndexAccessStructure);
	return ASTNodeFactory(*this, expression)
	       .createNode<ExpressionStatement>(_docString, expression);
}
...
ASTPointer<Expression> Parser::parseExpression(
	...
	RecursionGuard recursionGuard(*this);
	ASTPointer<Expression> expression =
		parseBinaryExpression(4, _lookAheadIndexAccessStructure);
	if (Token::isAssignmentOp(m_scanner->currentToken())) {
		Token::Value assignmentOperator = expectAssignmentOperator();
		ASTPointer<Expression> rightHandSide = parseExpression();
		ASTNodeFactory nodeFactory(*this, expression);
		nodeFactory.setEndPositionFromNode(rightHandSide);
		return nodeFactory.createNode<Assignment>(expression, assignmentOperator,
		                                          rightHandSide);
		...
		return nodeFactory.createNode<Conditional>(expression, trueExpression,
		                                           falseExpression);
	} else
		return expression;
}
...
ASTPointer<Expression> Parser::parseBinaryExpression(
	...
	RecursionGuard recursionGuard(*this);
	ASTPointer<Expression> expression =
		parseUnaryExpression(_lookAheadIndexAccessStructure);
	ASTNodeFactory nodeFactory(*this, expression);
	...
	for (; precedence >= _minPrecedence; --precedence)
		while (Token::precedence(m_scanner->currentToken()) == precedence) {
			...
			expression =
				nodeFactory.createNode<BinaryOperation>(expression, op, right);
		}
	return expression;
}
...
ASTPointer<Expression> Parser::parseUnaryExpression(
	...
	ASTNodeFactory nodeFactory =
		...
		? ASTNodeFactory(*this, _lookAheadIndexAccessStructure)
		: ASTNodeFactory(*this);
	Token::Value token = m_scanner->currentToken();
	if (!_lookAheadIndexAccessStructure &&
	    (Token::isUnaryOp(token) || Token::isCountOp(token))) {
		// prefix expression
		m_scanner->next();
		...
		ASTPointer<Expression> subExpression =
			parseLeftHandSideExpression(_lookAheadIndexAccessStructure);
	...
	while (true) {
		...
		case Token::Period: {
			...
			expression = nodeFactory.createNode<MemberAccess>(
				expression, expectIdentifierToken());
			break;
		}
		case Token::LParen: {
			m_scanner->next();
			...
			expectToken(Token::RParen);
			expression =
				nodeFactory.createNode<FunctionCall>(expression, arguments, names);
			break;
		}
		default:
			return expression;
...
ASTPointer<Expression> Parser::parsePrimaryExpression() {
	...
	case Token::Number:
		if (Token::isEtherSubdenomination(m_scanner->peekNextToken())) {
			...
			nodeFactory.markEndPosition();
			Literal::SubDenomination subdenomination =
				static_cast<Literal::SubDenomination>(m_scanner->currentToken());
			m_scanner->next();
			expression =
				nodeFactory.createNode<Literal>(token, literal, subdenomination);
			...
			nodeFactory.markEndPosition();
			expression =
				nodeFactory.createNode<Literal>(token, getLiteralAndAdvance());
		}
		break;
	...
	case Token::LBrack: {
		...
		Token::Value oppositeToken =
			(token == Token::LParen ? Token::RParen : Token::RBrack);
		bool isArray = (token == Token::LBrack);

		if (m_scanner->currentToken() != oppositeToken)
			while (true) {
				if (m_scanner->currentToken() != Token::Comma &&
				    m_scanner->currentToken() != oppositeToken)
	...
	default:
		if (Token::isElementaryTypeName(token)) {
			...
			expression = nodeFactory.createNode<ElementaryTypeNameExpression>(
...
pair<vector<ASTPointer<Expression> >, vector<ASTPointer<ASTString> > >
...
Parser::LookAheadInfo Parser::peekStatementType() const {
	...
	// name followed by an identifier, we also have a variable declaration. If we
	// get an identifier followed by a "[" or ".", it can be both ("lib.type[9]
	// a;" or "variable.el[9] = 7;"). In all other cases, we have an expression
	// statement.
	Token::Value token(m_scanner->currentToken());
	bool mightBeTypeName =
		(Token::isElementaryTypeName(token) || token == Token::Identifier);
...
ASTPointer<TypeName> Parser::typeNameIndexAccessStructure(
	...
	ASTPointer<TypeName> type;
	if (auto typeName = dynamic_cast<ElementaryTypeNameExpression const *>(
		    _path.front().get())) {
...
ASTPointer<Expression> Parser::expressionFromIndexAccessStructure(
	...
	for (size_t i = 1; i < _path.size(); ++i) {
		...
		expression = nodeFactory.createNode<MemberAccess>(
			expression, make_shared<ASTString>(identifier.name()));
	}
...
string Parser::currentTokenName() {
	...
	if (Token::isElementaryTypeName(
		    token)) // for the sake of accuracy in reporting
	{
		ElementaryTypeNameToken elemTypeName =
			m_scanner->currentElementaryTypeNameToken();
...
Token::Value Parser::expectAssignmentOperator() {
	...
	if (!Token::isAssignmentOp(op))
		fatalParserError(string("Expected assignment operator,  got '") +
...
ASTPointer<ASTString> Parser::expectIdentifierToken() {
	...
	if (id != Token::Identifier)
		fatalParserError(string("Expected identifier, got '") + currentTokenName() +
		                 string("'"));
...
ASTPointer<ASTString> Parser::getLiteralAndAdvance() {
	ASTPointer<ASTString> identifier =
		make_shared<ASTString>(m_scanner->currentLiteral());
	m_scanner->next();
	return identifier;
}
