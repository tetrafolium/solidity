 ...
 * @date 2014
 * Parser part that determines the declarations corresponding to names and the
...
NameAndTypeResolver::NameAndTypeResolver(
    ...
    : m_scopes(_scopes), m_errorReporter(_errorReporter) {
  if (!m_scopes[nullptr])
...
bool NameAndTypeResolver::registerDeclarations(ASTNode &_sourceUnit,
  ...
  // The helper registers all declarations in m_scopes as a side-effect of its
  // construction.
  try {
    DeclarationRegistrationHelper registrar(m_scopes, _sourceUnit,
                                            m_errorReporter, _currentScope);
...
bool NameAndTypeResolver::performImports(
    SourceUnit &_sourceUnit,
    map<string, SourceUnit const *> const &_sourceUnits) {
    ...
    if (auto imp = dynamic_cast<ImportDirective const *>(node.get())) {
      ...
      if (!_sourceUnits.count(path)) {
        m_errorReporter.declarationError(
            imp->location(), "Import \"" + path + "\" (referenced as \"" +
                                 imp->path() + "\") not found.");
        error = true;
        continue;
      }
      ...
      if (!imp->symbolAliases().empty())
        for (auto const &alias : imp->symbolAliases()) {
          auto declarations =
              scope->second->resolveName(alias.first->name(), false);
          if (declarations.empty()) {
            m_errorReporter.declarationError(
                imp->location(),
                "Declaration \"" + alias.first->name() + "\" not found in \"" +
                    path + "\" (referenced as \"" + imp->path() + "\").");
            error = true;
          } else
...
bool NameAndTypeResolver::updateDeclaration(Declaration const &_declaration) {
  try {
    ...
    solAssert(_declaration.scope() == nullptr,
              "Updated declaration outside global scope.");
  } catch (FatalError const &) {
    if (m_errorReporter.errors().empty())
      throw; // Something is weird here, rather throw again.
    return false;
...
NameAndTypeResolver::resolveName(ASTString const &_name,
                                 ASTNode const *_scope) const {
...
vector<Declaration const *>
NameAndTypeResolver::nameFromCurrentScope(ASTString const &_name,
                                          bool _recursive) const {
  return m_currentScope->resolveName(_name, _recursive);
}

Declaration const *
NameAndTypeResolver::pathFromCurrentScope(vector<ASTString> const &_path,
                                          bool _recursive) const {
  solAssert(!_path.empty(), "");
  vector<Declaration const *> candidates =
      m_currentScope->resolveName(_path.front(), _recursive);
...
vector<Declaration const *> NameAndTypeResolver::cleanedDeclarations(
  ...
  for (Declaration const *declaration : _declarations) {
    ...
    // the declaration is functionDefinition, eventDefinition or a
    // VariableDeclaration while declarations > 1
    solAssert(
    ...
    solAssert(functionType,
              "Failed to determine the function type of the overloaded.");
    ...
    if (uniqueFunctions.end() ==
        find_if(uniqueFunctions.begin(), uniqueFunctions.end(),
                [&](Declaration const *d) {
                  shared_ptr<FunctionType const> newFunctionType{
                      d->functionType(false)};
                  if (!newFunctionType)
                    newFunctionType = d->functionType(true);
                  return newFunctionType &&
                         functionType->hasEqualArgumentTypes(*newFunctionType);
                }))
  ...
  for (auto const &instruction : c_instructions) {
    ...
    for (Declaration const *const declaration : declarations) {
      ...
      if (dynamic_cast<MagicVariableDeclaration const *const>(declaration))
        ...
        continue;
      m_errorReporter.warning(declaration->location(),
                              "Variable is shadowed in inline assembly by an "
                              "instruction of the same name");
    }
  }
}

bool NameAndTypeResolver::resolveNamesAndTypesInternal(
    ASTNode &_node, bool _resolveInsideCode) {
  if (ContractDefinition *contract =
          dynamic_cast<ContractDefinition *>(&_node)) {
    bool success = true;
    m_currentScope = m_scopes[contract->scope()].get();
    solAssert(!!m_currentScope, "");

    for (ASTPointer<InheritanceSpecifier> const &baseContract :
    ...
    return ReferencesResolver(m_errorReporter, *this, _resolveInsideCode)
        .resolve(_node);
  }
}

void NameAndTypeResolver::importInheritedScope(
      ...
      if (declaration->scope() == &_base &&
        ...
        if (!m_currentScope->registerDeclaration(*declaration)) {
          ...
          if (dynamic_cast<VariableDeclaration const *>(declaration) &&
              dynamic_cast<VariableDeclaration const *>(conflictingDeclaration))
            continue;
          ...
          if (declaration->location().start <
              conflictingDeclaration->location().start) {
...
void NameAndTypeResolver::linearizeBaseContracts(
    ContractDefinition &_contract) {
  // order in the lists is from derived to base
  // list of lists to linearize, the last element is the list of direct bases
  list<list<ContractDefinition const *>> input(
      1, list<ContractDefinition const *>{});
  for (ASTPointer<InheritanceSpecifier> const &baseSpecifier :
       _contract.baseContracts()) {
    UserDefinedTypeName const &baseName = baseSpecifier->name();
    auto base = dynamic_cast<ContractDefinition const *>(
        baseName.annotation().referencedDeclaration);
    if (!base)
      m_errorReporter.fatalTypeError(baseName.location(), "Contract expected.");
    // "push_front" has the effect that bases mentioned later can overwrite
    // members of bases mentioned earlier
    input.back().push_front(base);
    vector<ContractDefinition const *> const &basesBases =
        base->annotation().linearizedBaseContracts;
    if (basesBases.empty())
      m_errorReporter.fatalTypeError(
          baseName.location(),
          "Definition of base has to precede definition of derived contract");
    input.push_front(
        list<ContractDefinition const *>(basesBases.begin(), basesBases.end()));
  }
  input.back().push_front(&_contract);
  vector<ContractDefinition const *> result = cThreeMerge(input);
  if (result.empty())
    m_errorReporter.fatalTypeError(
        _contract.location(), "Linearization of inheritance graph impossible");
  _contract.annotation().linearizedBaseContracts = result;
  _contract.annotation().contractDependencies.insert(result.begin() + 1,
                                                     result.end());
}

template <class _T>
vector<_T const *>
NameAndTypeResolver::cThreeMerge(list<list<_T const *>> &_toMerge) {
  ...
  // returns the next candidate to append to the linearized list or nullptr on
  // failure
  ...
  _toMerge.remove_if(
      [](list<_T const *> const &_bases) { return _bases.empty(); });
...
DeclarationRegistrationHelper::DeclarationRegistrationHelper(
    map<ASTNode const *, shared_ptr<DeclarationContainer>> &_scopes,
    ASTNode &_astRoot, ErrorReporter &_errorReporter,
    ASTNode const *_currentScope)
    : m_scopes(_scopes), m_currentScope(_currentScope),
...
bool DeclarationRegistrationHelper::registerDeclaration(
    DeclarationContainer &_container, Declaration const &_declaration,
    string const *_name, SourceLocation const *_errorLocation,
    bool _warnOnShadow, ErrorReporter &_errorReporter) {
  if (!_errorLocation)
    _errorLocation = &_declaration.location();

  Declaration const *shadowedDeclaration = nullptr;
  if (_warnOnShadow && !_declaration.name().empty() &&
      _container.enclosingContainer())
    for (auto const *decl : _container.enclosingContainer()->resolveName(
             _declaration.name(), true))
      shadowedDeclaration = decl;

  if (!_container.registerDeclaration(_declaration, _name,
                                      !_declaration.isVisibleInContract())) {
    SourceLocation firstDeclarationLocation;
    SourceLocation secondDeclarationLocation;
    Declaration const *conflictingDeclaration =
        _container.conflictingDeclaration(_declaration, _name);
    solAssert(conflictingDeclaration, "");
    bool const comparable = _errorLocation->sourceName &&
                            conflictingDeclaration->location().sourceName &&
                            *_errorLocation->sourceName ==
                                *conflictingDeclaration->location().sourceName;
    if (comparable &&
        _errorLocation->start < conflictingDeclaration->location().start) {
    ...
    _errorReporter.declarationError(
        secondDeclarationLocation,
        SecondarySourceLocation().append("The previous declaration is here:",
                                         firstDeclarationLocation),
        "Identifier already declared.");
    return false;
  } else if (shadowedDeclaration) {
    if (dynamic_cast<MagicVariableDeclaration const *>(shadowedDeclaration))
      _errorReporter.warning(_declaration.location(),
      ...
      _errorReporter.warning(
          ...
          SecondarySourceLocation().append("The shadowed declaration is here:",
                                           shadowedLocation));
    }
  }
  return true;
...
bool DeclarationRegistrationHelper::visit(SourceUnit &_sourceUnit) {
  if (!m_scopes[&_sourceUnit])
    ...
    m_scopes[&_sourceUnit].reset(new DeclarationContainer(
        m_currentScope, m_scopes[m_currentScope].get()));
  m_currentScope = &_sourceUnit;
  return true;
}

void DeclarationRegistrationHelper::endVisit(SourceUnit &_sourceUnit) {
  _sourceUnit.annotation().exportedSymbols =
      m_scopes[&_sourceUnit]->declarations();
  closeCurrentScope();
}
...
bool DeclarationRegistrationHelper::visit(ImportDirective &_import) {
  ...
  if (!m_scopes[importee])
    m_scopes[importee].reset(
        new DeclarationContainer(nullptr, m_scopes[nullptr].get()));
...
void DeclarationRegistrationHelper::endVisit(
    VariableDeclarationStatement &_variableDeclarationStatement) {
  ...
  for (ASTPointer<VariableDeclaration> const &var :
       _variableDeclarationStatement.declarations())
    if (var)
...
void DeclarationRegistrationHelper::enterNewSubScope(
    Declaration const &_declaration) {
  map<ASTNode const *, shared_ptr<DeclarationContainer>>::iterator iter;
  bool newlyAdded;
  shared_ptr<DeclarationContainer> container(
      new DeclarationContainer(m_currentScope, m_scopes[m_currentScope].get()));
...
void DeclarationRegistrationHelper::closeCurrentScope() {
  solAssert(m_currentScope && m_scopes.count(m_currentScope),
            "Closed non-existing scope.");
  m_currentScope = m_scopes[m_currentScope]->enclosingNode();
}

void DeclarationRegistrationHelper::registerDeclaration(
    Declaration &_declaration, bool _opensScope) {
  solAssert(m_currentScope && m_scopes.count(m_currentScope),
            "No current scope.");
  ...
  if (dynamic_cast<StructDefinition const *>(m_currentScope) ||
  ...
  registerDeclaration(*m_scopes[m_currentScope], _declaration, nullptr, nullptr,
                      warnAboutShadowing, m_errorReporter);
