...
/**
 * The module that performs type analysis on the AST, checks the applicability
 * of operations on those types and stores errors for invalid operations.
 * Provides a way to retrieve the type of an AST node.
 */
class TypeChecker : private ASTConstVisitor {
public:
/// @param _errorReporter provides the error logging functionality.
TypeChecker(ErrorReporter &_errorReporter)
...
/// @returns the type of the given variable and throws if the type is not
/// present (this can happen for variables with non-explicit types before
/// their types are resolved)
TypePointer const &type(VariableDeclaration const &_variable) const;

private:
virtual bool visit(ContractDefinition const &_contract) override;
/// Checks that two functions defined in this contract with the same name have
/// different arguments and that there is at most one constructor.
...
void overrideError(FunctionDefinition const &function,
                   FunctionDefinition const &super, std::string message);
void checkContractAbstractFunctions(ContractDefinition const &_contract);
void checkContractAbstractConstructors(ContractDefinition const &_contract);
/// Checks that different functions with external visibility end up having
/// different external argument types (i.e. different signature).
void checkContractExternalTypeClashes(ContractDefinition const &_contract);
/// Checks that all requirements for a library are fulfilled if this is a
/// library.
void checkLibraryRequirements(ContractDefinition const &_contract);
/// Checks (and warns) if a tuple assignment might cause unexpected overwrites
/// in storage. Should only be called if the left hand side is tuple-typed.
...
/// We need to do this manually because we want to pass the bases of the
/// current contract in case this is a base constructor call.
void visitManually(ModifierInvocation const &_modifier,
...
template <class T>
void findDuplicateDefinitions(
	std::map<std::string, std::vector<T> > const &_definitions,
	std::string _message);

bool contractDependenciesAreCyclic(
	ContractDefinition const &_contract,
	std::set<ContractDefinition const *> const &_seenContracts =
		std::set<ContractDefinition const *>()) const;
...
/// Runs type checks on @a _expression to infer its type and then checks that
/// it is implicitly convertible to @a _expectedType.
