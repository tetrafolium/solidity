...
bool SemVerMatchExpression::MatchComponent::matches(
    SemVerVersion const &_version) const {
...
bool SemVerMatchExpression::Conjunction::matches(
    SemVerVersion const &_version) const {
...
SemVerMatchExpression::MatchComponent
SemVerMatchExpressionParser::parseMatchComponent() {
  SemVerMatchExpression::MatchComponent component;
  Token::Value token = currentToken();
  if (token == Token::BitXor || token == Token::BitNot ||
      token == Token::LessThan || token == Token::LessThanOrEqual ||
      token == Token::GreaterThan || token == Token::GreaterThanOrEqual ||
      token == Token::Assign) {
  ...
  // TODO we do not support pre and build version qualifiers for now in match
  // expressions (but we do support them in the actual versions)
  return component;
}
