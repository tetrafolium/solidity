...
Declaration const *
DeclarationContainer::conflictingDeclaration(Declaration const &_declaration,
                                             ASTString const *_name) const {
  if (!_name)
  ...
  if (dynamic_cast<FunctionDefinition const *>(&_declaration) ||
    ...
    // check that all other declarations with the same name are functions or a
    // public state variable or events. And then check that the signatures are
    // different.
    for (Declaration const *declaration : declarations) {
      if (auto variableDeclaration =
              dynamic_cast<VariableDeclaration const *>(declaration)) {
        if (variableDeclaration->isStateVariable() &&
...
bool DeclarationContainer::registerDeclaration(Declaration const &_declaration,
                                               ASTString const *_name,
                                               bool _invisible, bool _update) {
  if (!_name)
    _name = &_declaration.name();
  if (_name->empty())
    return true;
  ...
  if (_update) {
    solAssert(!dynamic_cast<FunctionDefinition const *>(&_declaration),
...
DeclarationContainer::resolveName(ASTString const &_name,
