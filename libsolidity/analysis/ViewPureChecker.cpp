...
public:
  explicit AssemblyViewPureChecker(
      std::function<void(StateMutability, SourceLocation const &)>
          _reportMutability)
...
private:
  std::function<void(StateMutability, SourceLocation const &)>
      m_reportMutability;
  void checkInstruction(SourceLocation _location,
...
bool ViewPureChecker::check() {
  ...
  for (auto const &node : m_ast) {
    ...
    solAssert(source, "");
    bool enforceView = source->annotation().experimentalFeatures.count(
        ExperimentalFeature::V050);
    for (ContractDefinition const *c :
         source->filteredNodes<ContractDefinition>(source->nodes()))
...
void ViewPureChecker::endVisit(FunctionDefinition const &_funDef) {
  ...
  if (m_currentBestMutability < _funDef.stateMutability() &&
      _funDef.stateMutability() != StateMutability::Payable &&
      _funDef.isImplemented() && !_funDef.isConstructor() &&
      !_funDef.isFallback() && !_funDef.annotation().superFunction)
...
void ViewPureChecker::endVisit(Identifier const &_identifier) {
  ...
  if (VariableDeclaration const *varDecl =
  ...
  } else if (MagicVariableDeclaration const *magicVar =
...
void ViewPureChecker::reportMutability(StateMutability _mutability,
                                       SourceLocation const &_location) {
  if (m_currentFunction && m_currentFunction->stateMutability() < _mutability) {
    string text;
    if (_mutability == StateMutability::View)
      text = "Function declared as pure, but this expression (potentially) "
             "reads from the "
    ...
    solAssert(m_currentFunction->stateMutability() == StateMutability::View ||
                  m_currentFunction->stateMutability() == StateMutability::Pure,
              "");
    if (!m_enforceViewWithError &&
...
void ViewPureChecker::endVisit(FunctionCall const &_functionCall) {
  ...
  StateMutability mut = dynamic_cast<FunctionType const &>(
...
void ViewPureChecker::endVisit(MemberAccess const &_memberAccess) {
  ...
  case Type::Category::Struct: {
    if (_memberAccess.expression().annotation().type->dataStoredIn(
...
void ViewPureChecker::endVisit(IndexAccess const &_indexAccess) {
  if (!_indexAccess.indexExpression())
    solAssert(_indexAccess.annotation().type->category() ==
                  Type::Category::TypeType,
              "");
  else {
    bool writes = _indexAccess.annotation().lValueRequested;
    if (_indexAccess.baseExpression().annotation().type->dataStoredIn(
            DataLocation::Storage))
      reportMutability(writes ? StateMutability::NonPayable
...
void ViewPureChecker::endVisit(ModifierInvocation const &_modifier) {
  ...
  if (ModifierDefinition const *mod = dynamic_cast<decltype(mod)>(
    ...
    solAssert(dynamic_cast<ContractDefinition const *>(
                  _modifier.name()->annotation().referencedDeclaration),
              "");
}
