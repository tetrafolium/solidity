...
enum class Instruction : uint8_t {
  ...
  JUMPTO = 0xb0, ///< alter the program counter to a jumpdest -- not part of
                 ///< Instructions.cpp
  JUMPIF,        ///< conditionally alter the program counter -- not part of
                 ///< Instructions.cpp
  JUMPV,         ///< alter the program counter to a jumpdest -- not part of
                 ///< Instructions.cpp
  JUMPSUB,       ///< alter the program counter to a beginsub -- not part of
                 ///< Instructions.cpp
  JUMPSUBV,      ///< alter the program counter to a beginsub -- not part of
                 ///< Instructions.cpp
  BEGINSUB,      ///< set a potential jumpsub destination -- not part of
                 ///< Instructions.cpp
  BEGINDATA,     ///< begin the data section -- not part of Instructions.cpp
  RETURNSUB,     ///< return to subroutine jumped from -- not part of
                 ///< Instructions.cpp
  PUTLOCAL,      ///< pop top of stack to local variable -- not part of
                 ///< Instructions.cpp
  GETLOCAL,      ///< push local variable to top of stack -- not part of
                 ///< Instructions.cpp
  ...
  CREATE2 = 0xfb,    ///< create new account with associated code at address
                     ///< `sha3(sender + salt + sha3(init code)) % 2**160`

  REVERT = 0xfd,  ///< halt execution, revert state and return output data
  INVALID = 0xfe, ///< invalid instruction for expressing runtime errors (e.g.,
                  ///< division-by-zero)
  SELFDESTRUCT =
...
inline Instruction pushInstruction(unsigned _number) {
  assertThrow(1 <= _number && _number <= 32, InvalidOpcode,
              std::string("Invalid PUSH instruction requested (") +
...
inline Instruction dupInstruction(unsigned _number) {
  assertThrow(1 <= _number && _number <= 16, InvalidOpcode,
              std::string("Invalid DUP instruction requested (") +
                  std::to_string(_number) + ").");
...
inline Instruction swapInstruction(unsigned _number) {
  assertThrow(1 <= _number && _number <= 16, InvalidOpcode,
              std::string("Invalid SWAP instruction requested (") +
                  std::to_string(_number) + ").");
...
inline Instruction logInstruction(unsigned _number) {
  assertThrow(_number <= 4, InvalidOpcode,
              std::string("Invalid LOG instruction requested (") +
                  std::to_string(_number) + ").");
...
struct InstructionInfo {
  ...
  int additional; ///< Additional items required in memory for this instructions
                  ///< (only for PUSH).
  int args; ///< Number of items required on the stack for this instruction
            ///< (and, for the purposes of ret, the number taken from the
            ///< stack).
  int ret;  ///< Number of items placed (back) on the stack by this instruction,
            ///< assuming args items were removed.
  bool sideEffects; ///< false if the only effect on the execution environment
                    ///< (apart from gas usage) is a change to a topmost segment
                    ///< of the stack
