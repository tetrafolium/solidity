...
AssemblyItem AssemblyItem::toSubAssemblyTag(size_t _subId) const {
	assertThrow(data() < (u256(1) << 64), Exception,
	            "Tag already has subassembly set.");
...
bool AssemblyItem::canBeFunctional() const {
	switch (m_type) {
	case Operation:
		return !isDupInstruction(instruction()) &&
...
string AssemblyItem::toAssemblyText() const {
	...
	switch (type()) {
	case Operation: {
		assertThrow(isValidInstruction(instruction()), AssemblyException,
		            "Invalid instruction.");
	...
	case Tag:
		assertThrow(data() < 0x10000, AssemblyException,
		            "Declaration of sub-assembly tag.");
...
ostream &dev::eth::operator<<(ostream &_out, AssemblyItem const &_item) {
	...
	case Operation:
		...
		if (_item.instruction() == solidity::Instruction::JUMP ||
		    _item.instruction() == solidity::Instruction::JUMPI)
