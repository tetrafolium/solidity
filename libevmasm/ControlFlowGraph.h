...
/**
 * Identifier for a block, coincides with the tag number of an AssemblyItem but
 * adds a special ID for the initial block.
 */
class BlockId {
public:
BlockId() {
...
bool operator==(BlockId const &_other) const {
...
/**
 * Control flow block inside which instruction counter is always incremented by
 * one (except for possibly the last instruction).
 */
struct BasicBlock {
	...
	/// ID of the block that always follows this one (either non-branching part of
	/// JUMPI or flow into new block), or BlockId::invalid() otherwise
	BlockId next = BlockId::invalid();
	/// ID of the block that has to precede this one (because control flows into
	/// it).
	...
	/// Knowledge about the state when this block is entered. Intersection of all
	/// possible ways to enter this block.
 ...
 * ASSUMES THAT WE ONLY JUMP TO TAGS THAT WERE PREVIOUSLY PUSHED. THIS IS NOT
 * TRUE ANYMORE NOW THAT FUNCTION TAGS CAN BE STORED IN STORAGE.
 */
class ControlFlowGraph {
public:
/// Initializes the control flow graph.
/// @a _items has to persist across the usage of this class.
/// @a _joinKnowledge if true, reduces state knowledge to common base at the
/// join of two paths
explicit ControlFlowGraph(AssemblyItems const &_items,
                          bool _joinKnowledge = true)
	: m_items(_items), m_joinKnowledge(_joinKnowledge) {
}
/// @returns vector of basic blocks in the order they should be used in the
/// final code. Should be called only once.
