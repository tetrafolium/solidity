...
PathGasMeter::PathGasMeter(AssemblyItems const &_items) : m_items(_items) {
...
PathGasMeter::estimateMax(size_t _startIndex,
...
GasMeter::GasConsumption PathGasMeter::handleQueueItem() {
	...
	if (index >= m_items.size() || (index > 0 && m_items.at(index).type() != Tag))
		// Invalid jump usually provokes an out-of-gas exception, but we want to
		// give an upper bound on the gas that is needed without changing the
		// behaviour, so it is fine to return the current gas value.
		return gas;
	...
	for (; index < m_items.size() && !gas.isInfinite; ++index) {
		...
		if (item.type() == Tag || item == AssemblyItem(Instruction::JUMPDEST)) {
			// Do not allow any backwards jump. This is quite restrictive but should
			// work for the simplest things.
