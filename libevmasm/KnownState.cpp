 ...
 * @date 2015
 * Contains knowledge about the state of the virtual machine at a specific
 * instruction.
 */
...
KnownState::StoreOperation KnownState::feedItem(AssemblyItem const &_item,
                                                bool _copyItem) {
	StoreOperation op;
	if (_item.type() == Tag) {
		// can be ignored
		...
		if (_item.pushedValue())
			// only available after assembly stage, should not be used for
			// optimisation
			setStackElement(++m_stackHeight,
			                m_expressionClasses->find(*_item.pushedValue()));
		else
			setStackElement(++m_stackHeight,
			                m_expressionClasses->find(_item, {}, _copyItem));
	} else {
		Instruction instruction = _item.instruction();
		InstructionInfo info = instructionInfo(instruction);
		if (SemanticInformation::isDupInstruction(_item))
			setStackElement(m_stackHeight + 1,
			                stackElement(m_stackHeight - int(instruction) +
			                             int(Instruction::DUP1),
			                             _item.location()));
		else if (SemanticInformation::isSwapInstruction(_item))
			swapStackElements(m_stackHeight,
			                  m_stackHeight - 1 - int(instruction) +
			                  int(Instruction::SWAP1),
			                  _item.location());
			...
			if (_item.instruction() == Instruction::SSTORE)
				...
				setStackElement(m_stackHeight + _item.deposit(),
				...
				bool invMem =
					SemanticInformation::invalidatesMemory(_item.instruction());
				bool invStor =
					SemanticInformation::invalidatesStorage(_item.instruction());
				// We could be a bit more fine-grained here (CALL only invalidates part
				// of memory, etc), but we do not for now.
				if (invMem)
					resetMemory();
				if (invStor)
					resetStorage();
				if (invMem || invStor)
					m_sequenceNumber +=
						2; // Increment by two because it can read and write
...
/// Helper function for KnownState::reduceToCommonKnowledge, removes everything
/// from _this which is not in or not equal to the value in _other.
template <class _Mapping>
void intersect(_Mapping &_this, _Mapping const &_other) {
...
bool KnownState::operator==(KnownState const &_other) const {
	...
	for (; thisIt != m_stackElements.cend() &&
	     otherIt != _other.m_stackElements.cend();
	     ++thisIt, ++otherIt)
		if (thisIt->first - stackDiff != otherIt->first ||
		    thisIt->second != otherIt->second)
			return false;
	return (thisIt == m_stackElements.cend() &&
	        otherIt == _other.m_stackElements.cend());
}

ExpressionClasses::Id
KnownState::stackElement(int _stackHeight, SourceLocation const &_location) {
	if (m_stackElements.count(_stackHeight))
		return m_stackElements.at(_stackHeight);
	// Stack element not found (not assigned yet), create new unknown equivalence
	// class.
	return m_stackElements[_stackHeight] = m_expressionClasses->find(
		       AssemblyItem(UndefinedItem, _stackHeight, _location));
}

KnownState::Id
KnownState::relativeStackElement(int _stackOffset,
...
void KnownState::swapStackElements(int _stackHeightA, int _stackHeightB,
                                   SourceLocation const &_location) {
	assertThrow(_stackHeightA != _stackHeightB, OptimizerException,
	            "Swap on same stack elements.");
...
KnownState::StoreOperation
KnownState::storeInStorage(Id _slot, Id _value,
                           SourceLocation const &_location) {
	...
	// Copy over all values (i.e. retain knowledge about them) where we know that
	// this store operation will not destroy the knowledge. Specifically, we copy
	// storage locations we know are different from _slot or locations where we
	// know that the stored value is equal to _value.
	...
	Id id =
		m_expressionClasses->find(item, {_slot, _value}, true, m_sequenceNumber);
...
ExpressionClasses::Id
KnownState::loadFromStorage(Id _slot, SourceLocation const &_location) {
...
KnownState::StoreOperation
KnownState::storeInMemory(Id _slot, Id _value,
                          SourceLocation const &_location) {
	...
	StoreOperation operation(
		StoreOperation(StoreOperation::Memory, _slot, m_sequenceNumber, id));
...
ExpressionClasses::Id
KnownState::loadFromMemory(Id _slot, SourceLocation const &_location) {
...
KnownState::Id KnownState::applyKeccak256(Id _start, Id _length,
                                          SourceLocation const &_location) {
	...
	if (!l || *l > 128)
		return m_expressionClasses->find(keccak256Item, {_start, _length}, true,
		                                 m_sequenceNumber);

	vector<Id> arguments;
	for (u256 i = 0; i < *l; i += 32) {
		Id slot =
			m_expressionClasses->find(AssemblyItem(Instruction::ADD, _location),
	...
	if (all_of(arguments.begin(), arguments.end(), [this](Id _a) {
		return !!m_expressionClasses->knownConstant(_a);
	})) {
		bytes data;
		for (Id a : arguments)
			data += toBigEndian(*m_expressionClasses->knownConstant(a));
		data.resize(size_t(*l));
		v = m_expressionClasses->find(
			AssemblyItem(u256(dev::keccak256(data)), _location));
	} else
		v = m_expressionClasses->find(keccak256Item, {_start, _length}, true,
		                              m_sequenceNumber);
...
set<u256> KnownState::tagsInExpression(KnownState::Id _expressionId) {
	...
	ExpressionClasses::Expression expr =
		m_expressionClasses->representative(_expressionId);
