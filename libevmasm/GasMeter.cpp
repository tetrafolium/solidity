...
operator+=(GasConsumption const &_other) {
...
GasMeter::GasConsumption GasMeter::estimateMax(AssemblyItem const &_item,
	...
	case Operation: {
		...
		case Instruction::MSTORE:
			gas += memoryGas(
				classes.find(Instruction::ADD, {m_state->relativeStackElement(0),
		...
		case Instruction::KECCAK256:
			...
			gas += wordGas(GasCosts::keccak256WordGas,
			               m_state->relativeStackElement(-1));
			gas += memoryGas(0, -1);
			break;
		...
		case Instruction::LOG4: {
			...
			gas += memoryGas(0, -1);
			if (u256 const *value =
				    classes.knownConstant(m_state->relativeStackElement(-1)))
		...
		case Instruction::STATICCALL: {
			if (_includeExternalCosts)
				...
				if (u256 const *value =
					    classes.knownConstant(m_state->relativeStackElement(0)))
					gas += (*value);
				else
					gas = GasConsumption::infinite();
				...
				if (_item.instruction() == Instruction::DELEGATECALL ||
					...
					valueSize = 0;
				else if (!classes.knownZero(
						 m_state->relativeStackElement(-1 - valueSize)))
...
GasMeter::GasConsumption GasMeter::wordGas(u256 const &_multiplier,
                                           ExpressionClasses::Id _value) {
...
GasMeter::GasConsumption GasMeter::memoryGas(int _stackPosOffset,
                                             int _stackPosSize) {
	ExpressionClasses &classes = m_state->expressionClasses();
	if (classes.knownZero(m_state->relativeStackElement(_stackPosSize)))
		...
		return memoryGas(classes.find(
					 Instruction::ADD, {m_state->relativeStackElement(_stackPosOffset),
...
unsigned GasMeter::runGas(Instruction _instruction) {
	...
	case Tier::Special:
		return GasCosts::tier7Gas;
	case Tier::ExtCode:
		return GasCosts::extCodeGas;
	case Tier::Balance:
		return GasCosts::balanceGas;
	default:
		break;
	}
