...
bool BlockDeduplicator::deduplicate() {
	// Compares indices based on the suffix that starts there, ignoring tags and
	// stopping at opcodes that stop the control flow.

	// Virtual tag that signifies "the current block" and which is used to
	// optimise loops. We abort if this virtual tag actually exists.
	...
	function<bool(size_t, size_t)> comparator = [&](size_t _i, size_t _j) {
							    ...
							    // To compare recursive loops, we have to already unify PushTag opcodes of
							    // the block's own tag.
							    ...
							    BlockIterator first(m_items.begin() + _i, m_items.end(), &pushFirstTag,
							                        &pushSelf);
							    BlockIterator second(m_items.begin() + _j, m_items.end(), &pushSecondTag,
							                         &pushSelf);
...
bool BlockDeduplicator::applyTagReplacement(
	AssemblyItems &_items, map<u256, u256> const &_replacements,
	size_t _subId) {
	bool changed = false;
...
BlockDeduplicator::BlockIterator &BlockDeduplicator::BlockIterator::
operator++() {
	if (it == end)
		return *this;
