...
bool ExpressionClasses::Expression::
operator<(ExpressionClasses::Expression const &_other) const {
  ...
  if (type != otherType)
    ...
    return std::tie(item->data(), arguments, sequenceNumber) <
           std::tie(_other.item->data(), _other.arguments,
                    _other.sequenceNumber);
}

ExpressionClasses::Id ExpressionClasses::find(AssemblyItem const &_item,
...
void ExpressionClasses::forceEqual(ExpressionClasses::Id _id,
...
ExpressionClasses::newClass(SourceLocation const &_location) {
  Expression exp;
  exp.id = m_representatives.size();
  exp.item = storeItem(
      AssemblyItem(UndefinedItem, (u256(1) << 255) + exp.id, _location));
  m_representatives.push_back(exp);
  m_expressions.insert(exp);
  return exp.id;
}

bool ExpressionClasses::knownToBeDifferent(ExpressionClasses::Id _a,
                                           ExpressionClasses::Id _b) {
  // Try to simplify "_a - _b" and return true iff the value is a non-zero
  // constant.
  return knownNonZero(find(Instruction::SUB, {_a, _b}));
}

bool ExpressionClasses::knownToBeDifferentBy32(ExpressionClasses::Id _a,
                                               ExpressionClasses::Id _b) {
  // Try to simplify "_a - _b" and return true iff the value is at least 32 away
  // from zero.
...
bool ExpressionClasses::knownNonZero(Id _c) {
  return Pattern(u256(0)).matches(
      representative(find(Instruction::ISZERO, {_c})), *this);
}
...
ExpressionClasses::Id ExpressionClasses::tryToSimplify(Expression const &_expr,
  ...
  if (auto match = rules.findFirstMatch(_expr, *this)) {
    ...
    return rebuildExpression(
        ExpressionTemplate(match->second(), _expr.item->location()));
  }

  if (!_secondRun && _expr.arguments.size() == 2 &&
      SemanticInformation::isCommutativeOperation(*_expr.item)) {
...
ExpressionClasses::Id
ExpressionClasses::rebuildExpression(ExpressionTemplate const &_template) {
  if (_template.hasId)
    return _template.id;
