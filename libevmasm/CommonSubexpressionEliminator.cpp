...
vector<AssemblyItem> CommonSubexpressionEliminator::getOptimizedItems() {
  ...
  for (int height = minHeight; height <= m_initialState.stackHeight(); ++height)
    initialStackContents[height] =
        m_initialState.stackElement(height, SourceLocation());
  for (int height = minHeight; height <= m_state.stackHeight(); ++height)
    targetStackContents[height] =
        m_state.stackElement(height, SourceLocation());

  AssemblyItems items =
      CSECodeGenerator(m_state.expressionClasses(), m_storeOperations)
          .generateCode(m_initialState.sequenceNumber(),
                        m_initialState.stackHeight(), initialStackContents,
                        targetStackContents);
  if (m_breakingItem)
...
void CommonSubexpressionEliminator::optimizeBreakingItem() {
  ...
  if (*m_breakingItem == AssemblyItem(Instruction::JUMPI)) {
    ...
    Id condition =
        m_state.stackElement(m_state.stackHeight() - 1, itemLocation);
...
AssemblyItems
CSECodeGenerator::generateCode(unsigned _initialSequenceNumber,
  ...
  // generate the dependency graph starting from final storage and memory writes
  // and target stack contents
  ...
  for (auto const &p : m_neededBy)
    for (auto id : {p.first, p.second})
      if (unsigned seqNr =
              m_expressionClasses.representative(id).sequenceNumber) {
        if (seqNr < _initialSequenceNumber)
          // Invalid sequenced operation.
          // @todo quick fix for now. Proper fix needs to choose representative
          // with higher sequence number during dependency analyis.
  ...
  for (auto const &targetItem : m_targetStack) {
    if (m_stack.count(targetItem.first) &&
        m_stack.at(targetItem.first) == targetItem.second)
      continue; // already there
    generateClassElement(targetItem.second);
    assertThrow(!m_classPositions[targetItem.second].empty(),
                OptimizerException, "");
    ...
    if (m_expressionClasses.representative(targetItem.second).item)
      sourceLocation = m_expressionClasses.representative(targetItem.second)
                           .item->location();
  ...
  assertThrow(finalHeight == m_stackHeight, OptimizerException,
              "Incorrect final stack height.");
...
void CSECodeGenerator::addDependencies(Id _c) {
  ...
  if (expr.item->type() == UndefinedItem)
    BOOST_THROW_EXCEPTION(
        // If this exception happens, we need to find a different way to
        // generate the compound expression.
        ItemNotAvailableException()
        << errinfo_comment("Undefined item requested but not available."));
  ...
  if (expr.item && expr.item->type() == Operation &&
    ...
    // this loads an unknown value from storage or memory and thus, in addition
    // to its arguments, depends on all store operations to addresses where we
    // do not know that they are different that occur before this load
    StoreOperation::Target target =
        expr.item->instruction() == Instruction::SLOAD ? StoreOperation::Storage
    ...
    for (auto const &p : m_storeOperations) {
      ...
      case Instruction::KECCAK256: {
        ...
        Id offsetToStart =
            m_expressionClasses.find(offsetInstr, {slot, slotToLoadFrom});
        ...
        if (l && *l == 0)
          ...
          // but it is probably ok for the optimizer to produce wrong code for
          // such cases which cannot be executed anyway because of the
          // non-payable price.
          if (u2s(*o) <= -32)
            knownToBeIndependent = true;
...
void CSECodeGenerator::generateClassElement(Id _c, bool _allowSequenced) {
  ...
  if (m_classPositions.count(_c)) {
    assertThrow(!m_classPositions[_c].empty(), OptimizerException,
                "Element already removed but still needed.");
    return;
  }
  ExpressionClasses::Expression const &expr =
      m_expressionClasses.representative(_c);
  assertThrow(_allowSequenced || expr.sequenceNumber == 0, OptimizerException,
              "Sequence constrained operation requested out of sequence.");
  assertThrow(expr.item, OptimizerException,
              "Non-generated expression without item.");
  assertThrow(expr.item->type() != UndefinedItem, OptimizerException,
  ...
  // The arguments are somewhere on the stack now, so it remains to move them at
  // the correct place. This is quite difficult as sometimes, the values also
  // have to removed in this process (if canBeRemoved() returns true) and the
  // two arguments can be equal. For now, this is implemented for every single
  // case for combinations of up to two arguments manually.
  if (arguments.size() == 1) {
    ...
    assertThrow(arguments.size() <= 2, OptimizerException,
                "Opcodes with more than two arguments not implemented yet.");
  for (size_t i = 0; i < arguments.size(); ++i)
    assertThrow(m_stack[m_stackHeight - i] == arguments[i], OptimizerException,
  ...
  if (expr.item->type() != Operation ||
      instructionInfo(expr.item->instruction()).ret == 1) {
    m_stack[m_stackHeight] = _c;
    m_classPositions[_c].insert(m_stackHeight);
  } else {
    assertThrow(instructionInfo(expr.item->instruction()).ret == 0,
                OptimizerException, "Invalid number of return values.");
    m_classPositions[_c]; // ensure it is created to mark the expression as
                          // generated
  }
}

int CSECodeGenerator::classElementPosition(Id _id) const {
  assertThrow(m_classPositions.count(_id) && !m_classPositions.at(_id).empty(),
              OptimizerException, "Element requested but is not present.");
  return *max_element(m_classPositions.at(_id).begin(),
                      m_classPositions.at(_id).end());
}
...
bool CSECodeGenerator::canBeRemoved(Id _element, Id _result,
  ...
  if (m_finalClasses.count(_element))
    // It is part of the target stack. It can be removed if it is a copy that is
    // not in the target position.
    return haveCopy && (!m_targetStack.count(_fromPosition) ||
                        m_targetStack[_fromPosition] != _element);
  else if (!haveCopy) {
    // Can be removed unless it is needed by a class that has not been computed
    // yet. Note that m_classPositions also includes classes that were deleted
    // in the meantime.
...
void CSECodeGenerator::appendDup(int _fromPosition,
  ...
  assertThrow(instructionNum <= 16, StackTooDeepException,
