...
void Assembly::append(Assembly const &_a) {
	...
	// This does not transfer the names of named tags on purpose. The tags
	// themselves are transferred, but their names are only available inside the
	// assembly.
...
string locationFromSources(StringMap const &_sourceCodes,
                           SourceLocation const &_location) {
	if (_location.isEmpty() || _sourceCodes.empty() ||
	    _location.start >= _location.end || _location.start < 0)
		return "";
...
public:
Functionalizer(ostream &_out, string const &_prefix,
...
void feed(AssemblyItem const &_item) {
	...
	if (!(_item.canBeFunctional() && _item.returnValues() <= 1 &&
		...
		flush();
		m_out << m_prefix << (_item.type() == Tag ? "" : "  ")
		      << _item.toAssemblyText() << endl;
		return;
	}
...
void printLocation() {
	...
	if (!m_location.isEmpty())
		m_out << ":"
		      << to_string(m_location.start) + ":" + to_string(m_location.end);
...
void Assembly::assemblyStream(ostream &_out, string const &_prefix,
	...
	if (!m_data.empty() || !m_subs.empty()) {
		...
		for (auto const &i : m_data)
			if (u256(i.first) >= m_subs.size())
				_out << _prefix << "data_" << toHex(u256(i.first)) << " "
				     << toHex(i.second) << endl;
...
Json::Value Assembly::createJsonValue(string _name, int _begin, int _end,
                                      string _value, string _jumpType) {
	Json::Value value;
	value["name"] = _name;
	value["begin"] = _begin;
	value["end"] = _end;
...
Json::Value Assembly::assemblyJSON(StringMap const &_sourceCodes) const {
	...
	for (AssemblyItem const &i : m_items) {
		switch (i.type()) {
		case Operation:
			collection.append(createJsonValue(instructionInfo(i.instruction()).name,
			                                  i.location().start, i.location().end,
			                                  i.getJumpTypeAsString()));
			break;
		case Push:
			collection.append(
				createJsonValue("PUSH", i.location().start, i.location().end,
				                toStringInHex(i.data()), i.getJumpTypeAsString()));
			break;
		case PushString:
			collection.append(createJsonValue("PUSH tag", i.location().start,
			                                  i.location().end,
			                                  m_strings.at((h256)i.data())));
			break;
		case PushTag:
			if (i.data() == 0)
				collection.append(createJsonValue("PUSH [ErrorTag]", i.location().start,
				                                  i.location().end, ""));
			else
				collection.append(createJsonValue("PUSH [tag]", i.location().start,
			...
			break;
		case PushSub:
			collection.append(createJsonValue("PUSH [$]", i.location().start,
			                                  i.location().end,
			                                  dev::toString(h256(i.data()))));
			break;
		case PushSubSize:
			collection.append(createJsonValue("PUSH #[$]", i.location().start,
			                                  i.location().end,
		...
		case PushLibraryAddress:
			collection.append(createJsonValue("PUSHLIB", i.location().start,
			                                  i.location().end,
			                                  m_libraries.at(h256(i.data()))));
			break;
		case Tag:
			collection.append(createJsonValue("tag", i.location().start,
			                                  i.location().end, string(i.data())));
			collection.append(
				createJsonValue("JUMPDEST", i.location().start, i.location().end));
			break;
		case PushData:
			collection.append(createJsonValue("PUSH data", i.location().start,
			                                  i.location().end,
...
map<u256, u256>
Assembly::optimiseInternal(OptimiserSettings const &_settings,
	...
	for (size_t subId = 0; subId < m_subs.size(); ++subId) {
		...
		map<u256, u256> subTagReplacements = m_subs[subId]->optimiseInternal(
			settings, JumpdestRemover::referencedTags(m_items, subId));
	...
	for (unsigned count = 1; count > 0; ) {
		...
		if (_settings.runPeephole) {
			...
			while (peepOpt.optimise()) {
				count++;
				assertThrow(count < 64000, OptimizerException,
				            "Peephole optimizer seems to be stuck.");
			}
		}

		// This only modifies PushTags, we have to run again to actually remove
		// code.
		if (_settings.runDeduplicate) {
			BlockDeduplicator dedup(m_items);
			if (dedup.deduplicate()) {
				tagReplacements.insert(dedup.replacedTags().begin(),
				                       dedup.replacedTags().end());
				count++;
			}
		}

		if (_settings.runCSE) {
			// Control flow graph optimization has been here before but is disabled
			// because it assumes we only jump to tags that are pushed. This is not
			// the case anymore with function types that can be stored in storage.
...
LinkerObject const &Assembly::assemble() const {
	...
	vector<unsigned> sizeRef; ///< Pointers to code locations where the size of
	                          ///< the program is inserted
	unsigned bytesPerTag = dev::bytesRequired(bytesRequiredForCode);
	byte tagPush = (byte)Instruction::PUSH1 - 1 + bytesPerTag;

	unsigned bytesRequiredIncludingData =
		bytesRequiredForCode + 1 + m_auxiliaryData.size();
	for (auto const &sub : m_subs)
	...
	for (AssemblyItem const &i : m_items) {
		...
		case Tag:
			...
			assertThrow(i.splitForeignPushTag().first == size_t(-1),
			            AssemblyException, "Foreign tag.");
			assertThrow(ret.bytecode.size() < 0xffffffffL, AssemblyException,
			            "Tag too large.");
			assertThrow(m_tagPositionsInBytecode[size_t(i.data())] == size_t(-1),
			            AssemblyException, "Duplicate tag position.");
	...
	for (auto const &i : tagRef) {
		...
		assertThrow(subId == size_t(-1) || subId < m_subs.size(), AssemblyException,
		            "Invalid sub id");
		std::vector<size_t> const &tagPositions =
			subId == size_t(-1) ? m_tagPositionsInBytecode
			: m_subs[subId]->m_tagPositionsInBytecode;
		assertThrow(tagId < tagPositions.size(), AssemblyException,
		            "Reference to non-existing tag.");
		size_t pos = tagPositions[tagId];
		assertThrow(pos != size_t(-1), AssemblyException,
		            "Reference to tag without position.");
		assertThrow(dev::bytesRequired(pos) <= bytesPerTag, AssemblyException,
