...
enum AssemblyItemType {
	...
	Tag,
	PushData,
	PushLibraryAddress ///< Push a currently unknown address of another (library)
	                   ///< contract.
};
...
AssemblyItem(solidity::Instruction _i,
             SourceLocation const &_location = SourceLocation())
	: m_type(Operation), m_instruction(_i), m_location(_location) {
}
AssemblyItem(AssemblyItemType _type, u256 _data = 0,
             SourceLocation const &_location = SourceLocation())
	: m_type(_type), m_location(_location) {
	if (m_type == Operation)
...
/// Converts the tag to a subassembly tag. This has to be called in order to
/// move a tag across assemblies.
/// @param _subId the identifier of the subassembly the tag is taken from.
AssemblyItem toSubAssemblyTag(size_t _subId) const;
/// @returns splits the data of the push tag into sub assembly id and actual
/// tag id. The sub assembly id of non-foreign push tags is -1.
...
u256 const &data() const {
	assertThrow(m_type != Operation, Exception, "");
	return *m_data;
}
...
/// @returns an upper bound for the number of bytes required by this item,
/// assuming that the value of a jump tag takes @a _addressLength bytes.
unsigned bytesRequired(unsigned _addressLength) const;
int arguments() const;
int returnValues() const;
int deposit() const {
	return returnValues() - arguments();
}
...
void setJumpType(JumpType _jumpType) {
...
u256 const *pushedValue() const {
	return m_pushedValue.get();
}
...
/// Pushed value for operations with data to be determined during assembly
/// stage, e.g. PushSubSize, PushTag, PushSub, etc.
mutable std::shared_ptr<u256> m_pushedValue;
};

using AssemblyItems = std::vector<AssemblyItem>;

inline size_t bytesRequired(AssemblyItems const &_items,
