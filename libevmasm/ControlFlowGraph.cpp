...
BlockId::BlockId(u256 const &_id) : m_id(unsigned(_id)) {
  assertThrow(_id < initial().m_id, OptimizerException,
...
void ControlFlowGraph::resolveNextLinks() {
  ...
  for (auto &idAndBlock : m_blocks) {
    ...
    case BasicBlock::EndType::HANDOVER:
      assertThrow(blockByBeginPos.count(block.end), OptimizerException,
...
void ControlFlowGraph::setPrevLinks() {
  for (auto &idAndBlock : m_blocks) {
    ...
    case BasicBlock::EndType::JUMPI:
    case BasicBlock::EndType::HANDOVER:
      assertThrow(!m_blocks.at(block.next).prev, OptimizerException,
  ...
  // If block ends with jump to not yet linked block, link them removing the
  // jump
  for (auto &idAndBlock : m_blocks) {
    BlockId blockId = idAndBlock.first;
    BasicBlock &block = idAndBlock.second;
    if (block.endType != BasicBlock::EndType::JUMP ||
        block.end - block.begin < 2)
      continue;
    ...
    if (m_blocks.count(nextId) && m_blocks.at(nextId).prev)
      continue;
    bool hasLoop = false;
    for (BlockId id = nextId; id && m_blocks.count(id) && !hasLoop;
         id = m_blocks.at(id).next)
      hasLoop = (id == blockId);
    ...
    block.next = nextId;
    block.end -= 2;
    assertThrow(!block.pushedTags.empty() && block.pushedTags.back() == nextId,
                OptimizerException,
...
void ControlFlowGraph::gatherKnowledge() {
  ...
  auto addWorkQueueItem = [&](WorkQueueItem const &_currentItem, BlockId _to,
                              KnownStatePointer const &_state) {
    WorkQueueItem item;
    item.blockId = _to;
  ...
  while (!workQueue.empty()) {
    ...
    workQueue.pop_back();
    //@todo we might have to do something like incrementing the sequence number
    //for each JUMPDEST
    ...
    if (block.startState) {
      // We call reduceToCommonKnowledge even in the non-join setting to get the
      // correct sequence number
      if (!m_joinKnowledge)
        state->reset();
      state->reduceToCommonKnowledge(*block.startState,
                                     !item.blocksSeen.count(item.blockId));
    ...
    // Feed all items except for the final jump yet because it will erase the
    // target tag.
    unsigned pc = block.begin;
    while (pc < block.end &&
           !SemanticInformation::altersControlFlow(m_items.at(pc)))
    ...
    if (block.endType == BasicBlock::EndType::JUMP ||
      ...
      assertThrow(block.begin <= pc && pc == block.end - 1, OptimizerException,
                  "");
      //@todo in the case of JUMPI, add knowledge about the condition to the
      //state
      ...
      if (tags.empty()) {
        if (!unknownJumpEncountered) {
          // We do not know the target of this jump, so we have to reset the
          // states of all JUMPDESTs.
  ...
  // Remove all blocks we never visited here. This might happen because a tag is
  // pushed but never used for a JUMP. Note that this invalidates some contents
  // of pushedTags
  for (auto it = m_blocks.begin(); it != m_blocks.end();)
...
BasicBlocks ControlFlowGraph::rebuildCode() {
  ...
  for (BlockId blockId = BlockId::initial(); blockId;
       blockId = blocksToAdd.empty() ? BlockId::invalid()
                                     : *blocksToAdd.begin()) {
    ...
    for (; blockId; blockId = m_blocks.at(blockId).next) {
      ...
      if (previousHandedOver && !pushes[blockId] &&
          m_items[block.begin].type() == Tag)
        ++block.begin;
      if (block.begin < block.end) {
